...
static int sci_glb_write(unsigned long reg, u32 val, u32 msk)
{
	int value;
	regmap_read(gpu_dfs_ctx.pmu_apb_reg_base, reg , &value);
	value = (value & ~msk) | val;
	regmap_write(gpu_dfs_ctx.pmu_apb_reg_base, reg, value);
	return 0;
}

static int sci_glb_set(unsigned long reg, u32 bit)
{
	int value;
	regmap_read(gpu_dfs_ctx.pmu_apb_reg_base, reg , &value);
	value = value | bit;
...
static int sci_glb_clr(unsigned long reg, u32 bit)
{
	int value;
	regmap_read(gpu_dfs_ctx.pmu_apb_reg_base, reg , &value);
	value = value & ~bit;
...
static inline void mali_freq_init(struct device *dev)
{
	...
	gpu_dfs_ctx.pmu_apb_reg_base = syscon_regmap_lookup_by_phandle(dev->of_node,
			"sprd,syscon-pmu-apb");
