...
public class ConversationListContext {
    ...
    public static ConversationListContext forBundle(Bundle bundle) {
        ...
        if (Utils.SUPPORT_LOCAL_SEARCH) {
            ...
            ConversationListContext listContext = new ConversationListContext(account, bundle.getString(EXTRA_SEARCH_QUERY), folder);
    ...
    public static ConversationListContext forFolder(Account account, Folder folder) {
        /* SPRD:bug475886 add local search function @{ */
        if (Utils.SUPPORT_LOCAL_SEARCH) {
            ConversationListContext context = new ConversationListContext(account, null, folder);
            context.setLocalSearch(false);
            context.setSearchApproach(SearchParams.INVILID_SEARCH_APPROACH);
            return context;
        } else {
    ...
    public static ConversationListContext forSearchQuery(Account account, Folder folder,
        ...
        /* SPRD:bug475886 add local search function @{ */
        if (Utils.SUPPORT_LOCAL_SEARCH) {
            ConversationListContext context = new ConversationListContext(account,
                    Preconditions.checkNotNull(query), folder);
            context.setLocalSearch(true);
            context.setSearchApproach(SearchParams.DEFAULT_SEARCH_APPROACH);
            return context;
        } else {
