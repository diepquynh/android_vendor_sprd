...
public class CropImageView extends ImageViewTouchBase {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mBitmapDisplayed.getBitmap() != null) {
            for (HighlightView hv : mHighlightViews) {
                ...
                hv.invalidate();
                if (hv.mIsFocused && CropImage.isNeedRelaout) {
                    centerBasedOnHighlightView(hv);
                }
                CropImage.isNeedRelaout = true;
            }
        }
    }
    ...
    private DismissPopUp mDismissPop;
    public void setDismissPop(DismissPopUp dismiss) {
        this.mDismissPop = dismiss;
    }
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (cropImage.mWaitingToPick) {
                ...
                mLastX = event.getX();
                mLastY = event.getY();
                    //just add for bug 430610 in 5.0
                if (!judgeVisible(event.getX(),mLastX)) {
    ...
    //just add for bug 430610 in 5.0
    private boolean judgeVisible(float oldPoint,float lastPoint){
        if((mMotionHighlightView.mCropRect.left == getLeft()&&lastPoint < oldPoint)|(mMotionHighlightView.mCropRect.right == getRight()&&lastPoint > oldPoint)){
            return false;
        }else{
            return true;
        }
