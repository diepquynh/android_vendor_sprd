...
restart:
	...
	*count = num;
	...
	else
	{
		ASSERT();
		hdr = (t_msg_hdr_t *)(readTo);
		printkd("tx_big_hdr_t size = 0x%x, t_msg_hdr_t size = 0x%x\n", sizeof(tx_big_hdr_t), sizeof(t_msg_hdr_t));
		printkd("cp2_txRam = 0x%x, fifo->mem = 0x%x, fifo->size = 0x%x\n", fifo->cp2_txRam, fifo->mem, fifo->size);
		printkd("RD = 0x%x, WT = 0x%x, wt = 0x%x\n", fifo->RD, fifo->WT, wt);
		printkd("readTo = 0x%x, readFrom = 0x%x, readMax = 0x%x, msg num = 0x%x, msg unit len = 0x%x\n",
				readTo, readFrom, readMax, num, TX_MSG_UNIT_LEN(hdr));
		print_hex_dump(KERN_DEBUG, "big_hdr:", DUMP_PREFIX_OFFSET, 16, 1, readFrom, sizeof(tx_big_hdr_t), true);
		print_hex_dump(KERN_DEBUG, "readTo msg_hdr:", DUMP_PREFIX_OFFSET, 16, 1, readTo, sizeof(t_msg_hdr_t), true);
		ret = -5;
	}
