...
import com.android.util.Assert.DoesNotRunOnMainThread;
import com.android.util.exif.ExifInterface;

public class ImageUtils {
	private static final String TAG = "ImageUtils";
	private static final int MAX_OOM_COUNT = 1;
	private static final byte[] GIF87_HEADER = "GIF87a".getBytes(Charset
			.forName("US-ASCII"));
	private static final byte[] GIF89_HEADER = "GIF89a".getBytes(Charset
			.forName("US-ASCII"));
	...
	public static byte[] bitmapToBytes(final Bitmap bitmap, final int quality)
        ...
        while (!done) {
            try {
                ...
                Log.w(TAG, "OutOfMemory converting bitmap to bytes.");
                throw e;
            }
        }
        return imageBytes;
	 ...
	 * 
	 * @param quality
	 *            Value between 0 and 100 that the compressor uses to discern
	 *            what quality the resulting bytes should be
	 * @param bitmap
	 *            Bitmap to convert into bytes
	 ...
	 * @param source
	 *            The source bound of the bitmap.
	 * @param dest
	 *            The destination bound on the canvas.
	 * @param bitmapPaint
	 *            Optional Paint object for the bitmap
	 * @param fillBackground
	 *            when set, fill the circle with backgroundColor
	 * @param strokeColor
	 *            draw a border outside the circle with strokeColor
	 */


	/**
	 * Sets a drawable to the background of a view. setBackgroundDrawable() is
	 * deprecated since JB and replaced by setBackground().
	 */
	 ...
	 * 
	 * @param options
	 *            a BitmapFactory.Options instance containing the bounds info of
	 *            the bitmap
	 * @param reqWidth
	 *            the desired width of the bitmap. Can be
	 *            ImageRequest.UNSPECIFIED_SIZE.
	 * @param reqHeight
	 *            the desired height of the bitmap. Can be
	 *            ImageRequest.UNSPECIFIED_SIZE.
	 * @return
	 */

	private static final String[] MEDIA_CONTENT_PROJECTION = new String[] { MediaStore.MediaColumns.MIME_TYPE };
	...
	public static String getContentType(final ContentResolver cr, final Uri uri) {
		...
		if (UriUtil.isMediaStoreUri(uri)) {
			try {
				cursor = cr.query(uri, MEDIA_CONTENT_PROJECTION, null, null,
						null);
		...
		if (contentType == null) {
			...
			contentType = ContentType.getContentTypeFromExtension(
					uri.toString(), ContentType.IMAGE_UNSPECIFIED);
		}
		return contentType;
	}
	...
	public static int getOrientation(final Context context, final Uri uri) {
		try {
			return getOrientation(context.getContentResolver().openInputStream(
					uri));
		} catch (FileNotFoundException e) {
			Log.e(TAG, "getOrientation couldn't open: " + uri, e);
		}
	...
	/**
	 * @param inputStream
	 *            The stream to the image file. Closed on completion
	...
	public static boolean isGif(final Context context,String contentType, Uri contentUri) {
        ...
        if (ContentType.isImageType(contentType)) {
            try {
                ...
                Log.w(TAG, "Could not open GIF input stream", e);
            }
        }
	...
	/**
	 * Read an image and compress it to particular max dimensions and size. Used
	 * to ensure images can fit in an MMS. TODO: This uses memory very
	 * inefficiently as it processes the whole image as a unit (rather than
	 * slice by slice) but system JPEG functions do not support slicing and
	 * dicing.
	 */
	public static class ImageResizer {
		...
		// Initial size of the image (typically provided but can be
		// UNSPECIFIED_SIZE)
		private int mWidth;
		private int mHeight;
		 ...
		 * @param orientation
		 *            The orientation of the image as an ExifInterface constant
		 * @param widthLimit
		 ...
		 */
		private ImageResizer(final int width, final int height,
				final int orientation, final int widthLimit,
				final int heightLimit, final int byteLimit, final Uri uri,
				final Context context, final String contentType) {
			mWidth = width;
			mHeight = height;
			mOrientationParams = ExifInterface
		...
		public static byte[] getResizedImageData(final int width, final int height,
            ...
            return resizer.resize(context);
        }
		
		private byte[] resize(final Context context) {
            return ImageUtils.isGif(context ,mContentType, mUri) ? resizeGifImage() : resizeStaticImage();
        }
		...
		private boolean ensureImageSizeSet() {
            if (mWidth == -1 ||
                ...
                try {
                    ...
                    Log.e(TAG, "Could not open file corresponding to uri " + mUri, e);
                } catch (final NullPointerException e) {
                    Log.e(TAG, "NPE trying to open the uri " + mUri, e);
                } finally {
                    if (inputStream != null) {
                        try {
                            inputStream.close();
		...
		private boolean canBeCompressed() {
            //final boolean logv = Log.isLoggable(TAG, VERBOSE);
		...
		private byte[] recodeImage(final int attempt) throws FileNotFoundException {
            ...
            try {
                final ContentResolver cr = mContext.getContentResolver();
                final boolean logv = Log.isLoggable(TAG, Log.VERBOSE);
                if (logv) {
                    Log.v(TAG, "getResizedImageData: attempt=" + attempt
                ...
                if (mScaled == null) {
                    ...
                    if (logv) {
                        Log.v(TAG, "getResizedImageData: decoded w,h="
                    ...
                    if (mScaleFactor > 1.0 || mOrientationParams.rotation != 0) {
                        ...
                        if (logv) {
                            Log.v(TAG, "getResizedImageData: scaled w,h="
		...
		private byte[] resizeStaticImage() {
            ...
            try {
                ...
                Log.e(TAG, "File disappeared during resizing");
            } finally {
                // Release all bitmaps
                if (mScaled != null && mScaled != mDecoded) {
                    mScaled.recycle();
	...
	/**
	 * Scales and center-crops a bitmap to the size passed in and returns the
	 * new bitmap.
	 * 
	 * @param source
	 *            Bitmap to scale and center-crop
	 * @param newWidth
	 *            destination width
	 * @param newHeight
	 *            destination height
	 ...
	 */
	public static Bitmap scaleCenterCrop(final Bitmap source,
		...
		// will now
		// be
		final RectF targetRect = new RectF(left, top, left + scaledWidth, top
				+ scaledHeight);

		// Finally, we create a new bitmap of the specified size and draw our
		// new,
		// scaled bitmap onto it.
		final Bitmap dest = Bitmap.createBitmap(newWidth, newHeight,
				source.getConfig());
	 ...
	 * top (nine-patch size and color tint).
	 * 
	 * TODO: we are creating new drawable instances here, but there are
	 * optimizations that can be made. For example, message bubbles shouldn't
	 * need the mutate() call and the play/pause buttons shouldn't need to
	 * create new drawable from the constant state.
	 */
	public static Drawable getTintedDrawable(final Context context,
			final Drawable drawable, final int color) {
		// For some reason occassionally drawables on JB has a null constant
		// state
		final Drawable.ConstantState constantStateDrawable = drawable
				.getConstantState();
	 ...
	 */
	public static Rect decodeImageBounds(final Context context,
			final Uri imageUri) {
		...
		try {
			...
			Log.e(TAG, "Couldn't open input stream for uri = " + imageUri);
		}
