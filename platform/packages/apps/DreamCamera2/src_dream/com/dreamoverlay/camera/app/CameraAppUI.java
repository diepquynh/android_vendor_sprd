...
import com.dream.camera.modules.filterdream.DreamFilterModuleController;
import com.dream.camera.modules.intervalphoto.IntervalPhotoUI;
...
import com.dream.camera.ui.PhotoVoicePlayProgress;
import com.dream.camera.ui.DreamVoiceImageView;
...
public class CameraAppUI implements ModeListView.ModeSwitchListener,
    ...
    public static interface BottomPanel {
        ...
        /**
         * Sets a new or replaces an existing listener for bottom control
         * events.
         */
         ...
         * 
         * @param state
         *            one of {@link #VIEWER_NONE}, {@link #VIEWER_PHOTO_SPHERE},
         *            {@link #VIEWER_REFOCUS}.
         */
        ...
        /**
         * Classes implementing this interface can listen for events on the
         * bottom controls.
         */
        public static interface Listener {
            /**
             * Called when the user pressed the "view" button to e.g. view a
             * photo sphere or RGBZ image.
             */
    ...
    private TextView mCurrentModuleTip;
    ...
    private final CameraModuleScreenShotProvider mCameraModuleScreenShotProvider
        ...
        public Bitmap getScreenShot(int previewDownSampleFactor) {
            ...
            if (!mStartSwitchAnimation) {
                ...
                if (CameraUtil.isFreezeBlurEnable()) {
                    preview = CameraUtil.blurBitmap(CameraUtil.computeScale(mTextureViewHelper.getPreviewBitmap(previewDownSampleFactor), 0.2f), mController.getAndroidContext());
                } else {
                ...
                if (preview != null) {
                    canvas.drawBitmap(preview, null,
                            mTextureViewHelper.getPreviewArea(), null);
                }
        ...
        public Bitmap getScreenShot(Bitmap bmp) {
            ...
            if (!mStartSwitchAnimation) {
                canvas.drawARGB(255, 0, 0, 0);
                if (bmp != null) {
                    canvas.drawBitmap(bmp, null, mTextureViewHelper.getPreviewArea(), null);
                }
                Bitmap overlay = getPreviewOverlayAndControls();
                if (overlay != null) {
                    canvas.drawBitmap(overlay, 0f, 0f, null);
                ...
                if (Math.abs(deltaX) > mSlop || Math.abs(deltaY) > mSlop) {
                    ...
                    if (deltaX >= Math.abs(deltaY)) {
                        ...
                        if (mController != null
                            ...
                            if (mZoomPanel != null && mZoomPanel.getVisibility() == View.VISIBLE) {
                                return false;
                            }
                            setSwipeState(SWIPE_LEFT);
                        }
    ...
    public CameraAppUI(AppController controller,
        ...
        // SPRD: Fix 533869 Add for voice picture of new ui
        mVoicePreview = (ImageView) appRootView.findViewById(R.id.voice_preview);
    ...
    /**
     * Freeze what is currently shown on screen until the next preview frame
     * comes in.
     */
    ...
    public void freezeScreenUntilPreviewReady(Bitmap bitmap) {
        Log.v(TAG, "freezeScreenUntilPreviewReady bitmap="+bitmap);
        /* SPRD: Fix bug 574388 the preview screen will be black in filter to switch @{ */
        if (bitmap == null) {
    ...
    public void freezeScreenUntilPreviewReady(Bitmap bitmap, RectF previewArea) {
        ...
        if (bitmap == null || previewArea == null) {
            freezeScreenUntilPreviewReady();
        } else {
            mModeTransitionView.setupModeCover(mCameraModuleScreenShotProvider
                    .getScreenShot(bitmap, previewArea));
            mHideCoverRunnable = new Runnable() {
                @Override
                public void run() {
                    mModeTransitionView.hideImageCover();
     ...
     * 
     * @param viewerType
     *            defines which viewer the cling is for.
     */
    public void setupClingForViewer(int viewerType) {
        if (viewerType == BottomPanel.VIEWER_REFOCUS) {
            ...
            if (filmstripContent != null) {
                ...
                Cling refocusCling = (Cling) inflater.inflate(
                        R.layout.cling_widget, null, false);
                // Sets instruction text in the cling.
                refocusCling.setText(mController
                        .getAndroidContext()
                        .getResources()
                        .getString(
                                R.string.cling_text_for_refocus_editor_button));

                // Adds cling into view hierarchy.
                int clingWidth = mController.getAndroidContext().getResources()
                        .getDimensionPixelSize(R.dimen.default_cling_width);
                filmstripContent.addView(refocusCling, clingWidth,
                        ViewGroup.LayoutParams.WRAP_CONTENT);
                mFilmstripBottomControls.setClingForViewer(viewerType,
                        refocusCling);
            }
        }
    }
    ...
    public void clearClingForViewer(int viewerType) {
        Cling clingToBeRemoved = mFilmstripBottomControls
                .getClingForViewer(viewerType);
    ...
    private void onSwipeDetected(int swipeState) {
        ...
        if (swipeState == SWIPE_UP || swipeState == SWIPE_DOWN) {
            ...
            if (currentModuleIndex != moduleToTransitionTo) {
                ...
                int iconRes = CameraUtil.getCameraModeCoverIconResId(
                        moduleToTransitionTo, mController.getAndroidContext());
...
//                    showShimmyDelayed();
    ...
    private void onPreviewListenerChanged() {
        if (mDreamUI != null && mDreamUI.getUITpye() == DreamUI.DREAM_GIF_UI) {
            // Disable autofocus in Gif Module.
        } else {
...
//        mController.getSettingsManager().addListener(mIndicatorIconController);
    ...
    /**
     * @param listener
     *            The listener for bottom controls.
     */
    ...
    private void applyModuleSpecs(final HardwareSpec hardwareSpec,
        ...
        //    mHdrSupportMode = settingsManager.getString(
        //            SettingsManager.SCOPE_GLOBAL,
        //            Keys.KEY_HDR_SUPPORT_MODE_BACK_CAMERA);
        ...
        //            if (mHdrSupportMode
        //                    .equals(getResourceString(R.string.pref_camera_hdr_supportmode_hdr_plus))) {
        ...
        //    boolean flashBackCamera = settingsManager.getBoolean(
        //            SettingsManager.SCOPE_GLOBAL,
        //            Keys.KEY_FLASH_SUPPORTED_BACK_CAMERA);
        ...
        //                buttonManager.initializeButton(
        //                        ButtonManager.BUTTON_TORCH,
        //                        bottomBarSpec.flashCallback);
        //            } else if (bottomBarSpec.enableHdrPlusFlash) {
        //                buttonManager.initializeButton(
        //                        ButtonManager.BUTTON_HDR_PLUS_FLASH,
        //                        bottomBarSpec.flashCallback);
        //            } else {
        ...
        //        buttonManager
        //                .initializeButton(
        //                        ButtonManager.BUTTON_GRID_LINES,
        //                        bottomBarSpec.gridLinesCallback != null ? bottomBarSpec.gridLinesCallback
        //                                : getGridLinesCallback());
        //    } else {
        ...
        //    buttonManager
        //            .initializePanoOrientationButtons(bottomBarSpec.panoOrientationCallback);
        //}
        ...
        // enabled), but the device/module has the feature, then disable the
        // exposure
        // button.
        ...
        //        && mController.getSettingsManager().getBoolean(
        //                SettingsManager.SCOPE_GLOBAL,
        ...
        //                            .getCurrentCameraId().getLegacyValue())
        //            .isFacingBack()
        //            && Keys.isHdrOn(mController.getSettingsManager())) {
        //        buttonManager
        //                .disableButton(ButtonManager.BUTTON_EXPOSURE_COMPENSATION);
        //    }
        //} else if (mController.getSettingsManager().getBoolean(
        //        SettingsManager.SCOPE_GLOBAL,
        //        Keys.KEY_EXPOSURE_COMPENSATION_ENABLED)
...
//        mController.getSettingsManager().addListener(mIndicatorIconController);
    ...
    public void hideSettingUI() {
        if (mSettingLayout.getVisibility() == View.VISIBLE) {
            mSettingLayout.changeVisibilty(View.GONE);
        }
        if (mDreamUI != null) {
    ...
    public void initSidePanel() {
        if (mCameraRootView != null) {
            ...
            spIS = mCameraRootView.findViewById(R.id.sb_internal_storage_status);
            updateSidePanelItem(DreamUtil.SP_INTERNAL_STORAGE, true);

            spUSB = mCameraRootView.findViewById(R.id.sb_usb_storage_status);
            updateSidePanelItem(DreamUtil.SP_USB_STORAGE, true);

            spFD = mCameraRootView.findViewById(R.id.sb_face_detect_status);
            updateSidePanelItem(DreamUtil.SP_FACE_DETECT, true);

            spL = mCameraRootView.findViewById(R.id.sb_locate_status);
            updateSidePanelItem(DreamUtil.SP_LOCATE, true);

            spTC = mCameraRootView.findViewById(R.id.sb_touch_capture_status);
            updateSidePanelItem(DreamUtil.SP_TOUCH_CAPTURE, true);

            spF = mCameraRootView.findViewById(R.id.sb_filter_status);
            updateSidePanelItem(DreamUtil.SP_FILTER, true);

            spOP = mCameraRootView.findViewById(R.id.sb_opposite_status);
            updateSidePanelItem(DreamUtil.SP_OPPOSITE, true);

            spCA = mCameraRootView.findViewById(R.id.sb_ca_status);
            updateSidePanelItem(DreamUtil.SP_CA, true);

            spVA = mCameraRootView.findViewById(R.id.sb_va_status);
            updateSidePanelItem(DreamUtil.SP_VA, true);
        }
    }

    public void updateSidePanelItem(int icon, boolean show) {
        int visibility;
        if (show && mDreamInterface != null && (icon & mDreamInterface.getSidePanelMask()) != 0) {
            visibility = View.VISIBLE;
        } else {
            visibility = View.GONE;
        }
        DataModuleBasic mDataModuleCurrent = DataModuleManager.getInstance(
                mController.getAndroidContext()).getCurrentDataModule();
        DataModuleBasic mDataModuleCamera = DataModuleManager.getInstance(
                mController.getAndroidContext()).getDataModuleCamera();
        switch (icon) {
        ...
        case DreamUtil.SP_FACE_DETECT:
            ...
            if (mDataModuleCurrent == null
                    ...
                    .getString(Keys.KEY_CAMERA_AI_DATECT) == null || "off".equals(mDataModuleCurrent
        ...
        case DreamUtil.SP_LOCATE:
            ...
            Log.e(TAG, "spL = " + spL + " visibility = " + visibility);
            if (spL != null)
        ...
        case DreamUtil.SP_TOUCH_CAPTURE:
            ...
            if (mDataModuleCurrent == null
                    || !mDataModuleCurrent
                    .isEnableSettingConfig(Keys.KEY_CAMERA_TOUCHING_PHOTOGRAPH) || mDataModuleCurrent
                    .getString(Keys.KEY_CAMERA_TOUCHING_PHOTOGRAPH) == null || "0".equals(mDataModuleCurrent
        ...
        case DreamUtil.SP_OPPOSITE:
            ...
            break;
        case DreamUtil.SP_CA:
            if (mDataModuleCurrent == null) {
                visibility = View.GONE;
            }else{
                if (mDataModuleCurrent.getDataSetting().mIsFront) {
                    if (!CameraUtil.isEOISDcFrontEnabled()
                            || !mDataModuleCurrent
                            .isEnableSettingConfig(Keys.KEY_EOIS_DC_FRONT) || "0".equals(mDataModuleCurrent
                                    .getString(Keys.KEY_EOIS_DC_FRONT))) {
                        visibility = View.GONE;
                    }
                } else {
                    if (!CameraUtil.isEOISDcBackEnabled()
                            ||  !mDataModuleCurrent
                            .isEnableSettingConfig(Keys.KEY_EOIS_DC_BACK) || "0".equals(mDataModuleCurrent
                                    .getString(Keys.KEY_EOIS_DC_BACK))) {
                        visibility = View.GONE;
                    }
                }
            }
            ...
            if (mDataModuleCurrent == null) {
                ...
                if (mDataModuleCurrent.getDataSetting().mIsFront) {
                    if (!CameraUtil.isEOISDvFrontEnabled()
                            || !mDataModuleCurrent
                            .isEnableSettingConfig(Keys.KEY_EOIS_DV_FRONT) || "0".equals(mDataModuleCurrent
                                    .getString(Keys.KEY_EOIS_DV_FRONT))) {
                        visibility = View.GONE;
                    }
                } else {
                    if (!CameraUtil.isEOISDvBackEnabled()
                            || !mDataModuleCurrent
                            .isEnableSettingConfig(Keys.KEY_EOIS_DV_BACK) || "0".equals(mDataModuleCurrent
                                    .getString(Keys.KEY_EOIS_DV_BACK))) {
                        visibility = View.GONE;
                    }
                }
            }
...
//        }
    ...
    public void onCloseFilmStrip() {
        ...
        if (sidePanel != null && !mIsCaptureIntent)
    ...
    private ImageView gifCancelBtn;
    private ImageView gifFinishBtn;
    /* @} */
    ...
    public void initBottomButtons() {
        ...
        /* SPRD: ui check 49 gifModule ui @{ */
        gifCancelBtn = (ImageView) mCameraRootView
    ...
    /* SPRD: ui check 49 gifModule ui @{ */
    public void updateGifCancelAndFinish(int visible) {
        gifCancelBtn.setVisibility(visible);
        gifFinishBtn.setVisibility(visible);
    }
    /* @} */
    /**
...
//        mCurrentModuleTip.setText(mController.getModuleManager()
//                .getModuleAgent(moduleId).getScopeNamespace());
        mCurrentModuleTip.setText(CameraUtil.getCameraModeText(moduleId, mController.getAndroidContext()));
        mCurrentModuleTip.setVisibility(View.VISIBLE);

        mHideCurrentModuleTipRunnable = new Runnable() {
            @Override
            public void run() {
                hideCurrentModuleTip();
            }
        };
        mAppRootView.postDelayed(mHideCurrentModuleTipRunnable, 2000);
    }

    private void hideCurrentModuleTip() {
        if (mCurrentModuleTip != null) {
            mCurrentModuleTip.setVisibility(View.GONE);
        }
        mHideCurrentModuleTipRunnable = null;
    }
    ...
    public void updatePreviewUI(int visible) {
        if(isInIntentReview())
            return;

        updateTopPanelUI(visible);
        updateSidePanelUI(visible);
    ...
    public void updateSidePanelUI(int visible) {

        if((mBursting
                || (((CameraActivity) mController).getCurrentModule() != null
                && ((CameraActivity) mController).getCurrentModule().isAudioRecording()))
...
//    }
    ...
    public Bitmap getScreenShot() {
        return mCameraModuleScreenShotProvider.getScreenShot(DOWN_SAMPLE_RATE_FOR_SCREENSHOT);
    }
    ...
    /* SPRD: Add new Feature of Audio Picture @{ */
    public Bitmap getPreviewShot() {
    ...
    public void startCircle() {
        ((PhotoVoicePlayProgress)mFilmstripLayout.findViewById(R.id.photo_voice_play_progress)).playVoice();
        mFilmstripBottomControls.setVisible(false);
    }
    ...
    public void setCircleDuration(long duration) {
       ((PhotoVoicePlayProgress)mFilmstripLayout.findViewById(R.id.photo_voice_play_progress)).setDuration(duration);
    }
    ...
    public ImageView getVoicePreview() {
        return mVoicePreview;
    }
    /* @} */
    ...
    public boolean isGifCancelVisible() {
        return (gifCancelBtn != null && gifCancelBtn.getVisibility() == View.VISIBLE);
    }
