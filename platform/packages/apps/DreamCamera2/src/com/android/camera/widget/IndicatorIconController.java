...
public class IndicatorIconController
    ...
    private ImageView mVgestureIndicator;
    ...
    private TypedArray mVgestureIndicatorIcons;
    ...
    /* SPRD: New feature vgesture detect @{ */
    /**
     * Sync the icon and the visibility of the vgesture indicator.
     */
    private void syncVgestureIndicator() {
        if (!UCamUtill.isVgestureEnnable()) {
            return;
        }
        ButtonManager buttonManager = mController.getButtonManager();
        // If hdr isn't an enabled and visible option,
        // do not show the indicator.
        if (buttonManager.isEnabled(ButtonManager.BUTTON_VGESTURE)
                && buttonManager.isVisible(ButtonManager.BUTTON_VGESTURE)) {
            /* SPRD: Fix bug 545097, Add V gestures and facial mutex @{ */
            if (mController.getSettingsManager().getBoolean(SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_BEAUTY_ENTERED)) {
                buttonManager.disableButton(ButtonManager.BUTTON_VGESTURE);
            }
            /* @} */
            setIndicatorState(SettingsManager.SCOPE_GLOBAL,
                              Keys.KEY_CAMERA_VGESTURE, mVgestureIndicator,
                              mVgestureIndicatorIcons, false);
        } else {
            changeVisibility(mVgestureIndicator, View.GONE);
        }
    }
    /* @} */
