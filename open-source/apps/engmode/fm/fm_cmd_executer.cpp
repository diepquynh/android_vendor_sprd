...
static int fm_eng_send_back_cmd_result(int client_fd, char *str, int isOK)
{
    ...
    FMENG_LOGD("%s, client fd:%d, cmd_result: %s, ret: %d", __FUNCTION__, client_fd, buffer, ret);
    if(ret < 0)
    {
        FMENG_LOGD("write %s to client_fd:%d fail (error(%d):%s)", buffer, client_fd, errno, strerror(errno));
...
static FM_ENG_ERROR_E fm_eng_on(void)
{
    ...
    err = (FM_ENG_ERROR_E)FMR_open_dev(fm_eng_idx); // if success, then ret = 0; else ret < 0
    if (err)
    {
...
int fm_runcommand(int client_fd, int argc, char **argv)
{
    ...
    if (strncmp(*argv, "fm_on", 5) == 0)
    {
        ...
        if(err == FM_ENG_NONE_ERROR)
        {
            ...
            sprintf(result_buf, "fm_on success, return value: %d\nbsp_FM_test_function#open UTPASS", err);
        }
        else
        {
            sprintf(result_buf, "fm_on failed, return value: %d\nbsp_FM_test_function#open UTFAILED", err);
        }
    }
    else if (strncmp(*argv, "fm_off", 6) == 0)
    {
        ...
        if(err == FM_ENG_NONE_ERROR)
        {
            ...
            sprintf(result_buf, "fm_off success, return value: %d\nbsp_FM_test_function#close UTPASS", err);
        }
        else
        {
            sprintf(result_buf, "fm_off failed, return value: %d\nbsp_FM_test_function#close UTFAILED", err);
        }
    }
    else if (strncmp(*argv, "tune", 4) == 0 && argc > 1)
    {
        ...
        if(err == FM_ENG_NONE_ERROR)
        {
            ...
            sprintf(result_buf, "fm_tune success, return value: %d\nbsp_FM_test_function#tune UTPASS", err);
        }
        else
        {
            sprintf(result_buf, "fm_tune failed, return value: %d\nbsp_FM_test_function#tune UTFAILED", err);
        }
    }
    else if (strncmp(*argv, "getrssi", 7) == 0)
    {
        ...
        if(err == FM_ENG_NONE_ERROR)
        {
            ...
            sprintf(result_buf, "fm_getrssi success, rssi = %d, return value: %d\nbsp_FM_test_function#get_rssi UTPASS", fm_eng_rssi, err);
        }
        else
        {
            sprintf(result_buf, "fm_getrssi failed, return value: %d\nbsp_FM_test_function#get_rssi UTFAILED", err);
        }
    }
    else
    {
