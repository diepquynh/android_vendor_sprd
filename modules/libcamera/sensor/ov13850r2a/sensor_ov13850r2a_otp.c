...
#define OV13850R2A_OTP_CASE 		3
#define OV13850R2A_UPDATE_LNC		1
#define OV13850R2A_UPDATE_WB		2
#define OV13850R2A_UPDATE_LNC_WB	3
#define OV13850R2A_UPDATE_VCM		4
...
// otp_ptr: pouint32_ter of otp_info_t
// return: 0,
LOCAL uint32_t ov13850r2a_read_otp_info(struct otp_info_t *otp_ptr)
{
	//uint32_t i = 0;
	...
	// set 0x5002[1] to 1
	temp1 = Sensor_ReadReg(0x5002);
	Sensor_WriteReg(0x5002, (0x02 & 0x02) | (temp1 & (~0x02)));
	return 0;
}
 ...
 * return value. 0
 **/
uint8_t read_otp_VCM_R2A(uint32_t index, struct otp_info_t * otp_ptr)
{
	uint32_t otp_flag, addr,temp;
	uint8_t ret =0;
		//set 0x5002[1] to "0"
 ...
 **/
uint8_t read_otp_lenc_R2A(struct otp_info_t *otp_ptr)
{
 ...
 * return value 0 success 1 fail
 **/
uint8_t apply_otp_lenc_R2A(struct otp_info_t *otp_ptr)
{
	uint32_t i;
  ...
  uint32_t temp1;
	...
	temp1 = Sensor_ReadReg(0x5000);
	Sensor_WriteReg(0x5000, (temp1 | 0x01));
 ...
 * return value 0 success  1 fail
 **/
static uint8_t read_otp_wb_R2A(struct otp_info_t *otp_ptr)
{
	uint32_t otp_flag, addr, temp;
	uint8_t ret =0;
	//set 0x5002[1] to "0"
 ...
 * return value 0 success  1 fail
 **/
static uint8_t apply_otp_wb_R2A(struct otp_info_t *otp_ptr)
{
	uint8_t ret =0;
...
{
	struct otp_info_t current_otp;
	uint32_t rtn=1;
        memset(&current_otp, 0, sizeof(struct otp_info_t));
...
LOCAL uint32_t ov13850r2a_update_otp(void* param_ptr)
{
	uint16_t stream_value = 0;
	uint32_t rtn = SENSOR_FAIL;

	stream_value = Sensor_ReadReg(0x0100);
	SENSOR_PRINT("stream_value = 0x%x  OV13850R2A_OTP_CASE = %d\n", stream_value, OV13850R2A_OTP_CASE);
	...
	switch(OV13850R2A_OTP_CASE){
		case OV13850R2A_UPDATE_LNC:
			rtn = ov13850r2a_update_otp_lenc();
			break;
		case OV13850R2A_UPDATE_WB:
			rtn = ov13850r2a_update_otp_wb();
			break;

		case OV13850R2A_UPDATE_LNC_WB:
			rtn = ov13850r2a_update_otp_lenc();
			rtn = ov13850r2a_update_otp_wb();
			break;

		case OV13850R2A_UPDATE_VCM:
...
#if 1
LOCAL uint32_t ov13850r2a_Global_Identify_otp(void* param_ptr)
{
	struct otp_info_t current_otp;
	uint32_t temp = 0;
	uint16_t stream_value = 0;
	...
	if(OV13850R2A_RAW_PARAM_GLOBAL == current_otp.module_id){
		rtn=SENSOR_SUCCESS;
