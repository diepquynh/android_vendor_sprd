...
public class SearchableCorpora implements Corpora {
    ...
    public List<Corpus> getCorporaInAll() {
        ...
        /* SPRD：Modify 20141016 Spreadst of Bug 361200 Catch the exception, avoid ConcurrentModificationException  @{*/
        try {
            for (ListIterator<Corpus> iter = mEnabledCorpora.listIterator(); iter.hasNext();) {
                Corpus corpus = (Corpus) iter.next();
                if (corpus.includeInAll()) {
                    corpora.add(corpus);
                }
            }
        } catch (ConcurrentModificationException e) {
            Log.w(TAG, "ConcurrentModificationException ", e);
        }
        /* @} */
        return corpora;
    }
    ...
    public void update() {
        ...
        for (Corpus corpus : corpora) {
            ...
            if (mSettings.isCorpusEnabled(corpus)) {
                if (String.valueOf(corpus).equals("apps") && Util.DEBUG) {
                    Log.d(TAG, "MonkeyTest app not add mEnabledCorpora");
                } else {
