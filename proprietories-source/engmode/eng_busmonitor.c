...
int eng_diag_busmonitor_get_chaninfo(char *buf, int len, char *rsp,
                                     int rsplen) {
  char *rsp_ptr;
  int i;
  ...
  rsplen = 2 * sizeof(unsigned short) + cnt * sizeof(BUSMONITOR_CHAN_NAME) +
           sizeof(MSG_HEAD_T);
  rsp_ptr = (char *)malloc(rsplen);
  if (NULL == rsp_ptr) {
    ENG_LOG("%s: Buffer malloc failed\n", __FUNCTION__);
  ...
  *((unsigned short *)(rsp_ptr + sizeof(MSG_HEAD_T))) = cnt;
  ...
  for (i = 0; i < cnt; i++) {
    memcpy((rsp_ptr + sizeof(MSG_HEAD_T)) + 2 * sizeof(unsigned short) +
               i * sizeof(BUSMONITOR_CHAN_NAME),
           (char *)sprd_bm_get_chn_name(i), sizeof(BUSMONITOR_CHAN_NAME));
  }
...
int eng_diag_busmonitor_get_monitordata(char *buf, int len, char *rsp,
                                        int rsplen) {
  char *rsp_ptr;
  int i;
  unsigned short cnt, ret = 0;
  ...
  rsplen = 2 * sizeof(unsigned short) + cnt * sizeof(BUSMONITOR_CHANNEL_DATA) +
           sizeof(MSG_HEAD_T);
  rsp_ptr = (char *)malloc(rsplen);
  if (NULL == rsp_ptr) {
    ENG_LOG("%s: Buffer malloc failed\n", __FUNCTION__);
  ...
  *((unsigned short *)(rsp_ptr + sizeof(MSG_HEAD_T))) = cnt;
  ...
  for (i = 0; i < cnt; i++) {
    data = (BUSMONITOR_CHANNEL_DATA *)(rsp_ptr + 2 * sizeof(unsigned short) +
                                       sizeof(MSG_HEAD_T) +
                                       i * sizeof(BUSMONITOR_CHANNEL_DATA));
