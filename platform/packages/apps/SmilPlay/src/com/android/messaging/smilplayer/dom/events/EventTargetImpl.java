...
package com.android.messaging.smilplayer.dom.events;
    ...
    public boolean dispatchEvent(Event evt) throws EventException {
        ...
        if (!eventImpl.isPropogationStopped() && (mListenerEntries != null)) {
            for (int i = 0; i < mListenerEntries.size(); i++) {
                /* SPRD: add for bug339892 of IndexOutOfBoundsException @{ */
                EventListenerEntry listenerEntry = null;
                try {
                    listenerEntry = mListenerEntries.get(i);
                } catch (IndexOutOfBoundsException e ) {
                    Log.w(TAG, "Catched IndexOutOfBoundsException exception", e);
                }
                /* @} */
