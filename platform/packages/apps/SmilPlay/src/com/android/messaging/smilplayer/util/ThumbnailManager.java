...
import com.android.messaging.smilplayer.util.ImageCacheService.ImageData;
...
public class ThumbnailManager extends BackgroundLoaderManager {
    ...
    // SPRD: Modify for bug#271556 @{
    private Bitmap mVcardBitmap;
    private Bitmap mVcalendarBitmap;
    private Bitmap mOtherFileBitmap;
    // @}
    /* SPRD: Bug 375488 @{ */
    private static Bitmap mEmptyImageBitmapForDrm;
    /* }@ */
    ...
    public ThumbnailManager(final Context context) {
        super(context);
        // SPRD:Bug#271907,Occur OutOfMemoryError
        // mThumbnailCache = new SimpleCache<Uri, Bitmap>(8, 16, 0.75f, true);
        mThumbnailCache = new SimpleCache<Uri, Bitmap>(8, 16, 0.75f, false);
        mContext = context;
        ...
        Thread decodeThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				mVcardBitmap = BitmapFactory.decodeResource(context.getResources(),
						R.drawable.ic_vcard_layout);
			    mVcalendarBitmap = BitmapFactory.decodeResource(context.getResources(),
						R.drawable.ic_calender);
				mOtherFileBitmap = BitmapFactory.decodeResource(context.getResources(),
						R.drawable.otherfiles);
                /* SPRD:Add for drm @{ */
                if (DrmUtilsSprd.getInstance(context).isDrmEnabled()) {
                    mEmptyImageBitmapForDrm = BitmapFactory.decodeResource(
        ...
        decodeThread.setPriority(Thread.MIN_PRIORITY);
        decodeThread.start();
    }

    public void ensureEmptyImageBitmap() {
        if (mEmptyImageBitmap == null || mEmptyImageBitmap.isRecycled()) {
            mEmptyImageBitmap = BitmapFactory.decodeResource(mContext.getResources(),
                    R.drawable.ic_missing_thumbnail_picture);
        }
    }

    public void ensureEmptyVideoBitmap() {
        if (mEmptyVideoBitmap == null || mEmptyVideoBitmap.isRecycled()) {
            mEmptyVideoBitmap = BitmapFactory.decodeResource(mContext.getResources(),
                    R.drawable.ic_missing_thumbnail_video);
        }
    }

    public  Bitmap getVcardBitmap() {
        if (mVcardBitmap == null || mVcardBitmap.isRecycled()) {
            mVcardBitmap = BitmapFactory.decodeResource(mContext.getResources(),
                    R.drawable.ic_vcard_layout);
        }
        return mVcardBitmap;
    }

    public  Bitmap getVcalendarBitmap() {
        if (mVcalendarBitmap == null || mVcalendarBitmap.isRecycled()) {
            mVcalendarBitmap = BitmapFactory.decodeResource(mContext.getResources(),
                    R.drawable.ic_calender);
        }
        return mVcalendarBitmap;
    }

    public  Bitmap getOtherFilerBitmap() {
        if (mOtherFileBitmap == null || mOtherFileBitmap.isRecycled()) {
            mOtherFileBitmap = BitmapFactory.decodeResource(mContext.getResources(),
                    R.drawable.otherfiles);
        }
        return mOtherFileBitmap;
    }
    ...
    private ItemLoadedFuture getThumbnail(Uri uri, boolean isVideo,
        ...
        if (thumbnailExists) {
            if (callbackRequired && !DEBUG_DISABLE_CALLBACK) {
                ...
                ImageLoaded imageLoaded = new ImageLoaded(thumbnail, isVideo, uri);
    ...
    public class ThumbnailTask implements Runnable {
        ...
        public void run() {
            ...
            /* SPRD:Ensure mEmptyImageBitmap and mEmptyVideoBitmap  @{ */
            ensureEmptyVideoBitmap();
            ensureEmptyImageBitmap();
            /* @}*/
            /* SPRD:Bug 375488 Add for drm @{ */
            if (DrmUtilsSprd.getInstance(mContext).isDrmType(mUri, null)) {
                if (mEmptyImageBitmapForDrm == null
                        || mEmptyImageBitmapForDrm.isRecycled()) {
                    mEmptyImageBitmapForDrm = BitmapFactory.decodeResource(
            ...
            mCallbackHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (callbacks != null) {
                         ...
                         * @orig
                         *
                        Bitmap bitmap = resultBitmap == null ? (mIsVideo ? mEmptyVideoBitmap
                                : mEmptyImageBitmap)
                                : resultBitmap;
                        /* @{ */
                        Bitmap bitmap = null;
                        if (DrmUtilsSprd.getInstance(mContext).isDrmType(mUri,
                                null)) {
                            bitmap = resultBitmap == null ? (mIsVideo ? mEmptyVideoBitmap
                                    : mEmptyImageBitmapForDrm)
                                    : resultBitmap;
                        ...
                        for (final ItemLoadedCallback<ImageLoaded> callback : asList(callbacks)) {
                            ...
                            if (!DEBUG_DISABLE_CALLBACK) {
                                /* SPRD:Bug#276910  @{
                                 * org: ImageLoaded imageLoaded = new ImageLoaded(bitmap, mIsVideo);
                                */
                                ImageLoaded imageLoaded = new ImageLoaded(bitmap, mIsVideo, mUri);
                                /* @*/
                                callback.onItemLoaded(imageLoaded, null);
                            }
                        }
        ...
        private Bitmap requestDecode(final Uri uri, Options options, int targetSize) {
            ...
            try {
                ...
                Log.w(TAG, "Failed to reading because SecurityException uri is "+ uri);
                return null;
            }
    ...
    public static class ImageLoaded {
        ...
        public ImageLoaded(Bitmap bitmap, boolean isVideo, Uri thumbnail) {
            mBitmap = bitmap;
            mIsVideo = isVideo;
            // SPRD:Bug#276910
    ...
    private boolean isDrmTypeFile(String name) {
        String extension = "";
        if (name != null) {
    ...
    public void ensureEmptyImageBitmapForDrm() {
        if (mEmptyImageBitmapForDrm == null
                || mEmptyImageBitmapForDrm.isRecycled()) {
            mEmptyImageBitmapForDrm = BitmapFactory.decodeResource(
                    mContext.getResources(),
    ...
    public ItemLoadedFuture getThumbnail(Uri uri,
            final ItemLoadedCallback<ImageLoaded> callback, String name) {
        return getThumbnail(uri, false, callback, name);
    }
    ...
    private ItemLoadedFuture getThumbnail(Uri uri, boolean isVideo,
        ...
        final Bitmap thumbnail = DEBUG_DISABLE_CACHE ? null : mThumbnailCache
                .get(uri);
        ...
        if (Log.isLoggable(LogTag.THUMBNAIL_CACHE, Log.DEBUG)) {
            Log.v(TAG, "getThumbnail mThumbnailCache.get for uri: " + uri
                    + " thumbnail: " + thumbnail + " callback: " + callback
                    + " thumbnailExists: " + thumbnailExists + " taskExists: "
                    + taskExists + " newTaskRequired: " + newTaskRequired
                    + " callbackRequired: " + callbackRequired);
        }
        ...
        if (thumbnailExists) {
            if (callbackRequired && !DEBUG_DISABLE_CALLBACK) {
                /*
                 * SPRD:Bug#276910 @{ org: ImageLoaded imageLoaded = new
                 * ImageLoaded(thumbnail, isVideo);
                 */
                ImageLoaded imageLoaded = new ImageLoaded(thumbnail, isVideo,
                        uri);
                /* @ */
        ...
        return new ItemLoadedFuture() {
            ...
            public void cancel(Uri uri) {
                cancelCallback(callback);
                removeThumbnail(uri); // if the thumbnail is half loaded, force
                                      // a reload next time
            }
