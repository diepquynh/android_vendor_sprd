...
static int eng_str2intcallback(void *param, int argc, char **argv,
                               char **cname) {
  int i;
...
static int eng_str2strcallback(void *param, int argc, char **argv,
                               char **cname) {
  int i;
...
static int eng_str2inttablecallback(void *param, int argc, char **argv,
                                    char **cname) {
  int i, finished = 0;
...
int eng_sqlite_create(void) {
  ...
  sprintf(sql_createtable,
          "CREATE TABLE %s(id INTEGER PRIMARY KEY, groupid INTEGER,name "
          "VARCHAR(32),value INTEGER);",
          ENG_STRING2INT_TABLE);
  ...
  memset(sql_createtable, 0, SPRDENG_SQL_LEN);
  sprintf(sql_createtable,
          "CREATE TABLE %s(id VARCHAR(32) PRIMARY KEY,value VARCHAR(32));",
          ENG_STRING2STRING_TABLE);
...
int eng_sql_string2int_set(char *name, int value) {
  ...
  sprintf(sqlbuf, "SELECT * FROM %s WHERE name='%s'", ENG_STRING2INT_TABLE,
          name);
  ...
  if (rownum > 0) {  // update item
    memset(sqlbuf, 0, SPRDENG_SQL_LEN);
    sprintf(sqlbuf, "UPDATE %s SET value=%d where name='%s';",
            ENG_STRING2INT_TABLE, value, name);
    ENG_LOG("%s: sqlbuf=%s\n", __FUNCTION__, sqlbuf);
    ...
    memset(sqlbuf, 0, SPRDENG_SQL_LEN);
    sprintf(sqlbuf, "INSERT INTO %s VALUES(1,0,'%s',%d);", ENG_STRING2INT_TABLE,
            name, value);
    ENG_LOG("%s: sqlbuf=%s\n", __FUNCTION__, sqlbuf);
...
int eng_sql_string2string_set(char *id, char *value) {
  ...
  // check if item exists
  memset(sqlbuf, 0, SPRDENG_SQL_LEN);
  sprintf(sqlbuf, "SELECT * FROM %s WHERE id='%s'", ENG_STRING2STRING_TABLE,
          id);
  rc = sqlite3_get_table(db, sqlbuf, &result, &rownum, &colnum, &errmsg);
  sqlite3_free_table(result);
  ...
  if (rownum > 0) {  // update item
    memset(sqlbuf, 0, SPRDENG_SQL_LEN);
    sprintf(sqlbuf, "UPDATE %s SET value='%s' where id='%s';",
            ENG_STRING2STRING_TABLE, value, id);
    ENG_LOG("%s: sqlbuf=%s\n", __FUNCTION__, sqlbuf);
    ...
    memset(sqlbuf, 0, SPRDENG_SQL_LEN);
    sprintf(sqlbuf, "INSERT INTO %s VALUES('%s','%s');",
            ENG_STRING2STRING_TABLE, id, value);
    ENG_LOG("%s: sqlbuf=%s\n", __FUNCTION__, sqlbuf);
...
int eng_sql_string2int_table_get(eng_str2int_table_sqlresult *result) {
  ...
  if (!str2inttablecallback) ret = ENG_SQLSTR2INT_ERR;
