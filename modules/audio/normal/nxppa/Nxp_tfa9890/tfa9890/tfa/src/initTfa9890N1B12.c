...
Tfa98xx_Error_t Tfa9890_specific(Tfa98xx_handle_t handle)
{
	...
	if (Tfa98xx_Error_Ok == error)		/* not for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x06, 0x000F);

	if (Tfa98xx_Error_Ok == error)		/* not for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x07, 0x8FFF);

	if (Tfa98xx_Error_Ok == error)		/* change for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x08, 0x7832);

	if (Tfa98xx_Error_Ok == error)
		error = Tfa98xx_WriteRegister16(handle, 0x09, 0x824D);

	if (Tfa98xx_Error_Ok == error)		/* not for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x0A, 0x38E5);

	if (Tfa98xx_Error_Ok == error)		/* not for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x0E, 0x0F01);


	if (Tfa98xx_Error_Ok == error)		/* not for n1c */
		error = Tfa98xx_WriteRegister16(handle, 0x46, 0x4000);
 ...
 *
 * Note: This only works after i2c reset which will clear the MTP contents.
...
errorExit:
	*ready = 0;
	assert(error);		/* an error here can be fatal */
	return error;
}
...
Tfa98xx_Error_t Tfa9890_DspReset(Tfa98xx_handle_t handle, int state)
{
	...
	/* for TFA9890 temporarily disable clock gating upon dsp reset */
	error = Tfa98xx_ReadRegister16(handle, TFA98XX_CURRENTSENSE4,
						&value49);
	if (error)
		return error;
