...
public class AttachmentsView extends LinearLayout {
    ...
    // handle vcf attachment,cache the opened vcf file descriptor for further
    // using.
    private final ConcurrentHashMap<Uri, AssetFileDescriptor> mVcfFds = new ConcurrentHashMap<Uri, AssetFileDescriptor>();
    /* @} */
    ...
    protected void deleteAttachment(final View attachmentView,
        ...
        /* SPRD: Modify for bug557044 {@ */
        // close vcf fd when deleting attachment.
        closeVcfFileDescriptors(attachment);
        /* @} */
    ...
    public Attachment generateLocalAttachment(Uri contentUri) throws AttachmentFailureException {
        ...
        /* SPRD: Modify for bug627832, origin bug557044 {@ */
    ...
    /* SPRD: Modify for bug557044 {@ */
    // Modify for vcf, only use {@link ContentProvider#openAssetFile} to get
    // length
    private int getSizeForVcf(Uri uri, ContentResolver contentResolver) {
        int size = -1;
        ...
        try {
            ...
            if (fd != null) {
                ...
                // cache the fd when adding it to composing
                mVcfFds.put(uri, fd);
            } else {
    ...
    // add for vcf file descriptor caching. @{
    public AbstractMap<Uri, AssetFileDescriptor> getVcfFds() {
        return mVcfFds;
    }

    public void closeAllVcfFds() {
        for (AssetFileDescriptor fd : mVcfFds.values()) {
            if (fd != null) {
                try {
                    fd.close();
                    LogUtils.d(LOG_TAG, "closeAllVcfFds close fd");
                } catch (IOException e) {
                }
            }
        }
    ...
    public void closeVcfFileDescriptors(Attachment attachment) {
        if (attachment.contentUri == null) {
            return;
        }
        AssetFileDescriptor fd = mVcfFds.remove(attachment.contentUri);
        if (fd != null) {
            try {
                fd.close();
                LogUtils.d(LOG_TAG, "closeVcfFileDescriptors close fd");
            } catch (IOException e) {
            }
        }
    }
