...
//#define LOG_NDEBUG 0
#define LOG_TAG "WAVExtractorEx"
#include <utils/Log.h>
#include "include/WAVExtractor.h"

#include "WAVExtractorEx.h"
...
struct WAVSourceEx : public MediaSource {
    WAVSourceEx(
...
private:
    ...
    WAVSourceEx(const WAVSourceEx &);
    WAVSourceEx &operator=(const WAVSourceEx &);
};

WAVExtractorEx::WAVExtractorEx(const sp<DataSource> &source)
    : WAVExtractor(source),
      mDataSource(source),
      mValidFormat(false),
...
WAVExtractorEx::~WAVExtractorEx() {
}

sp<MetaData> WAVExtractorEx::getMetaData() {
...
size_t WAVExtractorEx::countTracks() {
    return mInitCheck == OK ? 1 : 0;
}

sp<IMediaSource> WAVExtractorEx::getTrack(size_t index) {
    if (mInitCheck != OK || index > 0) {
        return NULL;
    }

    return new WAVSourceEx(
            mDataSource, mTrackMeta,
            mWaveFormat, mBlockAlign, mBitsPerSample, mDataOffset, mDataSize);
}

sp<MetaData> WAVExtractorEx::getTrackMetaData(
...
const size_t WAVSourceEx::kMaxFrameSize = 32768;

WAVSourceEx::WAVSourceEx(
        const sp<DataSource> &dataSource,
        const sp<MetaData> &meta,
        uint16_t waveFormat,
        int32_t blockAlign,
...
status_t WAVSourceEx::start(MetaData * /* params */) {
    ALOGV("WAVSourceEx::start");
...
status_t WAVSourceEx::stop() {
    ALOGV("WAVSourceEx::stop");
...
sp<MetaData> WAVSourceEx::getFormat() {
    ALOGV("WAVSourceEx::getFormat");

    return mMeta;
}

status_t WAVSourceEx::read(
...
bool SniffWAVEx(
    ...
    sp<MediaExtractor> extractor = new WAVExtractorEx(source);
