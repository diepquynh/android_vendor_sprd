...
static const struct cap_size_to_mem cam_mem_size_tab[IMG_SIZE_NUM] = {
	{PIXEL_1P3_MEGA, (6  << 20)},
	{PIXEL_2P0_MEGA, (8  << 20)},
...
static const struct cap_size_to_mem cam_raw_mem_size_tab[IMG_SIZE_NUM] = {
	{PIXEL_1P3_MEGA, (5  << 20)},
	{PIXEL_2P0_MEGA, (8  << 20)},
					...
					cmr_u32 *mem_sum)
{
	...
	*mem_sum = CMR_CAPTURE_MEM_SUM;
	...
	if ((BACK_CAMERA_ID == camera_id) || (FRONT_CAMERA_ID == camera_id)) {
		mem_tab_ptr = (struct cap_size_to_mem*)&cam_raw_mem_size_tab[0];
		yuv_mem_tab_ptr = (struct cap_size_to_mem*)&cam_mem_size_tab[0];
					...
					uint32_t *mem_size)
{
	...
	if (SENSOR_IMAGE_FORMAT_RAW == sn_fmt) {
		...
		if ((BACK_CAMERA_ID == camera_id) || (FRONT_CAMERA_ID == camera_id)) {
			mem_tab_ptr = (struct cap_size_to_mem*)&cam_mem_size_tab[0];
		} else {
			mem_tab_ptr = (struct cap_size_to_mem*)&mem_size_tab[0];
		}
	}
