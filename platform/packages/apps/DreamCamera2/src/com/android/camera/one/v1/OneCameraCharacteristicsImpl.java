...
public class OneCameraCharacteristicsImpl implements OneCameraCharacteristics {
    ...
    public Size[] getSupportedThumbnailSizes() {
        List<android.hardware.Camera.Size> originalSize = mCameraParameters.getSupportedJpegThumbnailSizes();
        Size[] sizes = new Size[originalSize.size()];
        for (int i = 0; i < originalSize.size(); i++) {
            sizes[i] =  new Size(originalSize.get(i));
        }
        return sizes;
    }
}
