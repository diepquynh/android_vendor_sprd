...
public class Storage {
    ...
    public static Uri addImage(ContentResolver resolver, String title, long date,
            Location location, int orientation, ExifInterface exif, byte[] data, int width,
            int height, String mimeType, String photoVoicePath) throws IOException {
     /* @} */
        ...
        */
        ...
        if (fileLength >= 0) {
             ...
             */
            return addImageToMediaStore(resolver, title, date, location, orientation, fileLength,
                    path, width, height, mimeType, photoVoicePath);
            /* @} */
        }
        return null;
    }
     ...
     /* @} */
        ...
        ContentValues values =
                getContentValuesForData(resolver, title, date, location, orientation, jpegLength, path, width,
                        height, mimeType, photoVoicePath);
        /* @} */
    ...
    public static ContentValues getContentValuesForData(ContentResolver resolver, String title,
            long date, Location location, int orientation, long jpegLength,
            String path, int width, int height, String mimeType, String photoVoicePath) {
    /* @} */
        ...
        /* SPRD: Fix bug 535110, Photo voice record. @{ */
        if (null != photoVoicePath && null != resolver) {
            ContentValues voiceVal = new ContentValues();
            voiceVal.put(MediaStore.Audio.Media.DATA, photoVoicePath);
            Uri voiceUri = resolver.insert(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, voiceVal);
            Log.e(TAG, "getContentValuesForData--> voiceUri = " + voiceUri);
            if (voiceUri != null) {
                Cursor c = resolver.query(voiceUri, new String[] {
                        MediaStore.Audio.Media._ID
                }, null, null, null);
                if (c != null && c.moveToFirst()) {
                    int id = c.getInt(c.getColumnIndex(MediaStore.Audio.Media._ID));
                    Log.e(TAG, "getContentValuesForData--> voice id = " + id);
                    values.put("photo_voice_id", id);
                }
                if(c != null){
                    c.close();
                }
...
//        return -1;
    ...
    private static Uri updateImage(Uri imageUri, ContentResolver resolver, String title,
        ...
        if (Storage.isSessionUri(imageUri)) {
             ...
             */
            resultUri = addImageToMediaStore(resolver, title, date, location, orientation,
                    jpegLength, path, width, height, mimeType, null);
            /* @} */
