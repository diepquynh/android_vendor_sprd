...
public final class GeometryMathUtils {
    ...
    private static Bitmap applyFullGeometryMatrix(Bitmap image, GeometryHolder holder) {
        ...
        /* SPRD: Modify for bug582000, only part of image is shown when straighten large image @{  */
        float scale = bitmapCache.getResizeScale();
        if (scale < 1)  {
            m = getCropSelectionToScreenMatrix(null, holder, width, height,
