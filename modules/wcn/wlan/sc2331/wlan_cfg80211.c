...
static void check_and_set_ip_address(wlan_vif_t *vif)
{
	int ret;
	__be32 ip_addr = 0;
	...
	if (ndev->flags & IFF_UP)
	{
		...
		if (ip_addr) {
			ret = wlan_cmd_get_ip(vif->id, (u8 *)&ip_addr);
			printkd("Current ip address: %pI4 is available, ret = %d\n", &ip_addr, ret);
		}
	}
}
				     ...
				     struct key_params *params)
{
	...
	/* fix bug 636606 : To check current ip address and set to CP2 if it is available */
	if (!pairwise && vif->id == 0)
		check_and_set_ip_address(vif);
...
void cfg80211_report_fake_probe(struct wiphy *wiphy, u8 *ie, size_t ielen)
{
	static int local_mac_ind;
	struct ieee80211_channel *channel;
	...
	if (0 == ielen) {
		pr_err("invalid ie length:%d,return",ielen);
		return;
	}
	...
	/*fill in with some nessary parameter*/
	freq = 2412;
	capability = 0x2D31;
	beacon_interval = 100;
	signal = -20;
	channel = ieee80211_get_channel(wiphy, freq);
	bss = cfg80211_inform_bss(wiphy, channel,fake_bssid, 0, capability,
				beacon_interval, fake_ie,fake_ielen, signal, GFP_KERNEL);
	cfg80211_put_bss(wiphy, bss);
}
...
void cfg80211_report_scan_frame(unsigned char vif_id, unsigned char *pData, int len)
{
	...
	if (vif->local_mac_flag)
		cfg80211_report_fake_probe(wiphy, ie, ielen);
...
int mac_addr_cfg(wlan_vif_t *vif, unsigned char vif_id)
{
	...
	/*check mac address--valid verify*/
	if (!is_valid_ether_addr(vif->ndev->dev_addr)) {
		netdev_err(vif->ndev, "%s invalid MAC address (%pM)\n",
				   __func__, vif->ndev->dev_addr);
		return -EINVAL;
	}
	/*check mac address--local verify*/
	if (is_local_ether_addr(vif->ndev->dev_addr)) {
