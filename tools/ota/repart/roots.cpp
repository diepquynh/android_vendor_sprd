...
#define TRY_MOUNT_TIMES 8
        ...
        while(tryCount++ < TRY_MOUNT_TIMES){
            if( mount(dev_buf, v->mount_point, v->fs_type, MS_NOATIME | MS_NODEV | MS_NODIRATIME, "") )
                sleep(SLEEP_SDCARD);
            else
                return 0;
        }
    ...
    if (strcmp(v->fs_type, "ext4") == 0 || strcmp(v->fs_type, "f2fs") == 0) {
        ...
        if (strcmp(v->fs_type, "ext4") == 0) {
            ...
            if (v->key_loc != NULL && strcmp(v->key_loc, "footer") == 0 && length < 0) {
                printf("format_volume: crypt footer + negative length (%zd) not supported on %s\n", length, v->fs_type);
                return -1;
            }
            if (length < 0) {
                printf("format_volume: negative length (%zd) not supported on %s\n", length, v->fs_type);
                return -1;
            }
            char *num_sectors;
        ...
        if (result != 0) {
            printf("format_volume: make %s failed on %s with %d(%s)\n", v->fs_type, v->blk_device, result, strerror(errno));
            return -1;
        }
        return 0;
    }
...
int setup_install_mounts() {
    if (fstab == NULL) {
        printf("can't set up install mounts: no fstab loaded\n");
        return -1;
    }
