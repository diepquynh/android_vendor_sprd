...
public class CompactContactEditorFragment extends ContactEditorBaseFragment implements
     ...
     * @{
     */
    private boolean mClickOnLinkedRawContact = false;
    /**
     * @
     */
    @Override
    ...
    protected void bindEditors() {
        ...
        /**
         * SPRD: Bug 596249 The DUT occurs crash after editing sim contact and rotate the screen.
         * SPRD: Bug 596697 The DUT occurs crash after adding photo for contact and rotating the
         * screen.
         * @{
         */
        if (mAccountWithDataSet == null && mRawContacts != null
                && mRawContacts.size() > 0) {
            RawContact rawContact = mRawContacts.get(0);
            String type = rawContact.getAccountTypeString();
            String name = rawContact.getAccountName();
            String dataSet = rawContact.getDataSet();
            mAccountWithDataSet = new AccountWithDataSet(name, type, dataSet);
        }
        /*
         * @}
         */
    ...
    public void setIntentExtras(Bundle extras) {
        super.setIntentExtras(extras);
        if (extras == null) {
            return;
        }
        int errorForGroupCreate = extras.getInt(ContactSaveService.EXTRA_ERROR_TOAST, 0);
        if (errorForGroupCreate != 0) {
            getContent().setIsCreateNewGroup(mGroupMetaData,mState);
        }
    }
    /**
     * @}
    ...
    public void removePhoto() {
        ...
        // SPRD : Bug 609182 when remove photo, should clear updatedphotos all
//        mUpdatedPhotos.remove(String.valueOf(mPhotoRawContactId));
    ...
    public void onNameFieldChanged(long rawContactId, ValuesDelta valuesDelta) {
        ...
        /**
         * SPRD: Bug 598042 should't show suggestion list when sim card
         * 
         * @{
         */
        if (!isSimType()) {
    ...
    public void onRebindEditorsForNewContact(RawContactDelta oldState,
        ...
        /**
         * SPRD: Bug 604106 save photo when switch phone to sim account with photos
         * 607477
         * 
         * @{
         */
...
//            mUpdatedPhotos.remove(String.valueOf(mPhotoRawContactId));
    ...
    public void onStop() {
        ...
        Log.d(TAG, "CompactContactEditorFragment onStop() mClickOnLinkedRawContact: " + mClickOnLinkedRawContact);
        Log.d(TAG, "CompactContactEditorFragment onStop() ((CompactContactEditorActivity) getActivity()).mIsEditPhoto: " + ((CompactContactEditorActivity) getActivity()).mIsEditPhoto);
        if (mClickOnLinkedRawContact
                || ((CompactContactEditorActivity) getActivity()).mIsEditPhoto
                || mJoinContacts) {
            return;
        }
        if (!isDeleting() && !getActivity().isChangingConfigurations() && mStatus == Status.EDITING) {
            //add for SPRD:Bug 606119
            /**
