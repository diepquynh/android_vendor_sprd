...
isp_s32 isp_u_pwd_block(isp_handle handle, void *block_info)
{
	...
	param.sub_block = ISP_BLOCK_PRE_WAVELET;
	param.property = ISP_PRO_PWD_BLOCK;
...
isp_s32 isp_u_pwd_slice_size(isp_handle handle, isp_u32 width, isp_u32 height)
{
	...
	if (!handle) {
		ISP_LOGE("isp_u_pwd_slice_size: handle is null error.");
		return -1;
	}
	...
	param.isp_id = file->isp_id;
	param.sub_block = ISP_BLOCK_PRE_WAVELET;
	param.property = ISP_PRO_PWD_SLICE_SIZE;
	size.width = width;
	size.height = height;
