...
public class CarrierHandoverTimesFragment extends Fragment {

    private String TAG = "CarrierHandoverTimesFragment";
    ...
    // Inflate the layout for this fragment
        ...
        return inflater.inflate(R.layout.carrier_handover_times, container, false);
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mTemp = R.id.changetimes_label00;
        Log.d(TAG,"temp="+mTemp+" "+R.id.changetimes_label01+" "+R.id.changetimes_label10+" "+R.id.changetimes_label11);
        for (int mc = 0; mc < ROW; mc++) {
            for (int ml = 0; ml < COL; ml++) {
                mViewID[mc][ml] = mTemp++;
                Log.d(TAG,"mc="+mc+",ml="+ml+",id="+mViewID[mc][ml]);
                if (ml == (COL - 1)) {
                    mTemp++;
                }
            }
    ...
    private void parseNetStatistic(int simindex) {
        ...
        String atRSP = IATUtils.sendATCmd("AT+SPENGMD=0,3,4", "atchannel"
            + simindex);
        if (mFragmentDestroyed) {
            Log.d(TAG, "mFragmentDestroyed ==" + mFragmentDestroyed);
            return;
        }
        Log.d(TAG,"AT+SPENGMD=0,3,4 result is:"+atRSP);
        if (atRSP != null && atRSP.contains(IATUtils.AT_OK)) {
            ...
            for(int i=0;i<str.length;i++){
                mTextValue[i][1]=str[i];
            }
            if (!isSupportLTE) {
                mTextValue[ROW-2][1] = "NA";
                mTextValue[ROW-1][1] = "NA";
            }
            ...
            mUiThread.post(new Runnable() {
                ...
                public void run() {
                    for (int i = 0; i < ROW; i++) {
                        mTextView[i][1].setText(mTextValue[i][1]);
                    }
                }
            });
        } else {
