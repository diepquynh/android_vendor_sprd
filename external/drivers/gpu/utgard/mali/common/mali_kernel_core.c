		...
		if (NULL != group) {
			...
			if (NULL != pp_core) {
				...
				case MALI200_PP_PRODUCT_ID:
					...
					MALI_DEBUG_PRINT(4, ("Found Mali GPU Mali-200 r%up%u\n", global_gpu_major_version, global_gpu_minor_version));
				...
				case MALI300_PP_PRODUCT_ID:
					...
					MALI_DEBUG_PRINT(4, ("Found Mali GPU Mali-300 r%up%u\n", global_gpu_major_version, global_gpu_minor_version));
					break;
				case MALI400_PP_PRODUCT_ID:
					global_product_id = _MALI_PRODUCT_ID_MALI400;
					MALI_DEBUG_PRINT(4, ("Found Mali GPU Mali-400 MP r%up%u\n", global_gpu_major_version, global_gpu_minor_version));
					break;
				case MALI450_PP_PRODUCT_ID:
					global_product_id = _MALI_PRODUCT_ID_MALI450;
					MALI_DEBUG_PRINT(4, ("Found Mali GPU Mali-450 MP r%up%u\n", global_gpu_major_version, global_gpu_minor_version));
					break;
				case MALI470_PP_PRODUCT_ID:
					global_product_id = _MALI_PRODUCT_ID_MALI470;
					MALI_DEBUG_PRINT(4, ("Found Mali GPU Mali-470 MP r%up%u\n", global_gpu_major_version, global_gpu_minor_version));
					break;
				default:
		...
		_mali_osk_resource_t *resource_bcast)
{
	...
	MALI_DEBUG_PRINT(4, ("Starting new virtual group for MMU PP broadcast core %s\n", resource_mmu_pp_bcast->description));
...
static _mali_osk_errcode_t mali_parse_config_memory(void)
{
	...
	if (0 < mali_dedicated_mem_size && 0 != mali_dedicated_mem_start) {
		MALI_DEBUG_PRINT(4, ("Mali memory settings (dedicated: 0x%08X@0x%08X)\n",
	...
	if (0 < mali_shared_mem_size) {
		MALI_DEBUG_PRINT(4, ("Mali memory settings (shared: 0x%08X)\n", mali_shared_mem_size));
	...
	if (0 == mali_fb_start && 0 == mali_fb_size) {
		...
		MALI_DEBUG_PRINT(4, ("Using device defined frame buffer settings (0x%08X@0x%08X)\n",
				     mali_fb_size, mali_fb_start));
	} else {
		MALI_DEBUG_PRINT(4, ("Using module defined frame buffer settings (0x%08X@0x%08X)\n",
				     mali_fb_size, mali_fb_start));
	}
...
void mali_terminate_subsystems(void)
{
	...
	MALI_DEBUG_PRINT(4, ("terminate_subsystems() called\n"));
...
_mali_osk_errcode_t _mali_ukk_wait_for_notification(_mali_uk_wait_for_notification_s *args)
{
	...
	if (NULL == queue) {
		MALI_DEBUG_PRINT(4, ("No notification queue registered with the session. Asking userspace to stop querying\n"));
...
_mali_osk_errcode_t _mali_ukk_request_high_priority(_mali_uk_request_high_priority_s *args)
{
	...
	if (!session->use_high_priority_job_queue) {
		...
		MALI_DEBUG_PRINT(4, ("Session 0x%08X with pid %d was granted higher priority.\n", session, _mali_osk_get_pid()));
	}
