...
import com.android.sprdlauncher1.CellLayout;
import com.android.sprdlauncher1.DeleteDropTarget;
import com.android.sprdlauncher1.DragView;
import com.android.sprdlauncher1.Workspace;
import com.android.sprdlauncher1.DragController;
import com.android.sprdlauncher1.DragSource;
import com.android.sprdlauncher1.DropTarget;
import com.android.sprdlauncher1.Launcher;
import com.android.sprdlauncher1.PagedView;
...
//        margin=mLeftMargin;
    ...
    private View updateOnDismiss(View currentPreviewItem){
        ...
        if (currentPreviewItem != null) {
            // SPRD: fix bug286343
            mWorkspace.mCurrentPage = ((PreviewsBaseItem)currentPreviewItem).mIndex;
        }else{
            for (int i = 0; i < count; i++) {
    ...
    private void displayDismissAnimation(int workspaceCurrentScreen, View child) {
        ...
        /* SPRD:fix bug 289294,monkey test ,NullPointerException
        mWorkspace.getChildAt(workspaceCurrentScreen).startAnimation(
                workspaceAnimation);*/
        View currentView = mWorkspace.getChildAt(workspaceCurrentScreen);
        if(currentView != null){
            currentView.startAnimation(workspaceAnimation);
        }else{
            Log.w(TAG, "The workspace childCount is "+mWorkspace.getChildCount()+";the workspaceCurrentScreen is "+workspaceCurrentScreen);
        }
        /* @} */
    ...
    private void displayPreviewsAnimation(int workspaceCurrentScreen) {
        ...
        Animation previewViewAnimation = new ScaleAnimation(multipleX, 1,
                multipleX, 1, Animation.RELATIVE_TO_SELF, animationFocusX
                        / (float) (thumbnailWidth), Animation.RELATIVE_TO_SELF,
                animationFocusY / (float) thumbnailHeight);
        previewViewAnimation.setDuration(mAnimationDuration);
        /* SPRD fix bug296145 null pointer of currentChild @{ */
        try{
            this.getChildAt(mWorkspace.getCurrentScreen()).setVisibility(View.GONE);
        }catch(Exception e){
            e.printStackTrace();
            if(mWorkspace != null){
                Log.w(TAG, "currentScreen:" + mWorkspace.getCurrentPage()
                        + ",PreviewsChildCount:" + this.getChildCount());
            }else{
                Log.w(TAG, "Previews: mWorkspace is null");
            }
        }
        ...
        /* SPRD: fix bug284427 null pointer of currentChild @{ */
        try {
            mWorkspace.getChildAt(mWorkspace.getCurrentScreen())
                    .startAnimation(workspaceScaleAnimation);
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(TAG, "currentScreen:" + mWorkspace.getCurrentPage()
    ...
    private void initPreviewsItem(int index) {
        ...
        /* SPRD: Fix bug 264250,do not show preview_add_item while reached MAX_PAGE. @{ */
        } else if (index == mWorkspace.getChildCount() && index < Workspace.MAX_PAGE) {
        /* @} */
            child = (PreviewsBaseItem) factory.inflate(
    ...
    protected void deleteChild(PreviewsBaseItem child) {
        ...
        /* SPRD: Fix bug 264250, we'd better user the child count of Workspace to ensure screen count. @{ */
        int count = mWorkspace.getChildCount();
        /* @} */
        if (count < Workspace.MIN_PAGE + 1) {
        ...
        removeView(child);
        // SPRD : fix bug202233 cellLayout not removed from database
        /* SPRD: Fix bug 265652, delete the right screen. @{ */
        mWorkspace.deleteCellLayout(id);
        /* @} */
        count = getChildCount();
        for (int i = 0; i < count; i++) {
            ...
            /* SPRD: Fix bug 264250, the min count of previewItem should equals Workspacce.MIN_PAGE + 1. @{ */
    ...
    protected void addChild() {
        ...
        int currCount = mWorkspace.getChildCount();
        mWorkspace.insertNewWorkspaceScreenWithDb(newId);
        if (mWorkspace.getChildCount() != currCount + 1) {
            return;
        }
        /* @} */
        int count = mWorkspace.getChildCount();
        ...
        /* SPRD: Fix bug 264250,update CloseView of previewItem after add an item. @{ */
        for (int i = 0; i < getChildCount(); i++) {
            PreviewsBaseItem view = (PreviewsBaseItem)getChildAt(i);
            if (view instanceof PreviewsDefaultItem) {
    ...
    public void dismissPreview(int workspaceCurrentScreen, View child) {
        ...
        mAnimate.clear();
        snapToScreen(curScreen); // SPRD:fix bug 286051,the previews displayed abnormal when lock the screen.
    }
    ...
    class DisMissPreviewsAnimationListener implements AnimationListener {
        ...
        public void onAnimationEnd(Animation arg0) {
         ...
         //   mWorkspace.updateOwnerWidget(workspaceCurrentScreen, false);
            ...
            /* SPRD: bug316020 2014-05-28 don't hold memory after previews disappear. @{ */
