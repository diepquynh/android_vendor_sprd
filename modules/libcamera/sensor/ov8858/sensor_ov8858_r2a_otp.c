...
#define CHANNEL_SCALE_RATIO			0xC8
#define R_TYPICAL_ov8858_truly		0x0070
#define G_TYPICAL_ov8858_truly		0x00C8
#define B_TYPICAL_ov8858_truly		0x0076
...
//convert r/g,b/g ration to r,g,b chanel data
static uint32_t ov8858_rgb_ratio_to_data(void*param_ptr){
	uint32_t rtn = SENSOR_SUCCESS;
	int r,g,b;
	if(!param_ptr) {
		SENSOR_PRINT("error:param_ptr is null");
		rtn = SENSOR_FAIL;
	}
	struct otp_info_t *otp_ptr=(struct otp_info_t *)param_ptr;
	r = ((float)(otp_ptr->rg_ratio) - 0.5) / 512 * CHANNEL_SCALE_RATIO;
	b = ((float)(otp_ptr->bg_ratio) - 0.5) / 512 * CHANNEL_SCALE_RATIO;
	g = 1 * CHANNEL_SCALE_RATIO;
	otp_ptr->r_current = r;
	otp_ptr->g_current = g;
	otp_ptr->b_current = b;
	return rtn;
...
static uint32_t ov8858_otp_read_otp_info(struct otp_info_t *otp_ptr)
{
	...
	ov8858_rgb_ratio_to_data(&otp_info);
...
static uint32_t ov8858_otp_update_otp(void *param_ptr)
{
	...
	char	value[100];
	property_get("debug.otp.disable.otp_cal", value, "0");
	SENSOR_PRINT("update_otp:%d",atoi(value));
	if(!atoi(value)){
		SENSOR_PRINT("update_otp:%d",atoi(value));
