...
public class VideoItem extends FilmstripItemBase<VideoItemData> {
    /* SPRD: Fix bug 605492 that video thumbnail is blurred @{ */
    private final Size DEFAULT_DISPLAY_SIZE = CameraUtil.getDefaultDisplaySize();
    private Size mResizedSize = null;
    /* @} */
    ...
    /* SPRD: Fix bug 605492 that video thumbnail is blurred @{ */
    private Size getResizedDimensions() {
        if (mResizedSize != null) {
            return mResizedSize;
        }
        Size size = getDimensions();
        if (size.getWidth() <= DEFAULT_DISPLAY_SIZE.getWidth()
                && size.getHeight() <= DEFAULT_DISPLAY_SIZE.getHeight()) {
            return size;
        } else {
            float widthRatio = size.getWidth() / DEFAULT_DISPLAY_SIZE.getWidth();
            float heightRatio = size.getHeight() / DEFAULT_DISPLAY_SIZE.getHeight();
    ...
    public void renderThumbnail(@Nonnull View view) {
         ...
         */
        // SPRD: Fix bug 624078, 1080P video thumbnail blur, load full size thumbnail
        mGlideManager.loadFull(mData.getUri(), generateSignature(mData), getResizedDimensions())
                .thumbnail(mGlideManager.loadMediaStoreThumb(mData.getUri(),
                        generateSignature(mData)))
                .into(getViewHolder(view).mVideoView);
        /* @} */
