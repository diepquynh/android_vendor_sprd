...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        void loadBookmarks() {
            ...
            /*Bug504481 requery the root bookmarks to avoid show root bookmark*/
            if ((mBookmarks != null)&&(mBookmarks.getCount() == 0)){
                resetBookmarks();
                mCurrentFolder = 1;
                Uri requeryUri = ContentUris.withAppendedId(
                        BrowserContract.Bookmarks.CONTENT_URI_DEFAULT_FOLDER,
                        mCurrentFolder);
                mBookmarks = mContext.getContentResolver().query(requeryUri, PROJECTION,
                        null, null, null);
            }
