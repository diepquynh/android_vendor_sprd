...
public class Terminal {
    ...
    public synchronized boolean initializeAccessControl(boolean reset) {
        ...
        if (isCardPresent) {
            ...
            if (mAccessControlEnforcer == null) {
                Log.d(SmartcardService.LOG_TAG, "Initializing Access Control for " + getName() + " CREATE AccessControlEnforcer");
                mAccessControlEnforcer = new AccessControlEnforcer(this);
            } else {
                Log.d(SmartcardService.LOG_TAG, "Initializing Access Control for " + getName() + " AccessControlEnforcer ALREADY EXISTS");
            }
    ...
    public ISmartcardServiceSession openSession(int readerId) throws Exception {
        if (!isCardPresent()) {
            throw new IOException("Secure Element is not presented.");
        }

        synchronized (mLock) {
            /* SPRD add for following CMCC OMAPI Test spec @{
                Try to update the AC rules while open a new session.
            Original:
            ...
            */
            ...
            Session session = new Session(this, readerId , mContext);
            Log.d(SmartcardService.LOG_TAG, "Terminal::openSession - session.addr" + session.toString());
    ...
    private synchronized void closeSessions(int readerId) throws Exception {
        int count = mSessions.size();
        int index = 0;

        for (int i = 0; i < count; i++) {
            if (readerId == mSessions.get(index).getReaderId()) {
    ...
    private class SmartcardServiceReader extends ISmartcardServiceReader.Stub {
        ...
        public SmartcardServiceReader () {
            mReaderId = mIdGenerator.getAndIncrement();
            Log.d(SmartcardService.LOG_TAG , "mReaderId : " + mReaderId);
        }
        ...
        public ISmartcardServiceSession openSession(SmartcardError error) throws RemoteException {
            try {
                return Terminal.this.openSession(mReaderId);
            } catch (Exception e) {
        ...
        public void closeSessions(SmartcardError error) throws RemoteException {
            try {
                Terminal.this.closeSessions(mReaderId);
            } catch (Exception e) {
