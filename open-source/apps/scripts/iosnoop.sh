...
flock=/dev/.ftrace-lock
bufsize_kb=4096
opt_duration=0; duration=; opt_name=0; name=; opt_pid=0; pid=; ftext=
opt_start=0; opt_issue=1; opt_end=0; opt_device=0; device=; opt_iotype=0; iotype=
opt_queue=0
...
[[ -x /usr/bin/$use ]] && awk=$use || awk="busybox awk"
wroteflock=1
...
if (( opt_iotype || opt_device || opt_pid )); then
	if ! echo "$filter_i" > events/block/$b_start/filter || \
	    ! echo "$filter_i" > events/block/block_rq_issue/filter || \
...
(( opt_start )) && busybox printf "%-15s " "STARTs"
(( opt_issue )) && busybox printf "%-15s " "ISSUEs"
(( opt_end )) && busybox printf "%-15s " "ENDs"
if (( opt_issue )); then
	busybox printf "%-12.12s %-6s %-4s %-8s %-12s %-6s %-10s %-10s %8s\n" \
	"COMM" "PID" "TYPE" "DEV" "BLOCK" "BYTES" "LATms(HW)" "LATms(ALL) (HW/ALL)%"
else
	busybox printf "%-12.12s %-6s %-4s %-8s %-12s %-6s %8s\n" \
...
( if (( opt_duration )); then
    ...
    -v opt_duration=$opt_duration -v opt_start=$opt_start -v opt_issue=$opt_issue -v opt_end=$opt_end \
    -v b_start=$b_start '
	# common fields
	$1 != "#" {
	...
	$1 != "#" && $0 ~ /rq_complete/ {
		...
		nsec = $(NF-1)
		if (starts[dev, loc] > 0) {
			latency_ALL = sprintf("%.2f",
			    1000 * (time - starts[dev, loc]))
			if (opt_issue)
				latency_HW = sprintf("%.2f",
					1000 * (time - issue_time[dev, loc]))
			comm = comms[dev, loc]
			pid = pids[dev, loc]
			if (opt_start)
				printf "%-15s ", starts[dev, loc]
			if (opt_issue)
				printf "%-15s ", issue_time[dev, loc]
			if (opt_end)
				printf "%-15s ", time
			if (opt_issue)
				printf "%-12.12s %-6s %-4s %-8s %-12s %-6s %-10s %-10s %8s%%\n",
					comm, pid, dir, dev, loc, nsec * 512, latency_HW, latency_ALL, latency_HW*100/latency_ALL
			else
				printf "%-12.12s %-6s %-4s %-8s %-12s %-6s %8s\n",
					comm, pid, dir, dev, loc, nsec * 512, latency_ALL
			if (!opt_duration)
				fflush()
			delete starts[dev, loc]
			delete issue_time[dev, loc]
