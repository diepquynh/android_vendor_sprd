    ...
    private class LoaderTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... v) {
            ...
            try{
                ...
                // Do not create default contact for profile owner of cts test.
                ...
                try {
                    cursor = getContentResolver().query(
                            ...
                            RawContacts.RAW_CONTACT_IS_READ_ONLY + "=1 AND "
                                    + RawContacts.SYNC1 + " IS NOT 'sdn'",
                            null, null);
                    Log.d(TAG, "PreDefineContactsService cursor.getCount() is " + cursor.getCount());
                    Log.d(TAG, "PreDefineContactsService packageName is " + packageName);
                    if (cursor.getCount() > 0 || packageName.equals("com.android.cts.managedprofile")) {
                        return true;
                    }
                } finally {
                    if (cursor != null) {
