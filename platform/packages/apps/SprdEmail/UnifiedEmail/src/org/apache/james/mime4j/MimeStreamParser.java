...
//        }
        ...
        while (pos < sb.length()) {
            ...
            if (pos >= sb.length() - 2 || fieldChars.get(sb.charAt(pos + 2))) {
                 ...
                 * trailing \r\n.
                 */
                /* SPRD: Add for bug553432 @{ */
                String field = sb.substring(start, pos);
                byte[] bytesOfUnkown = bytesFromUnknownString(field);
                String properCharset = CharsetUtilities.emailCharsetDetect(new ByteArrayInputStream(bytesOfUnkown));
                if (!TextUtils.isEmpty(properCharset)) {
                    try {
                        field = new String(bytesOfUnkown, properCharset);
