...
int loadlibrary_camera_so()
{
	...
	DBGMSG("linking camera_handle symbol is failed");
	return -1;
    }
	...
	DBGMSG("linking atcamera_set_testmode symbol is failed");
    ...
    {
	DBGMSG("linking atcamera_get_image_from_buf symbol is failed");
	return -1;
    }
    ...
    {
	DBGMSG("linking atcamera_close_testmode symbol is failed");
	return -1;
    }
    ...
    {
	DBGMSG("linking eng_tst_camera_init symbol is failed");
	return -1;
    }
    else
    {
...
int skd_testCamParal(const uchar * data, int data_len, uchar * rsp, int rsp_size)
{
	...
	if( loadlibrary_camera_so() < 0){
	    ERRMSG("the loadlibrary camera so fail \n");
	    return -1;
	};
	test_result_init();
	switch( data[0] ) {
...
void   cam_sd_tcard_addvoltage(void)
{
    ...
    usleep(100*1000);
    system("echo 1 > /sys/kernel/debug/sprd-regulator/vddsim2/enable");
    usleep(100*1000);
...
//------------------------------------------------------------------------------
//modify the new interface func sim_newCheck()
//------------------------------------------------------------------------------
...
int testLKBV(const uchar * data, int data_len, uchar * rsp, int rsp_size)
{
  	...
  	case 0x0a: //battery tempture
	{
		...
		rettemp = batGetTemperatureValue(bat_temp);
	  	if (rettemp != 0)
			ret = -1;
		else if(rettemp == 0){
			ret = 4;
...
int testFM(const uchar * data, int data_len, uchar * rsp, int rsp_size)
{
	...
	if( Radio_Open(freq) < 0 || Radio_Play(freq) < 0 ){
	    ...
	    //INFMSG("Radio_Open or Radio_Play the fm open freq = %d!!!\n",freq);
...
#else
#ifdef SPRD_WCNBT_SR2351 //for 2351
