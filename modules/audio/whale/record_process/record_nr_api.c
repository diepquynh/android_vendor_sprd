...
#define LOG_TAG "audio_hw_record_nr"
                                    ...
                                    void *data_read_handle, int request_channel)
{
    ...
    LOG_D("Audio Nr Nv Info: 0x%x   0x%x    0x%x   %d    %d   %d   %d   %d",
          pNvInfo[0], pNvInfo[1],
          pNvInfo[2], pNvInfo[3], pNvInfo[4], pNvInfo[5], pNvInfo[6],
          pNvInfo[7]);
    /*init record nr*/
    audio_record_nr_init(pNvInfo);
    LOG_D("NR:audio_record_nr_init compelte\n");
                                 ...
                                 size_t bytes, int channel)
{
    ...
    do {
        ...
        bytes -= read_bytes;
        LOG_D("AudioRecordNr_ProcEx byres = %d, read_bytes = %d", bytes, read_bytes);
        if(channel == 1) { //one channel,left buffer same as right buffer
            ...
            LOG_D("AudioRecordNr_ProcEx 1  read_byres  = %d, dest_count = %d", read_bytes,
                  dest_count);
            for ( i = 0;  i < read_bytes / 2; i++) {
            ...
            LOG_D("AudioRecordNr_ProcEx 2 read_byres  = %d, dest_count = %d", read_bytes,
                  dest_count);
            for ( i = 0;  i < read_bytes / 4; i++) {
