...
public class CellLayout extends ViewGroup {
    ...
    public boolean mEditAdd = false;
    protected Drawable mAddBg;
    /* @} */
    ...
    public void restoreInstanceState(SparseArray<Parcelable> states) {
        /* SPRD: modify 278498, sometimes on rotating the phone, @{
         * Binding Items fail to restore its states... so...
        dispatchRestoreInstanceState(states); */
        try{
            dispatchRestoreInstanceState(states);
        }catch (Exception e){
            Log.i(TAG, "dispatchRestoreInstanceState",e);
        }
        /* @} */
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child == mDeleteView) {
                child.measure(
                    MeasureSpec.makeMeasureSpec(newWidth, MeasureSpec.UNSPECIFIED),
                    MeasureSpec.makeMeasureSpec(newHeight, MeasureSpec.UNSPECIFIED));
            } else {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child == mDeleteView) {
                child.layout(
                    getMeasuredWidth() - getPaddingRight() - child.getMeasuredWidth() - mMarginPixels,
                    top + mMarginPixels,
                    getMeasuredWidth() - getPaddingRight() - mMarginPixels,
                    top + child.getMeasuredHeight() + mMarginPixels);
            } else {
    ...
    class ReorderHintAnimation {
        ...
        void animate() {
            ...
            /* SPRD: set duration scale to default value only when getting zero value @{ */
            if (va.getDurationScale() < 1.0f) va.setDurationScale(1.0f); /* @} */
            va.setDuration(DURATION);
    ...
    */
    ...
    public static int[] rectToCell(int width, int height, int[] result) {
        ...
        /* SPRD: Fix bug288492, @{ */
        int numColumns = (int)grid.numColumns;
        int numRows = (int)grid.numRows;
        if (spanX > numColumns) {
            Log.d(TAG, "this widget's spanX= "+ spanX + ", big than defined max spanX:" + numColumns
                    );
            spanX = numColumns;
        }
        if (spanY > numRows) {
            Log.d(TAG, "this widget's spanY= " + spanY + ", big than defined max spanY:" + numRows);
            spanY = numRows;
        }
        /* @} */
        if (result == null) {
