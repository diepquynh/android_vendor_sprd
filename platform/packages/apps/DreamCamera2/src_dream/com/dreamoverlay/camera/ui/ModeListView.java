        ...
        public boolean shouldHandleTouchEvent(MotionEvent ev) {
            if (MotionEvent.ACTION_DOWN == ev.getActionMasked()) {
                if (isTouchInsideList(ev) &&
                    ...
                    int itemSelected = getFocusItem(ev.getX(), ev.getY());
                    if (itemSelected != NO_ITEM_SELECTED) {
                        mChildViewTouched = mModeSelectorItems[itemSelected];
                        return true;
                    }
                }
        ...
        public SelectedState(ModeSelectorItem selectedItem) {
            ...
            if (mScreenShotProvider != null) {
                if (modeId == WIDEANGLE_MODE_INDEX
                        ...
                        || (((CameraActivity) getContext()).getCurrentModule() != null
                            && ((CameraActivity) getContext()).getCurrentModule().getModuleTpye() == DreamModule.GIF_MODULE)) {
                    effect.setBackground(
...
//            mListView.addView(selectorItem);
    ...
    private int getFocusItem(float x, float y) {
        ...
        Log.d(TAG, "getFocusItem " + x + "," + y);
        x -= mListView.getX();
        y -= mListView.getY();
        Log.d(TAG, "getFocusItem " + x + "," + y);
        for (int i = 0; i < mModeSelectorItems.length; i++) {
            if (mModeSelectorItems[i].getParent() == null)
                continue;
            float left = mModeSelectorItems[i].getLeft();
            float right = mModeSelectorItems[i].getRight();
            float top = mModeSelectorItems[i].getTop();
            float bottom = mModeSelectorItems[i].getBottom();
            Log.d(TAG, "i=" + i + "," + left + "," + right + "," + top + "," + bottom);
...
//            mModeSelectorItems[i].setTextAlpha(openRatio);
            mModeSelectorItems[i].setTextAlpha(1);
        }
                        ...
                        public void run() {
                            ...
                            if ((ModeListView.this.getContext() == null)
                                    || (ModeListView.this.getContext() != null
                                            ...
                                            .isPaused() || ((CameraActivity) ModeListView.this
                                            .getContext())
    ...
    public void updateList(int mode, int camera) {
        mCount = 0;
        if (mModeSelectorItems != null && mModeSelectorItems.length > 0) {
...
//                                R.dimen.mode_selector_margin_top);
//                    }
//                    params.leftMargin = getResources().getDimensionPixelSize(
//                            R.dimen.mode_selector_margin);
...
//                    selectorItem.setLayoutParams(params1);
                }
            }
        }
    }
