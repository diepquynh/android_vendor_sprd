        ...
        protected Uri doInBackground(Void... v) {
            ...
            try {
                ...
                if (CameraUtil.mTimeStamp) {
                    ...
                    Log.i(TAG, "CML_addWatermarkForJpeg end newData = " + newData);
                    /*SPRD:fix bug648222 bad jpeg data for timestamp @*/
                    if (newData == null) {
                        return null;
                    }
                    /* @} */
...
/*        int filterType = settingManager.getInteger(SettingsManager.SCOPE_GLOBAL
        ...
        int cameraId = FilterTranslationUtils.getInstance().getCameraId();;
        ...
        if (bitmap != null) {
            ...
            /* SPRD: Fix bug 625214, the position of thumbnail incorrect in filter. @{ */
        ...
        if (result) {
            ...
            if (path != null) {
                File imageFile = new File(path);
                if (imageFile != null) {
                    newLength = (int) imageFile.length();
                }
            }

            return Storage.addImageToMediaStore(resolver, title, date, location, orientation, newLength,
