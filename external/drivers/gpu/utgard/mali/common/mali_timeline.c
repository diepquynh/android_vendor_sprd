...
static void mali_timeline_destroy(struct mali_timeline *timeline)
{
	...
	if (NULL != timeline->delayed_work) {
		/* print timeline related info */
		MALI_DEBUG_PRINT(4,("timeline = %p\n",timeline));
		MALI_DEBUG_PRINT(4,("timeline->id = %d\n",timeline->id));
#if defined(CONFIG_SYNC)
		MALI_DEBUG_PRINT(4,("timeline->destroyed  = %d\n",timeline->destroyed));
#endif
		MALI_DEBUG_PRINT(4,("timeline->delayed_work= %p\n",timeline->delayed_work));
		MALI_DEBUG_PRINT(4,("timeline->timer_active = %d\n",timeline->timer_active));
		_mali_osk_wq_delayed_cancel_work_sync(timeline->delayed_work);
		...
		if (NULL != timeline->spinlock) {
			mali_spinlock_reentrant_term(timeline->spinlock);
		}
