...
*/        if (into) {
            ...
            switch (what) {
                ...
                case AsyncLoadResource.VAL_STATE_FAILED:
                    synchronized (mLoadTask.mStateLock) {
                        ...
                        if (mResource != null) {
                            if (!mResource.isRecycled()) {
                                mResource.recycle();
                            }
                            mResource = null;
                        }
                        synchronized (mLoadTask.mTmpLock) {
                            if (mTmpResource != null) {
                                if (!mTmpResource.isRecycled()) {
                                    mTmpResource.recycle();
                                }
                                mTmpResource = null;
                            }
                        }
                ...
                case AsyncLoadResource.VAL_STATE_FINISHED:
                    synchronized (mLoadTask.mStateLock) {
                        synchronized (mLoadTask.mTmpLock) {
                            if (mTmpResource != null && !mTmpResource.isRecycled() && mImage != null) {
                                mImage.setImageBitmap(mTmpResource);
                                if (mResource != null && !mResource.isRecycled()) {
                                    mResource.recycle();
                                }
                                mResource = mTmpResource;
    ...
    private class AsyncLoadResource implements Runnable {
        ...
        /*package*/ final Object mTmpLock = new Object();
        ...
        public void run() {
            synchronized (mMutexLock) {
                ...
                if (tmpBitmap != null) {
                    synchronized (mTmpLock) {
                        mTmpResource = tmpBitmap;
                    }
