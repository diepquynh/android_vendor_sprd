...
public class AddBookmarkPage extends Activity
    ...
    public void onClick(View v) {
        if (v == mButton) {
            ...
            createHandler();
            Message msg = Message.obtain(mHandler, BOOKMARK_DELETED);
            sAlertdialog = BookmarkUtils.displayRemoveBookmarkDialog(id,
        ...
        public void onLoadFinished(Loader<EditBookmarkInfo> loader,
            ...
            if (info.id != -1) {
                ...
                if (info.title != null) {
                    ...
                    if (isPause) {
                        /*SPRD: change for bug 573960 @{*/
                        /*mTitle.setSelection(mTitle.getText().length());*/
                        if (mTitle.getText().length() > max_length) {
                            mTitle.setSelection(max_length);
                        } else {
                            mTitle.setSelection(mTitle.getText().length());
                        }
                        /*@}*/
                        ...
                        if (info.title.length() > max_length) {
                            mTitle.setSelection(max_length);
                        } else {
                            mTitle.setSelection(info.title.length());
                        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mMap != null) {
            ...
            /*SPRD: add for bug 565848,remove space and newline character @{*/
            if (title!=null) {
                /* SPRD:bug626540 add begin @{ */
                title = title.trim();
                Pattern p = Pattern.compile("\t|\r|\n");
                /* @} */
                Matcher m = p.matcher(title);
                title_change = m.replaceAll("");
            }
            /*@}*/
            /**
             * modify save bookmark to folder
             *@{
             */
            /*SPRD: change for bug 565848 @{*/
            mOriginalFolderName = title_change;
            /*@}*/
            /*@}*/
        ...
        /*SPRD: add for bug 565848,set focus to tilte EditText @{*/
        ...
        /*SPRD: change for bug 565848 @{*/
        int max_length = getResources().getInteger(R.integer.max_url_length);
        mTitle.setFilters(new InputFilter[]{ new InputFilter.LengthFilter(max_length)});
        mTitle.setText(title_change);
        if (title_change != null) {
            /*SPRD: change for bug 573960 @{*/
            /*mTitle.setSelection(title_change.length());*/
            if (title_change.length() > max_length) {
                mTitle.setSelection(max_length);
            } else {
                mTitle.setSelection(title_change.length());
            }
    ...
    private class SaveBookmarkRunnable implements Runnable {
        ...
        public void run() {
            ...
            //url editbox has max length, use the original url for data url in bookmark
            if(DataUri.isDataUri(url)) {
                if (url != null && mOriginalUrl != null) {
                    if (mOriginalUrl.startsWith(url)) {
                        Log.d(LOGTAG,"SaveBookmarkRunnable, data url too long");
                        url = mOriginalUrl;
                    }
                }
            }
                ...
                public void handleMessage(Message msg) {
                    switch (msg.what) {
                        ...
                        case TOUCH_ICON_DOWNLOADED:
                            ...
                            String title = b.getString(BrowserContract.Bookmarks.TITLE);
                            ...
                            Toast.makeText(AddBookmarkPage.this, AddBookmarkPage.this.getString(R.string.shortcut_installed, title),
            ...
            if (mEditingFolder) {
                ...
                if (mSaveToHomeScreen) {
                    if (mTouchIconUrl != null && urlUnmodified) {
                        ...
                        Toast.makeText(this, this.getString(R.string.shortcut_installed, title),
                            Toast.LENGTH_SHORT).show();
                    }
                } else {
