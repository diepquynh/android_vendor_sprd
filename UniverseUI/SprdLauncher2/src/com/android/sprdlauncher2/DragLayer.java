...
public class DragLayer extends FrameLayout implements ViewGroup.OnHierarchyChangeListener {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        /* SPRD: bug272782 2014.1.23 Uninstallbar is show,the first row can move @{ */
        /* MODIFY: remove the codes which get the instance of MotionEvent */
        if(mLauncher.isUninstallerVisible()) return true;
        /* @} */
    ...
    public void addResizeFrame(ItemInfo itemInfo, LauncherAppWidgetHostView widget,
        ...
        /* SPRD: Fix bug 270382, do not do resize of widget while in EditingMode. @{ */
        if (mLauncher.isEditingMode()) return;
        /* @} */
        ...
        /* SPRD: bug281026 2014-02-21 delay to add resize frame animation turned off @{ */
        final AppWidgetResizeFrame finalResizeFrame = resizeFrame;
        if (ValueAnimator.getDurationScale() < 1.0f) {
            finalResizeFrame.post(new Runnable() {
                @Override
                public void run() {
                    finalResizeFrame.snapToWidget(false);
                }
            });
        } else {
    ...
    public void clearAnimatedView() {
        ...
        if (mDropView != null) {
            ...
            /* SPRD: bug325509 2014-06-20 immediate recycle bitmap when no need it.@{ */
            mDropView.clearBitmap();
            /* SPRD: bug325509 2014-06-20 immediate recycle bitmap when no need it. @} */
        }
        mDropView = null;
        invalidate();
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mInScrollArea && !LauncherAppState.getInstance().isScreenLarge()) {
            ...
            /* SPRD:add null-pointer check for bug 282198 @{ */
            try {
                getDescendantRectRelativeToSelf(workspace.getChildAt(0), childRect);
            } catch (Exception e) {
                android.util.Log.d("TAG","dispatchDraw-workspace.getChildAt(0) is null");
                e.printStackTrace();
            }
            /* @} */
    ...
    /* SPRD: bug281026 2014-02-21 delay to add resize frame animation turned off @{ */
    public void delayResizeFrame(AppWidgetResizeFrame resizeFrame){
        final AppWidgetResizeFrame finalResizeFrame = resizeFrame;
