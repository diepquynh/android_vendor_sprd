...
/**
 * Modified from Contact MultiShrinkScroll Handle the touch event and change header size and scroll
 */
public class FmScroller extends FrameLayout {
    ...
    private float mReceivedDownY = 0;
    ...
    /**
     * Interpolator from android.support.v4.view.ViewPager. Snappier and more elastic feeling than
    ...
    private boolean shouldStartDrag(MotionEvent event) {
        /*
         * SPRD : bug568587, new feature FM new UI @{ if (mIsBeingDragged) { mIsBeingDragged =
         * false; return false; }
         * @}
         */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        /* SPRD : bug568587, new feature FM new UI @{ */
        int minHeaderHeight = getMinHeight(STATE_HAS_FAVORITE);
        int halfHeaderHeight = getMinHeight(STATE_NO_FAVORITE);
        int fullHeaderHeight = getMaxHeight(STATE_NO_FAVORITE);
        /* @} */
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                /** SPRD : bug568587, new feature FM new UI @{ */
                // If dragdown is true,means header slide down.
                boolean dragdown = mLastEventPosition[1] - mReceivedDownY > 0;

                if (getHeaderHeight() == minHeaderHeight || getHeaderHeight() == halfHeaderHeight
                        || getHeaderHeight() == fullHeaderHeight) {
                    return true;
                }
                ...
                if (getResources().getConfiguration().orientation != Configuration.ORIENTATION_LANDSCAPE) {
                    if (mAdapter != null && mAdapter.getCount() < 7) {
                        if (dragdown) {
                            if (getHeaderHeight() >= halfHeaderHeight
                                    + (fullHeaderHeight - halfHeaderHeight) / 6) {
                                expandHeader();
                            } else {
                                collapseHeader();
                            }
                        } else {
                            if (getHeaderHeight() >= fullHeaderHeight
                                    - (fullHeaderHeight - halfHeaderHeight) / 6) {
                                expandHeader();
                            } else {
                                collapseHeader();
                        ...
                        if (dragdown) {
                            if (getHeaderHeight() < minHeaderHeight
                                    + (halfHeaderHeight - minHeaderHeight) / 4) {
                                collapseHeader();
                            } else if (getHeaderHeight() >= minHeaderHeight
                                    + (halfHeaderHeight - minHeaderHeight) / 4
                                    && getHeaderHeight() < halfHeaderHeight
                                            + (fullHeaderHeight - halfHeaderHeight) / 6) {
                                changeHeader(halfHeaderHeight);
                            } else if (getHeaderHeight() >= halfHeaderHeight
                                    + (fullHeaderHeight - halfHeaderHeight) / 6) {
                                expandHeader();
                            }
                        } else {
                            if (getHeaderHeight() > fullHeaderHeight
                                    - (fullHeaderHeight - halfHeaderHeight) / 6) {
                                expandHeader();
                            } else if (getHeaderHeight() <= fullHeaderHeight
                                    - (fullHeaderHeight - halfHeaderHeight) / 6
                                    && getHeaderHeight() > halfHeaderHeight
                                            - (halfHeaderHeight - minHeaderHeight) / 4) {
                                changeHeader(halfHeaderHeight);
    ...
    /** SPRD : bug568587, new feature FM new UI @{ */
    private void changeHeader(int headerHeight) {
        final ObjectAnimator animator = ObjectAnimator.ofInt(this, "headerHeight",
                headerHeight);
        animator.addListener(mHeaderExpandAnimationListener);
        animator.start();
    ...
    public void scrollTo(int x, int y) {
        ...
        int curOrientation = getResources().getConfiguration().orientation;
        boolean IsLandscape = curOrientation == Configuration.ORIENTATION_LANDSCAPE;
        if (IsLandscape) {
            if (delta > 0) {
                scrollUp(delta);
            } else {
            ...
            float posY = mLastEventPosition[1];
            if (posY < getHeaderHeight()) {
                ViewGroup.LayoutParams toolbarLayoutParams = mHeader
                        .getLayoutParams();
                if (toolbarLayoutParams.height >= getFullyCompressedHeaderHeight()) {
                    toolbarLayoutParams.height -= delta;
                    if (delta > 0) {
                        toolbarLayoutParams.height = Math.max(
                                toolbarLayoutParams.height,
                                getFullyCompressedHeaderHeight());
                    } else {
                        toolbarLayoutParams.height = Math.min(
                                toolbarLayoutParams.height, getHeight());
                    }
    ...
    /**
     * A variant of {@link #getScroll} that pretends the header is never larger than than
     * mIntermediateHeaderHeight. This function is sometimes needed when making scrolling decisions
     * that will not change the header size (ie, snapping to the bottom or top). When
     * mIsOpenContactSquare is true, this function considers mIntermediateHeaderHeight ==
     * mMaximumHeaderHeight, since snapping decisions will be made relative the full header size
     * when mIsOpenContactSquare = true. This value should never be used in conjunction with
     * {@link #getScroll} values.
     */
     ...
     * discontinuity between dragging speed and flinging speed. Similar to a
     * {@link android.view.animation.AccelerateInterpolator} in the sense that getInterpolation() is
                ...
                if (IsLandscape) {
                    ...
                    if (mAdapter != null && mAdapter.getCount() > 0) {
                        if (mAdapter.getCount() < 7) {
                            ...
                            height = (int) getResources()
                                    .getDimension(R.dimen.fm_main_header_small);
                        }
                    }
                }
                /* @} */
    ...
    public void onResume() {
        Cursor c = getData();
        // SPRD : bug606352 FMRadio crash when phone storage is full.
        if (c == null) return;
        mAdapter.swipResult(c);
        if (mFirstOnResume) {
    ...
    public void onRemoveFavorite() {
        ...
        if (c != null && c.getCount() == 0) {
            ...
            // Last time count is 1, so need set STATE_NO_FAVORITE then expand
            // header
            mMinimumHeaderHeight = getMinHeight(STATE_NO_FAVORITE);
            mMaximumHeaderHeight = getMaxHeight(STATE_NO_FAVORITE);
            expandHeader();
    ...
    public interface EventListener {
        ...
        /**
         * SPRD: bug496334, change station frequency. Callback when click gridview item to change
         * freqency
         * 
    ...
    private class Adjuster {
        ...
        // SPRD : bug568587, new feature FM new UI
        private TextView mCustomTimerText;

        /*
         * The five control buttons view(previous, next, increase, decrease, favorite) and stop
         * button
         */
        ...
        public Adjuster(Context context) {
            ...
            mCustomTimerText = (TextView) findViewById(R.id.custom_time);
            ...
            mStationNameText.addTextChangedListener(new TextWatcher() {
                @Override
        ...
        private class FirstRangeAdjuster {
            ...
            // SPRD : bug568587, new feature FM new UI
            protected float mCustomTimerTextSize;
            ...
            public FirstRangeAdjuster() {
                ...
                mPlayButtonPaddingRate = (mPlayButtonMarginTopStart - mPlayButtonMarginTopTarget)
                        / scrollHeight;
                // custom timer text size FM new UI
                mCustomTimerTextSize = res.getDimension(R.dimen.text_fm_size);
            }
            ...
            public void handleScroll() {
                ...
                mFrequencyText.setTextSize(TypedValue.COMPLEX_UNIT_PX, newTextSize);
                ...
                // 7.custom timer text (margin) FM new UI
                newMargin = getNewSize(currentHeight, mTargetHeight, mFmDescriptionMarginTopTarget,
                        mFmDescriptionPaddingRate);
                setNewPadding(mCustomTimerText, newMargin + mFmDescriptionText.getHeight());
            }
            ...
            private void handleScrollLandscapeMode() {
                ...
                float lastHeight = 0;
                float newTextSize;
                float oldTextSize = mFrequencyText.getTextSize();
                // 1. FM description (color, alpha and margin)
                newMargin = getNewSize(currentHeight, mTargetHeight, mFmDescriptionMarginTopTarget,
                        mFmDescriptionPaddingRate);
                setNewPadding(mFmDescriptionText, newMargin);

                newTextSize = getNewSize(currentHeight, mTargetHeight,
                        mFmDescriptionTextSizeTarget,
                        mFmDescriptionTextSizeRate);
                ...
                /**
                 * SPRD: bug509502 Change station value clickable area. Original Android code:
                 ...
                 * @{
                 */
                RelativeLayout.LayoutParams freqlp = (android.widget.RelativeLayout.LayoutParams) mFrequencyText
                        .getLayoutParams();
                freqlp.topMargin = (int) (newMargin);
                freqlp.leftMargin = (int) (addPadding + mFmDescriptionStartPaddingLeft);
                // SPRD: bug571992 in landscape mode,FM tag TextView overlap with FM station value
                // TextView
                freqlp.rightMargin = freqlp.leftMargin;
                ...
                lastHeight = setNewPadding(mStationNameText, newMargin + lastHeight);
                /*
                 * 4. station rds (margin), in landscape with favorite it need parallel to station
                 * name
                 */
                newMargin = getNewSize(currentHeight, mTargetHeight, mStationRdsMarginTopTarget,
                        mStationRdsPaddingRate);
                ...
                /**
                 * SPRD: bug509502 Change station value clickable area. Original Android code: if
                 * (currentHeight <= targetHeight) { String stationName = "" +
                 * mStationNameText.getText(); int stationNameTextWidth =
                 * mStationNameText.getPaddingLeft(); if (!stationName.equals("")) { Paint paint =
                 * mStationNameText.getPaint(); stationNameTextWidth += (int)
                 * paint.measureText(stationName) + 8; } mStationRdsText.setPadding((int)
                 * stationNameTextWidth, (int) (newMargin + lastHeight),
                 * mStationRdsText.getPaddingRight(), mStationRdsText.getPaddingBottom()); } else {
                 * mStationRdsText.setPadding((int) (16 * mDensity), (int) (newMargin + lastHeight),
                 * mStationRdsText.getPaddingRight(), mStationRdsText.getPaddingBottom()); }
                 * 
                 * @{
                 */
                RelativeLayout.LayoutParams rdslp = (android.widget.RelativeLayout.LayoutParams) mStationRdsText
                        .getLayoutParams();
                if (currentHeight <= targetHeight) {
                    ...
                    // SPRD: bug571992 in landscape mode,FM tag TextView overlap with FM station
                    // value TextView
                    rdslp.rightMargin = rdslp.leftMargin;
                    mStationRdsText.setLayoutParams(rdslp);
                } else {
                    rdslp.topMargin = (int) (newMargin + lastHeight);
                    // SPRD: bug561076 PS and RT data is not aligned.
                ...
                newMargin = getNewSize(currentHeight, mTargetHeight, mFrequencyMarginTopTarget,
                        mFrequencyPaddingRate);
                RelativeLayout.LayoutParams timerlp = (android.widget.RelativeLayout.LayoutParams) mCustomTimerText
                        .getLayoutParams();
                // timerlp.topMargin = (int) (newMargin + (mFmDescriptionText.getHeight() - mCustomTimerTextSize));
                double timerTopMargin = 0;
                if (oldTextSize < mFrequencyText.getTextSize()) {
                    timerTopMargin = (1 + 0.5) * mCustomTimerTextSize;
                } else {
                    timerTopMargin = (1 - 0.5) * mCustomTimerTextSize;
                }
                timerlp.topMargin = (int) (freqlp.topMargin + timerTopMargin);
                mCustomTimerText.setLayoutParams(timerlp);
            }
        ...
        private float setNewPadding(TextView current, float newMargin) {
             ...
             * @{
             */
            RelativeLayout.LayoutParams lp = (android.widget.RelativeLayout.LayoutParams) current
                    .getLayoutParams();
