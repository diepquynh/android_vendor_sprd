...
abstract class ImageViewTouchBase extends ImageView {
    ...
    // CID 109177 : UrF: Unread field (FB.URF_UNREAD_FIELD)
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        /*if(mBitmapDisplayed.getBitmap() != null && mIsFirstShow) {
            mIsFirstShow = false;
            showProperView(mThisWidth, mThisHeight);
        }*/
    }

    private void showProperView(float layoutW, float layoutH) {
        float bitmapW = mBitmapDisplayed.getBitmap().getWidth();
        float bitmapH = mBitmapDisplayed.getBitmap().getHeight();
        float scaleW = layoutW / bitmapW;
        float scaleH = layoutH / bitmapH;
        float scaleFactor = scaleH;
        if(scaleH > scaleW) {
            scaleFactor = scaleW;
        }
        if(scaleFactor < 1.0f) {
        ...
        zoomTo(scaleFactor, layoutW / 2, layoutH / 2, 500);
    }
    ...
    public void setImageBitmap(Bitmap bitmap, int rotation) {
        if(bitmap != null && bitmap.isRecycled()) {
            //listener the bug32822
            Log.d(TAG, "setImageBitmap(bm, int): (bitmap is recycled) = " + bitmap.isRecycled() + ", bitmap = " + bitmap);
        }
...
//        float scale = Math.min(Math.min(widthScale, heightScale),1);
        float scale = 1;
        matrix.postConcat(bitmap.getRotateMatrix());
        matrix.postScale(scale, scale);
        matrix.postTranslate((viewWidth - w * scale) / 2F, (viewHeight - h * scale) / 2F);

        scale = Math.min(Math.min(widthScale, heightScale), 3)/getScale();
        mSuppMatrix.postScale(scale, scale, viewWidth/2, viewHeight/2);
        /*
         * BUG FIX: 4553 4020
         * BUG COMMENT: msuppmatirx change
         * FIX DATE: 2013-08-07
         * */
        center(true, true);
    }
    ...
    protected Matrix getImageViewMatrix() {
        ...
        Bitmap bitmapDisplay = mBitmapDisplayed.getBitmap();
        if (mIsFullScreen && bitmapDisplay != null && bitmapDisplay.getHeight() < getHeight()) {
            float ratio = (float)getHeight()/(float)bitmapDisplay.getHeight();
            Matrix tempMatrix = new Matrix();
            float cx = getWidth() / 2F;
            float cy = getHeight() / 2F;
            tempMatrix.postScale(ratio, ratio, cx, cy);
            mDisplayMatrix.postConcat(tempMatrix);
        }
