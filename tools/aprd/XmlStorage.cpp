...
XmlStorage::XmlStorage(Observable *o, char* pdir, char* pfname):Observer(o)
{
	...
	strcpy(m_pathname_swp1, m_pathname);
	strcat(m_pathname_swp1, ".swp1");
	strcpy(m_pathname_swp2, m_pathname);
	strcat(m_pathname_swp2, ".swp2");
...
int XmlStorage::badfile_process(const char *filename)
{
	APR_LOGE("%s not load successfully.\n", filename);
	char * badfilename;
	badfilename = _findAndReturnBadFile();

	if (badfilename) {
		if(rename(filename,badfilename )) {
			APR_LOGE("we didn't rename %s to %s, so exit.\n", filename, badfilename);
//			property_set("persist.sys.apr.enabled", "0");
			APR_LOGE("APR Goodbye!!\n");
			exit(1);
			//exit_apr(1);
		}
		free(badfilename);
...
void XmlStorage::init()
{
	...
	if (_fileIsExist(REAL_FILE))
	{
		...
		if (NULL == m_doc)
	       	{
		    badfile_process(m_pathname);
		    m_doc = xmlParseFile(m_pathname_swp1);
		}
		if (NULL == m_doc)
		{
		   badfile_process(m_pathname_swp1);
		   m_doc = xmlParseFile(m_pathname_swp2);
		}
		if (NULL == m_doc)
		{
		   badfile_process(m_pathname_swp2);
		   goto next;
		}
...
next:
	...
	{
	    flag_copy = 0;
	    copy_to_xml(m_pathname_swp1, m_pathname);
	}
	else 
	{
	    flag_copy = 1;
	    copy_to_xml(m_pathname_swp2, m_pathname);
	}
	/*
...
void XmlStorage::copy_to_xml(const char * file_swp, const char * file)
{
#define BUFSIZE 512
    int fd1 = 0;
    int fd2 = 0;
    ...
    fd1 = open(file_swp, O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU);
    if (fd1 == -1)
	exit(1);
	//exit_apr(1);
    fd2 = open(file, O_RDONLY, S_IRWXU);
    if (fd2 == -1)
	exit(1);
	//exit_apr(1);

    while(bytes_read = read(fd2, buf, BUFSIZE))
    {
	if((bytes_read == -1) && (errno != EINTR))
	    break;
	else if(bytes_read > 0)
	{
	    bytes_write = write(fd1,buf, bytes_read);
	    if (bytes_write < 0)
		break;
	}
    }
...
int XmlStorage::createAprNode(xmlNodePtr *node)
{
	...
	m_CPVersionNode = xmlNewTextChild(aprNode, NULL, BAD_CAST "CPVersion", BAD_CAST v.c_str());
	//     <extraInfo> </extraInfo>
...
void XmlStorage::handleEvent(void* arg)
{
	...
	if (!strcmp(aprData->getCPVersion().c_str(), default_value))
	{
	    property_get("gsm.version.baseband", value, default_value);
	    aprData->setCPVersion(value);
	    xmlNodeSetContent(m_CPVersionNode, (const xmlChar*)value);
	    APR_LOGD("APR: CPVersion: %s", value);
	}
	...
	ret = xmlSaveFormatFileEnc(m_pathname, m_doc, MY_ENCODING, 1);//this func call fopen() which mode is "wb", so the file will be null before write new data to the file.[to the bug what pressing power key 7s and the apr.xml be null file]
	/*
	fp = fopen(m_pathname, "a+");
	if (fp == NULL)
	    exit(-1);
//	ret = xmlDocDump(fp, m_doc);
	xmlDocFormatDump(fp, m_doc, 1);
	fclose(fp);
	*/
...
int XmlStorage::_fileIsExist(EXISTS_FLAG flag)
{
	int retval;
	/* clear the umask */
	umask(0);
	...
	switch(flag)
	{
	    ...
	    case SWP1:
		if (access(m_pathname_swp1, F_OK) < 0) {
			return false;
		} else {
			return true;
		}
	    case SWP2:
		if (access(m_pathname_swp2, F_OK) < 0) {
			return false;
		} else {
			return true;
		}
