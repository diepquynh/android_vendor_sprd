/*
 *  ext_storage_monitor.cpp - kernel will send a netlink socket to notify the
 * sdcard's status.
 *
...
ExtStorageMonitor::ExtStorageMonitor(StorageManager* stor_mgr,
                                     LogController* ctrl, Multiplexer* multi)
...
int ExtStorageMonitor::init() {
  ...
  if ((m_fd = socket(PF_NETLINK, SOCK_RAW, NETLINK_KOBJECT_UEVENT)) < 0) {
...
void ExtStorageMonitor::subscribe_ext_storage_evt(
    void* client_ptr, ext_storage_evt_callback_t cb) {
...
void ExtStorageMonitor::notify_stor_evt(StorageEvent stor_evt,
                                        unsigned major_num,
                                        unsigned minor_num) {
  for (auto it = m_event_clients.begin(); it != m_event_clients.end(); ++it) {
    EventClient* client = *it;
    client->cb(client->client_ptr, stor_evt, major_num, minor_num);
  }
}
...
void ExtStorageMonitor::process(int /*events*/) {
  ...
  StorageEvent evt_action = EVT_NONE;
  ...
  if (evt_block_subsystem &&
    ...
    info_log("Block device:%u,%u %s event message.",
             ...
             (evt_action == EVT_REMOVE) ? "remove" : "add");

    notify_stor_evt(evt_action, evt_major_num, evt_minor_num);
  }
}
