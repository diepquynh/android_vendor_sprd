...
int gralloc_backend_register(private_handle_t* hnd)
{
	...
	case private_handle_t::PRIV_FLAGS_USES_ION:
		...
		{
			ALOGE("Could not get gralloc module for handle: %p", hnd);
			retval = -errno;
			break;
		}
		...
		if (m->ion_client <= 0)
		{
			...
			if (m->ion_client < 0) 
			{
				ALOGE( "Could not open ion device for handle: %p", hnd );
				retval = -errno;
				break;
			}
		}
		...
		{
			ALOGE( "mmap( share_fd:%d ) failed with %s",  hnd->share_fd, strerror( errno ) );
			retval = -errno;
			break;
		}
