...
int LogConfig::parse_cmd_line(int argc, char** argv) {
  ...
  while (i < argc) {
    if (!strcmp(argv[i], "-c")) {
      ++i;
      if (i < argc) {
        LogString config_file_type;
        str_assign(config_file_type, argv[i], strlen(argv[i]));
        LogString config_file_name = config_file_type + "_slog_modem.conf";
        info_log("%s mode - %s configuration file is used.",
                 ls2cstring(config_file_type), ls2cstring(config_file_name));

        LogString config_file_path;
        str_assign(config_file_path, TMP_CONFIG_PATH,
                   strlen(TMP_CONFIG_PATH));
        m_config_file = config_file_path + config_file_name;
      } else {
        err_log("CP logs configuration file is not specified.");
        ret = -1;
        break;
      }
    } else {
