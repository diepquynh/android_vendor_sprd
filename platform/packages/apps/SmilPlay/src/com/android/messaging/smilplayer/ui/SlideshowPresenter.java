...
import com.android.messaging.smilplayer.model.MediaModel;
import com.android.messaging.smilplayer.model.MediaModel.MediaAction;
import com.android.messaging.smilplayer.model.Model;
import com.android.messaging.smilplayer.model.RegionMediaModel;
import com.android.messaging.smilplayer.model.RegionModel;
import com.android.messaging.smilplayer.model.SlideModel;
import com.android.messaging.smilplayer.model.SlideshowModel;
import com.android.messaging.smilplayer.model.TextModel;
import com.android.messaging.smilplayer.model.VideoModel;
import com.android.messaging.smilplayer.model.VcalendarModel;
import com.android.messaging.smilplayer.model.VcardModel;
import com.android.messaging.smilplayer.util.ItemLoadedCallback;

//import com.android.mms.MmsApp;
import com.android.messaging.smilplayer.ui.AdaptableSlideViewInterface.OnSizeChangedListener;
...
public class SlideshowPresenter extends Presenter {
    ...
    protected void presentSlide(SlideViewInterface view, SlideModel model) {
        ...
        /* SPRD: Add for bug 304285 @{ */
        if(model == null) {
            Log.d(TAG,"presentSlide(): para model is null");
            return ;
        }
         /* @} */
        for (MediaModel media : model) {
            ...
            /* add for bug 447934
                ...
                presentVcalendar(view, (VcalendarModel) media, true);
            }*/
        }
    }
    ...
    protected void presentImage(SlideViewInterface view, ImageModel image,
        ...
        if (dataChanged) {
            if (image.isGifType() && (image.isGifEncode(/*mContext, image.getUri()*/)) && view instanceof SlideView) {
                ((SlideView) view).setGifImage(image.getSrc(), image.getUri());
            } else {
                if (view instanceof AdaptableSlideViewInterface) {
        ...
        if (image.isGifType() && view instanceof SlideView) {
            ((SlideView) view).setGifImageVisibility(image.isVisible());
        } else {
    ...
    protected void presentVcard(SlideViewInterface view, VcardModel vcard, boolean dataChanged) {
        if (dataChanged) {
            view.setImage(vcard.getSrc(), MmsApp.getApplication().getThumbnailManager()
                    .getVcardBitmap());
        }
    }
    protected void presentVcalendar(SlideViewInterface view, VcalendarModel vcalendar, boolean dataChanged) {
        if (dataChanged) {
            view.setImage(vcalendar.getSrc(), MmsApp.getApplication().getThumbnailManager()
                    .getVcalendarBitmap());
        }
    }*/
