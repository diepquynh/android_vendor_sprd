...
namespace android_audio_legacy
{
    ...
    AudioPolicyManagerSPRD::AudioPolicyManagerSPRD(AudioPolicyClientInterface
            *clientInterface)
            ...
            int session)
    {
        ...
        if((!is_voip_set) && (stream == AudioSystem::VOICE_CALL)) {
            for (size_t i = 0; i < mOutputs.size(); i++) {
                ...
                ALOGD("startOutput() outputDesc->mRefCount[AudioSystem::VOICE_CALL] %d",
                      outputDesc->mRefCount[AudioSystem::VOICE_CALL]);
            ...
            int session)
    {
        ...
        ALOGD("stopOutput() is_voip_set %d,stream %d,output size %d", is_voip_set,
              stream, mOutputs.size());
        if(is_voip_set && (stream == AudioSystem::VOICE_CALL)) {
            for (size_t i = 0; i < mOutputs.size(); i++) {
                AudioOutputDescriptor *outputDesc = mOutputs.valueAt(i);
                ALOGD("stopOutput() outputDesc->mRefCount[AudioSystem::VOICE_CALL] %d",
                      outputDesc->mRefCount[AudioSystem::VOICE_CALL]);
                if(outputDesc->mRefCount[AudioSystem::VOICE_CALL] == 1) {
                    AudioParameter param;
        ...
        if (desc->mFlags & AudioSystem::OUTPUT_FLAG_DIRECT) {
            ...
            if (--desc->mDirectOpenCount == 0) {
                ...
                if (dstOutput != mPrimaryOutput) {
                    mpClientInterface->moveEffects(AUDIO_SESSION_OUTPUT_MIX, mPrimaryOutput,
                                                   dstOutput);
                }
            }
        }
