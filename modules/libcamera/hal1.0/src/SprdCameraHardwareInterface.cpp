...
/* [Vendor_6/40] */
	...
	if(!mHalOem) {
	    ...
	    LOGV("loaded HAL libcamoem2v0 handle=%p", handle);
	}
...
int SprdCameraHardware::Callback_OtherMalloc(enum camera_mem_cb_type type, cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr)
{
	...
	*vir_addr = 0;
	...
	} else if (type == CAMERA_ISP_RAWAF) { /* [Vendor_7/40] */
		cmr_u64 kaddr = 0;
		cmr_u64* phy_addr_64 = (cmr_u64*)phy_addr;
		cmr_u64* vir_addr_64 = (cmr_u64*)vir_addr;
		size_t ksize = 0;
												...
												cmr_uint *vir_addr, void* private_data)
{
	...
	if (CAMERA_PREVIEW == type) {
		...
		CAMERA_ISP_BINGING4AWB == type || CAMERA_ISP_ANTI_FLICKER == type || CAMERA_ISP_RAWAE == type  || CAMERA_ISP_RAWAF == type ) {/* [Vendor_8/40] */
...
int SprdCameraHardware::Callback_Free(enum camera_mem_cb_type type, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_u32 sum, void* private_data)
{
	...
	if (CAMERA_PREVIEW == type) {
		...
		CAMERA_SNAPSHOT_ZSL_RESERVED_MEM == type || CAMERA_ISP_LSC == type ||
		CAMERA_ISP_BINGING4AWB == type || CAMERA_ISP_ANTI_FLICKER == type || CAMERA_ISP_RAWAE == type || CAMERA_ISP_RAWAF == type ) { /* [Vendor_10/40] */
		...
		if (mData_cb) {
			...
			if (1 == mCaptureNum) {
				if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE){
					...
					memcpy((void *)(mem->data + frame_size), (void*)(frame->uv_vir_addr), frame_size >> 1);
	...
	if (mData_cb) {
		...
		if (1 == mCaptureNum) {
			if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE){
				...
				if (isp_info_addr) {
					memcpy((void *)(mem->data+mJpegSize),isp_info_addr,isp_info_size);
				}
				mData_cb(CAMERA_MSG_COMPRESSED_IMAGE,mem, 0, NULL, mUser );
				mem->release(mem);
			}
