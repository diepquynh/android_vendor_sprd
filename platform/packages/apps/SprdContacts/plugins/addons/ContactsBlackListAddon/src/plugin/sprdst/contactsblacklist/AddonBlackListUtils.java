    ...
    public static class BlackColumns {
        ...
        public static final class BlackMumber implements BaseColumns {
            public static final Uri CONTENT_URI = Uri
                    .parse("content://com.android.blockednumber/blocked");
            
//            public static final Uri CONTENT_URI = BlockedNumberContract.BlockedNumbers.CONTENT_URI;
            
            public static final String BLOCK_TYPE = "call_type";
...
//      filter-able? (has any phone number)
    ...
    public boolean deleteFromBlockList(Context context, String phoneNumber) {
        ContentResolver cr = context.getContentResolver();
        String[] columns = new String[] {
                BlackColumns.BlackMumber._ID, BlockedNumbers.COLUMN_ORIGINAL_NUMBER,
        };
        ...
        try {
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    original_number = cursor.getString(cursor.getColumnIndex(
                            BlockedNumbers.COLUMN_ORIGINAL_NUMBER));

                    if (PhoneNumberUtils.compare(phoneNumber.trim(), original_number.trim())) {

                        result = cr.delete(BlackColumns.BlackMumber.CONTENT_URI,
                                BlockedNumbers.COLUMN_ORIGINAL_NUMBER + "='" + original_number + "'",
                                null);
                        break;
                    }
                } while (cursor.moveToNext());
    ...
    public boolean putToBlockList(Context context, String phoneNumber, int Blocktype,
        ...
        if (values != null) {
            try {
                ...
                values.put(BlackColumns.MIN_MATCH,
                        PhoneNumberUtils.toCallerIDMinMatch(normalizedNumber));
                values.put(BlockedNumbers.BLOCK_TYPE, Blocktype);
                values.put(BlockedNumbers.NAME, name);
            } catch (Exception e) {
