...
static uint32_t s_ov5670_sensor_close_flag = 0;
...
static uint32_t _ov5670_set_sensor_close_flag()
{
	uint32_t rtn = SENSOR_SUCCESS;

	s_ov5670_sensor_close_flag = 1;

	return rtn;
}

LOCAL cmr_uint ov5670_access_val(cmr_uint param)
{
	cmr_uint  rtn = SENSOR_SUCCESS;
	SENSOR_VAL_T* param_ptr = (SENSOR_VAL_T*)param;
	uint16_t tmp;

	SENSOR_PRINT("SENSOR_ov5670: ov5670_access_val E");
	if(!param_ptr){
		return rtn;
	}
	...
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_SHUTTER:
			*((uint32_t*)param_ptr->pval) = ov5670_read_shutter( );
			break;
		case SENSOR_VAL_TYPE_READ_OTP_GAIN:
			*((uint32_t*)param_ptr->pval) = ov5670_read_gain( );
			break;
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			rtn = _ov5670_set_sensor_close_flag();
			break;
		default:
			break;
	}

	SENSOR_PRINT("SENSOR_ov5670: ov5670_access_val X");
