    ...
    protected static class MailFactory
        ...
        public RemoteViews getViewAt(int position) {
            synchronized (sWidgetLock) {
                ...
                // On click intent.
                /* SPRD: Modify for bug582197 && 598427 {@ */
                if (mFolderType != FolderType.DRAFT) {
                    ...
                    try {
                        Cursor mCursor = mContext.getContentResolver().query(
                                conversation.messageListUri, UIProvider.MESSAGE_PROJECTION, null, null, null);
                        MessageCursor messageCursor = new MessageCursor(
                                new ObjectCursor<ConversationMessage>(mCursor, ConversationMessage.FACTORY));
                        messageCursor.moveToFirst();
                        ConversationMessage message = messageCursor.getMessage();

                        final Intent composeIntent = new Intent(ComposeActivity.ACTION_LAUNCH_COMPOSE);
                        ...
                        remoteViews.setOnClickFillInIntent(R.id.widget_conversation_list_item, composeIntent);
                    } catch (CursorIndexOutOfBoundsException e) {
                        LogUtils.e(LOG_TAG,"appear CursorIndexOutOfBoundsException when loading draft" + e.toString());
                    } catch (SecurityException e) {
                        LogUtils.e(LOG_TAG,"appear SecurityException when loading draft" + e.toString());
                    }
                }
                /* @} */
                return remoteViews;
