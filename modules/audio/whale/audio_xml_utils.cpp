...
TiXmlElement *private_get_param(param_group_t group, const char *param_name)
{
    ...
    if(param_name_size>sizeof(path_name)){
        LOG_E("private_get_param param_name size:0x%x error",param_name_size);
    }
    memcpy(path_name,param_name,param_name_size);
    if((path_name[param_name_size-1])==0x0d){
        path_name[param_name_size-1]=0;
    }
    char *name = strdup(path_name);
