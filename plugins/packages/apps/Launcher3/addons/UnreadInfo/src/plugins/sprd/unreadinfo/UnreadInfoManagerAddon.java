...
public class UnreadInfoManagerAddon extends SprdUnreadInfoManager implements
        AddonManager.InitialCallback, Runnable {
    ...
    private static final ComponentName sMmsComponentName = new ComponentName(
            "com.android.messaging",
            "com.android.messaging.ui.conversationlist.ConversationListActivity");
    private static final ComponentName sDialerComponentName = new ComponentName(
            "com.android.dialer", "com.android.dialer.DialtactsActivity");
    private static final ComponentName sEmailComponentName = new ComponentName(
    ...
    private static final String mGmsCalendarPackageName = "com.google.android.calendar";

    static {
        sURIMatcher.addURI("call_log", "calls", MATCH_CALL);
        sURIMatcher.addURI("mms-sms", null, MATCH_MMSSMS);
        sURIMatcher.addURI("com.android.email.notifier", "message/*",
                MATCH_EMAIL_MESSAGE_NOTIFIER);
        sURIMatcher.addURI("com.android.email.notifier", "message/*/*",
                MATCH_EMAIL_MESSAGE_NOTIFIER);
        sURIMatcher.addURI("com.android.email.notifier", "account/*/*",
                MATCH_EMAIL_ACCOUNT_NOTIFIER);
    ...
    private static final Uri EMAILS_CONTENT_URI = Uri
            .parse("content://com.android.email.provider/mailbox");
    private static final Uri EMAILS_NOTIFY_URI = Uri
            .parse("content://com.android.email.notifier");
    private static final Uri CALENDARS_CONTENT_URI = CalendarAlerts.CONTENT_URI;

    private static final String MISSED_CALLS_SELECTION = CallLog.Calls.TYPE
            + " = " + CallLog.Calls.MISSED_TYPE + " AND " + CallLog.Calls.NEW
            + " = 1";
    ...
    public void init(LauncherAppState appState) {
        ...
        mLargeTextSize = mContext.getResources().getDimension(
                R.dimen.unread_info_large_text_size);
        mMiddleTextSize = mContext.getResources().getDimension(
                R.dimen.unread_info_middle_text_size);
        mSmallTextSize = mContext.getResources().getDimension(
        ...
        for (PackageInfo packageInfo : list) {
            ...
            if (mGmsCalendarPackageName.equals(packageName)) {
                sCalendarComponentName = new ComponentName(
                        mGmsCalendarPackageName,
                        "com.android.calendar.AllInOneActivity");
                Log.d(TAG, "SprdUnreadInfoManager: sCalendarComponentName = "
                        + sCalendarComponentName);
            }
        }
    ...
    class IconCacheThread extends Thread {
        ...
        public void run() {
            ...
            mUnreadInfoCache.put(sMmsComponentName, new UnreadInfo(-1,
                    originIcon, null));

            intent.setComponent(sDialerComponentName);
            originIcon = mIconCache.getIcon(intent, mUser);
            mUnreadInfoCache.put(sDialerComponentName, new UnreadInfo(-1,
                    originIcon, null));

            intent.setComponent(sEmailComponentName);
            originIcon = mIconCache.getIcon(intent, mUser);
            mUnreadInfoCache.put(sEmailComponentName, new UnreadInfo(0,
                    originIcon, null));

            //intent.setComponent(sCalendarComponentName);
            //originIcon = mIconCache.getIcon(intent, mUser);
            //mUnreadInfoCache.put(sCalendarComponentName, new UnreadInfo(0,
                    //originIcon, null));
        }
    }
    ...
    public void registerContentObservers() {
        ...
        resolver.registerContentObserver(EMAILS_NOTIFY_URI, true, this);
        resolver.registerContentObserver(CALENDARS_CONTENT_URI, true, this);
    }
    ...
    public void updateUnreadInfoIfNeeded() {
        Log.d(TAG, "updateUnreadInfoIfNeeded mUnreadInfoChangedCache.size() = "
                + mUnreadInfoChangedCache.size());
        if (mUnreadInfoChangedCache.size() > 0 && mLauncherRef != null
                && mLauncherRef.get() != null) {
            mLauncherRef.get().updateUnreadInfo();
        }
    }

    /**
     * Note: this method is called by Working thread in LauncherModel before
     * load and bind any icon into Workspace or AppsCustomizePagedView.
     * 
     * Prepare the unread data and bitmap and then apply these bitmap when new
     * BubbleTextView need to be created.
     * 
    ...
    public void prepareUnreadInfo() {
        if (mUpdateUnreadInfoTheFirstTime) {
            ...
            for (ComponentName cn : mUnreadInfoCache.keySet()) {
                asyncGetUnreadInfoAndTriggerUpdate(cn, true);
            }
        }
    }
    ...
    public void onChange(boolean selfChange, Uri uri) {
        ...
        if (DEBUG) {
            Log.d(TAG,
                    String.format("onChange: uri=%s selfChange=%b",
                            uri.toString(), selfChange));
        }
        int match = sURIMatcher.match(uri);
        
        Log.d(TAG,
                "addon Launcher.getPermissionFlag() = "
                        + Launcher.getPermissionFlag());
        switch (match) {
            case MATCH_CALL:
                if (Launcher.getPermissionFlag()) {
                    asyncGetUnreadInfoAndTriggerUpdate(sDialerComponentName, false);
                }
                break;
            case MATCH_MMSSMS:
                if (Launcher.getPermissionFlag()) {
                    asyncGetUnreadInfoAndTriggerUpdate(sMmsComponentName, false);
                }
                break;
            case MATCH_EMAIL_MESSAGE_NOTIFIER:
            case MATCH_EMAIL_ACCOUNT_NOTIFIER:
                asyncGetUnreadInfoAndTriggerUpdate(sEmailComponentName, false);
                break;
            case MATCH_CALENDAR:
                if (Launcher.getPermissionFlag()) {
                    asyncGetUnreadInfoAndTriggerUpdate(sCalendarComponentName, false);
                }
                break;
        }
    }
    ...
    public boolean updateAppinfoUnreadInfo(AppInfo info) {
        ...
        FastBitmapDrawable topDrawable = mLauncherRef.get().createIconDrawable(
                b);
        bubble.setTag(info);
            ...
            if (tag instanceof ShortcutInfo) {
                ...
                if (mUnreadInfoCache.containsKey(cn)) {
                    ...
                    if (mLauncherRef != null && mLauncherRef.get() != null) {
                        FastBitmapDrawable iconDrawable = mLauncherRef.get()
                                .createIconDrawable(b);
//                        iconDrawable.setGhostModeEnabled(info.isDisabled != 0);
                ...
                if (mUnreadInfoCache.containsKey(cn)) {
                    Bitmap b = getBitmapWithUnreadInfo(info, null);
                    if (mLauncherRef != null && mLauncherRef.get() != null) {
                        FastBitmapDrawable topDrawable = mLauncherRef.get()
                                .createIconDrawable(b);
                        bubble.setCompoundDrawables(null, topDrawable, null, null);
                    }
                }
    ...
    private Bitmap getBitmapWithUnreadInfoInternal(ComponentName cn,
            Bitmap origin) {
        if (mUnreadInfoCache.containsKey(cn)) {
    ...
    private void asyncGetUnreadInfoAndTriggerUpdate(final ComponentName cn, boolean isPrepare) {
    ...
    private int loadUnreadInfoCount(ComponentName component) {
        ...
        } else if (component.equals(sEmailComponentName)) {
            return getUnreadEmailCount();
        }
        // else if (component.equals(sCalendarComponentName)) {
    ...
    private int getUnreadMessageCount() {
        ...
        cursor = resolver.query(SMS_CONTENT_URI,
                new String[] { BaseColumns._ID }, "type = 1 AND read = 0",
                null, null);
        if (cursor != null) {
            unreadSms = cursor.getCount();
            cursor.close();
            if (DEBUG) {
                Log.i(TAG, String.format("getUnreadMessageCount unreadSms=%d",
                        unreadSms));
            }
        }
        ...
        if (cursor != null) {
            ...
            cursor.close();
            if (DEBUG) {
                Log.i(TAG, String.format("getUnreadMessageCount unreadMms=%d",
                        unreadMms));
            }
        }
    ...
    private int getMissedCallCount() {
        ...
        cursor = resolver.query(CALLS_CONTENT_URI,
                new String[] { BaseColumns._ID }, MISSED_CALLS_SELECTION, null,
                null);
        if (cursor != null) {
            missedCalls = cursor.getCount();
            cursor.close();
            if (DEBUG) {
                Log.i(TAG, String.format("getMissedCallCount missedCalls=%d",
                        missedCalls));
            }
        }
        return missedCalls;
    ...
    private int getUnreadEmailCount() {
        ...
        cursor = resolver.query(EMAILS_CONTENT_URI,
                new String[] { "unreadCount" }, "type = ?",
                new String[] { "0" }, null);
        if (cursor != null) {
            try {
                ...
                Log.d(TAG,
                        "getUnreadEmailCount: updateUnreadEmailCount Exception");
            } finally {
                cursor.close();
            }
        }

        Log.d(TAG, "getUnreadEmailCount: unreadEms = " + unreadEms);
        return unreadEms;
    }
    ...
    private int getUnreadCalendarCount() {
        ...
        Cursor alertCursor = resolver
                .query(CALENDARS_CONTENT_URI,
                        ...
                        "begin DESC, end DESC");
        if (alertCursor != null) {
            unreadeEvents = alertCursor.getCount();
            alertCursor.close();
        }

        Log.d(TAG, "getUnreadCalendarCount: unreadeEvents = " + unreadeEvents);
        return unreadeEvents;
    }
        ...
        if (mUnreadInfoCache.containsKey(cn)) {
            ...
            if (!(info.mUnreadInfoNum > MAX_UNREAD_COUNT && unreadNum > MAX_UNREAD_COUNT)) {
                if (unreadNum == 0) {
                    ...
                    if (info.mOriginIcon != null) {
                        info.mIconWithNum = createBitmapWithUnreadInfo(
                                info.mOriginIcon, unreadNum);
                    }
                }
    ...
    private Bitmap createBitmapWithUnreadInfo(Bitmap origin, int unreadNum) {
        ...
        // center
        // the number in the background circle. May be it is the red circle not
        // in
        // then center of the background resource
        // 'R.drawable.unread_info_background'
        // cause this problem.
        int textWidth = (int) paint.measureText(finalText, 0,
                finalText.length());
        ...
        // Because the red circle's height in the background bitmap occupied
        // 71%.
        ...
        // pixel
        // between the top of the red circle and the top of the background
        // bitmap.
        // With xhdpi resource bitmap, it is 2 pixel, but we can not detect
        // which
        // resource be used easily, so we always plus 1 pixel when calculate the
        // y position to make the number be drawn in the center of the red
        // circle.
        //
    ...
    public void run() {
        ...
        intent.setComponent(sMmsComponentName);
        Bitmap originIcon = mIconCache.getIcon(intent, mUser);
        mUnreadInfoCache.put(sMmsComponentName, new UnreadInfo(-1, originIcon,
                null));
        ...
        intent.setComponent(sEmailComponentName);
        originIcon = mIconCache.getIcon(intent, mUser);
        mUnreadInfoCache.put(sEmailComponentName, new UnreadInfo(0, originIcon,
                null));
    ...
    public void updateUnreadInfo(Workspace wp) {
        ArrayList<ShortcutAndWidgetContainer> childrenLayouts = wp
                ...
                if (childV instanceof FolderIcon) {
                    ...
                    for (View bubble : bubbles) {
                        if (bubble instanceof BubbleTextView) {
                             ...
                             * (LauncherAppState.getInstance().getUnreadInfoManager
                             * ()
                             * .updateBubbleTextViewUnreadInfo((BubbleTextView)
                             * bubble)) {
                             */
    ...
    public void updateUnreadInfo(Launcher mLauncher,
