...
extern int brcm_bt_bqb_init(void);
extern void brcm_bt_lpm_wake_up(void);
extern void brcm_bt_power_ctrl(unsigned char on);
...
int eng_controller_bqb_start(void) {
 ...
 //  upio_set_bluetooth_power(BT_VND_PWR_OFF);
  // upio_set_bluetooth_power(BT_VND_PWR_ON);
  brcm_bt_power_ctrl(BT_VND_PWR_OFF);
  brcm_bt_power_ctrl(BT_VND_PWR_ON);
  bt_fd = brcm_bt_bqb_init();

  ret = pthread_create(&ntid_bqb, NULL, (void *)eng_receive_data_thread,
                       NULL); /*create thread*/
  if (ret == -1) {
