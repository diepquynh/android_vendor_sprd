...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
// add the 2351 macro to support the mutil wcn george wang
#ifdef SPRD_WCNBT_SR2351 //for 2351
int fmOpen( void )
{
	if( NULL != s_hwDev ) {
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
//------add the interface of google orgianal get rssi function -----------------
int Native_GetRssi(uint *fm_freq,int *fm_rssi)
{
    int ret = -1;
    struct fm_check_status cur_freq;
    memset(&cur_freq, 0, sizeof(fm_check_status));
    cur_freq.freq = *fm_freq;
    Radio_Tune(cur_freq.freq);

    ret = Radio_GetRssi(&cur_freq.rssi);
    if(!ret)
    {
	LOGI("seek channels success: rssi:%d\n", cur_freq.rssi);
	*fm_rssi = cur_freq.rssi;
	ret = 0;
    }else{
	ret = -1;
