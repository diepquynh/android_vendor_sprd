...
public class DragController {
    ...
    public void startDrag(View v, DragSource source, Object dragInfo, int dragAction,
        ...
        startDrag(b, dragLayerX, dragLayerY, source, dragInfo, dragAction, null, dragRegion, 1);
        b.recycle();
    ...
    /* SPRD: Fix bug282801, return is the folder in Dragging process. @{ */
    public boolean isFolderIconDragging(FolderInfo info, Context context) {
        if (!mDragging) {
            return false;
        }
        if (mDragObject != null) {
            Object rawDragInfo = mDragObject.dragInfo;
            if (rawDragInfo instanceof FolderInfo) {
                FolderInfo dragInfo = (FolderInfo) rawDragInfo;
                if (dragInfo.id == info.id) {
                    return true;
                }
            }
        }
    ...
    public void onPackagesRemoved(ArrayList<String> packageNames, Context context) {
        // Cancel the current drag if we are removing an app by packageName that we are dragging
        if (mDragObject != null) {
            Object rawDragInfo = mDragObject.dragInfo;
            if (rawDragInfo instanceof ShortcutInfo) {
                ShortcutInfo dragInfo = (ShortcutInfo) rawDragInfo;
                for (String pkgName : packageNames) {
                    ...
                    if (dragInfo != null
                        && pkgName != null
                        && dragInfo.intent != null
                        && dragInfo.intent.getComponent() != null) {
                        boolean isSamePackage = pkgName.equals(dragInfo.intent.getComponent().getPackageName());
                        if (isSamePackage) {
                            cancelDrag();
                            return;
                        }
        ...
        if (mDragging) {
            ...
            if (mDragObject.dragView != null) {
                ...
                if (!isDeferred) {
                    ...
                    /* SPRD: bug329076 2014-07-03 immediate recycle bitmap when no need it. @{ */
                    mDragObject.dragView.clearBitmap();
                    /* SPRD: bug329076 2014-07-03 immediate recycle bitmap when no need it. @} */
                }
