/*
 * Copyright (C) 2014 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.android.fmradio;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

import android.app.ActionBar;
import android.app.Activity;
import android.app.AlertDialog;
import android.app.Dialog;
import android.content.ComponentName;
import android.content.ContentValues;
import android.content.Context;
import android.content.DialogInterface;
import android.content.Intent;
import android.content.ServiceConnection;
import android.database.Cursor;
import android.graphics.Color;
import android.graphics.PorterDuff;
import android.graphics.drawable.ColorDrawable;
import android.location.Location;
import android.location.LocationManager;
import android.media.AudioManager;
import android.os.Bundle;
import android.os.Handler;
import android.os.IBinder;
import android.os.Looper;
import android.os.Message;
import android.text.SpannableString;
import android.text.TextUtils;
import android.text.style.ForegroundColorSpan;
import android.util.Log;
import android.view.ActionMode;
import android.view.Gravity;
import android.view.KeyEvent;
import android.view.LayoutInflater;
import android.view.Menu;
import android.view.MenuInflater;
import android.view.MenuItem;
import android.view.MotionEvent;
import android.view.View;
import android.view.View.OnClickListener;
import android.view.View.OnTouchListener;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.view.Window;
import android.widget.AdapterView;
import android.widget.AdapterView.OnItemLongClickListener;
import android.widget.BaseAdapter;
import android.widget.CheckBox;
import android.widget.CompoundButton;
import android.widget.CompoundButton.OnCheckedChangeListener;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ListView;
import android.widget.PopupWindow;
import android.widget.ProgressBar;
import android.widget.RelativeLayout;
import android.widget.TextView;
import android.widget.Toast;

import com.android.fmradio.FmConstants.StepType;
import com.android.fmradio.FmService.OnExitListener;
import com.android.fmradio.FmStation.Station;
import com.android.fmradio.views.FmVisualizerView;

/**
 * SPRD:
 *
 * @{
 */
import android.Manifest;
import android.content.pm.PackageManager;

/**
 * @}
 */

/**
 * This class interact with user, provider edit station information, such as add to favorite, edit
 * favorite, delete from favorite
 */
public class FmFavoriteActivity extends Activity {
    // Logging
    private static final String TAG = "FmFavoriteActivity";

    public static final String ACTIVITY_RESULT = "ACTIVITY_RESULT";

    private static final String SHOW_GPS_DIALOG = "SHOW_GPS_DIALOG";

    private static final String GPS_NOT_LOCATED_DIALOG = "GPS_NOT_LOCATED_DIALOG";

    private ListView mLvFavorites = null; // list view

    LinearLayout mSearchTips = null;

    private Context mContext = null; // application context

    private OnExitListener mExitListener = null;

    private MyFavoriteAdapter mMyAdapter;

    private ProgressBar mSearchProgress = null;

    private MenuItem mMenuRefresh = null;

    /**
     * SPRD: bug474750, scan from current freq.
     * 
     * @{
     */
    private MenuItem mMenuItemScanFromCurrent = null;
    /**
     * @}
     */
    private LocationManager mLocationManager;

    private Location mCurLocation;

    private boolean mIsActivityForeground = true;

    /**
     * SPRD: bug495859, request runtime permissions
     */
    private boolean mHasCriticalPermissions = false;
    /**
     * @}
     */

    /**
     * on create
     * 
     * @param savedInstanceState The save instance state
     */
    /* SPRD : bug568587, new feature FM new UI @{ */
    private int mCurrentStation = -1;
    private List<StationItem> mFavoriteList = new ArrayList<StationItem>();
    private List<StationItem> mOtherList = new ArrayList<StationItem>();
    private ActionMode mActionMode = null;
    private MenuItem mMenuItemFavoriteOn;
    private MenuItem mMenuItemFavoriteOff;
    private MenuItem mMenuItemDelete;
    private CheckBox mCheckBoxSelectAll = null;
    // manage checkboxs status
    private List<Boolean> mCheckboxStatus = new ArrayList<Boolean>();

    /* @} */

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setVolumeControlStream(AudioManager.STREAM_MUSIC);
        getWindow().requestFeature(Window.FEATURE_ACTION_BAR);
        setContentView(R.layout.favorite);
        // display action bar and navigation button
        ActionBar actionBar = getActionBar();
        actionBar.setTitle(getString(R.string.station_title));
        actionBar.setDisplayHomeAsUpEnabled(true);
        mLocationManager = (LocationManager) getSystemService(Context.LOCATION_SERVICE);
        mContext = getApplicationContext();

        mMyAdapter = new MyFavoriteAdapter(mContext);
        mLvFavorites = (ListView) findViewById(R.id.station_list);
        mSearchTips = (LinearLayout) findViewById(R.id.search_tips);
        mSearchProgress = (ProgressBar) findViewById(R.id.search_progress);
        mLvFavorites.setAdapter(mMyAdapter); // set adapter
        /* SPRD : bug568587, new feature FM new UI @{ */
        mCheckBoxSelectAll = (CheckBox) findViewById(R.id.ck_selectAll);
        Cursor c = getData();
        mMyAdapter.swipResult(c);
        query(c);
        /* @} */
        mLvFavorites.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            /**
             * Click list item will finish activity and pass value to other activity
             * 
             * @param parent adapter view
             * @param view item view
             * @param position current position
             * @param id current id
             */
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                /* SPRD : bug568587, new feature FM new UI @{ */
                if (mActionMode == null) {
                    // Set the selected frequency to main UI and finish the
                    // favorite manager.
                    TextView textView = (TextView) view.findViewById(R.id.lv_station_freq);
                    float frequency = 0;
                    try {
                        frequency = Float.parseFloat(textView.getText().toString());
                    } catch (NumberFormatException e) {
                        e.printStackTrace();
                    }
                    Intent intentResult = new Intent();
                    intentResult.putExtra(ACTIVITY_RESULT, FmUtils.computeStation(frequency));
                    setResult(RESULT_OK, intentResult);
                    finish();
                } else {
                    CheckBox checkBox = (CheckBox) view.findViewById(R.id.ck_select);
                    if (null == checkBox) {
                        return;
                    }
                    if (true == checkBox.isChecked()) {
                        checkBox.setChecked(false);
                        mCheckBoxSelectAll.setChecked(false);
                    } else {
                        checkBox.setChecked(true);
                    }
                    mCheckboxStatus.set(position, checkBox.isChecked());
                    if (getCheckBoxSelectSize() == 0) {
                        updateActionModeItem(false);
                    } else {
                        updateActionModeItem(true);
                        if (getCheckBoxSelectSize() == mCheckboxStatus.size()) {
                            mCheckBoxSelectAll.setChecked(true);
                        }
                    }
                    mActionMode.setTitle(String.valueOf(getCheckBoxSelectSize()));
                }
            }
            /* @} */
        });
        // SPRD : bug568587, new feature FM new UI
        mLvFavorites.setOnItemLongClickListener(new StationListOnItemLongClickListener());
        // Finish favorite when exit FM
        mExitListener = new FmService.OnExitListener() {
            @Override
            public void onExit() {
                Handler mainHandler = new Handler(Looper.getMainLooper());
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        FmFavoriteActivity.this.finish();
                    }
                });
            }
        };
        FmService.registerExitListener(mExitListener);
        if (!mIsServiceBinded) {
            bindService();
        }
        // SPRD : bug568587, new feature FM new UI
        mCurrentStation = getIntent().getIntExtra(FmFavoriteActivity.ACTIVITY_RESULT,
                mCurrentStation);
    }

    /**
     * When menu is selected
     * 
     * @param item The selected menu item
     * @return true to consume it, false to can handle other
     */
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                onBackPressed();
                break;
            case R.id.fm_station_list_refresh:
                /**
                 * SPRD: bug495859, request runtime permissions
                 */
                if (!FmUtils.hasLocationPermission(mContext)) {
                    mOptionSelectedId = R.id.fm_station_list_refresh;
                    String[] permissionsToRequest = new String[2];
                    permissionsToRequest[0] = Manifest.permission.ACCESS_COARSE_LOCATION;
                    permissionsToRequest[1] = Manifest.permission.ACCESS_FINE_LOCATION;
                    requestPermissions(permissionsToRequest, FmUtils.FM_PERMISSIONS_REQUEST_CODE);
                    return true;
                }
                /**
                 * @}
                 */
                if (null != mService) {
                    refreshMenuItem(false);

                    mMyAdapter.swipResult(null);
                    /**
                     * SPRD: bug511231 Both emptyView and mSearchTips displayed when no station
                     * searched.
                     * 
                     * @{
                     */
                    View emptyView = (View) findViewById(R.id.empty_tips);
                    emptyView.setVisibility(View.GONE);
                    /**
                     * @}
                     */
                    mLvFavorites.setEmptyView(mSearchTips);
                    mSearchProgress.setIndeterminate(true);

                    // If current location and last location exceed defined distance, delete the RDS
                    // database
                    if (isGpsOpen()) {
                        mCurLocation = mLocationManager
                                .getLastKnownLocation(LocationManager.GPS_PROVIDER);
                        if (mCurLocation != null) {
                            double[] lastLocations = FmUtils.getLastSearchedLocation(mContext);
                            float distance[] = new float[2];
                            Location.distanceBetween(lastLocations[0], lastLocations[1],
                                    mCurLocation.getLatitude(), mCurLocation.getLongitude(),
                                    distance);
                            float searchedDistance = distance[0];
                            boolean exceed =
                                    searchedDistance > FmUtils.LOCATION_DISTANCE_EXCEED;
                            mService.setDistanceExceed(exceed);
                            FmUtils.setLastSearchedLocation(mContext, mCurLocation.getLatitude(),
                                    mCurLocation.getLongitude());
                        }
                    }

                    mService.startScanAsync();
                }
                break;
            /**
             * SPRD: bug474750, scan from current freq. bug511231
             * 
             * @{
             */
            case R.id.scan_from_current:
                if (!FmUtils.hasLocationPermission(mContext)) {
                    mOptionSelectedId = R.id.scan_from_current;
                    String[] permissionsToRequest = new String[2];
                    permissionsToRequest[0] = Manifest.permission.ACCESS_COARSE_LOCATION;
                    permissionsToRequest[1] = Manifest.permission.ACCESS_FINE_LOCATION;
                    requestPermissions(permissionsToRequest, FmUtils.FM_PERMISSIONS_REQUEST_CODE);
                    return true;
                }
                onOptionsItemSelectedScan(true /* fromCurrent */);
                break;
            /**
             * @}
             */
            default:
                break;
        }
        return super.onOptionsItemSelected(item);
    }

    /**
     * SPRD: bug519384 Fix IllegalStateException in monkey test.
     * 
     * @{
     */
    @Override
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (!this.isResumed()) {
            return true;
        }
        return super.dispatchKeyEvent(event);
    }

    /**
     * @}
     */

    @Override
    public void onBackPressed() {
        if (null == mService) {
            Log.w(TAG, "onBackPressed, mService is null");
        } else {
            boolean isScanning = mService.isScanning();
            if (isScanning) {
                mService.stopScan();
            }
        }
        super.onBackPressed();
    }

    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        MenuInflater inflater = getMenuInflater();
        inflater.inflate(R.menu.fm_station_list_menu, menu);
        mMenuRefresh = menu.findItem(R.id.fm_station_list_refresh);
        /**
         * SPRD: bug474750, scan from current freq.
         * 
         * @{
         */
        mMenuItemScanFromCurrent = menu.findItem(R.id.scan_from_current);
        /**
         * @}
         */
        resetMenuTitleColor(true);
        return true;
    }

    private void resetMenuTitleColor(boolean enabled) {
        SpannableString ss = new SpannableString(mMenuRefresh.getTitle());
        int titleColor = enabled ? getResources().getColor(R.color.actionbar_overflow_title_color)
                : getResources().getColor(R.color.actionbar_overflow_title_disabled_color);
        ss.setSpan(new ForegroundColorSpan(titleColor), 0, ss.length(), 0);
        mMenuRefresh.setTitle(ss);
        /**
         * SPRD: bug474750, scan from current freq.
         * 
         * @{
         */
        SpannableString sfc = new SpannableString(mMenuItemScanFromCurrent.getTitle());
        sfc.setSpan(new ForegroundColorSpan(titleColor), 0, sfc.length(), 0);
        mMenuItemScanFromCurrent.setTitle(sfc);
        /**
         * @}
         */
    }

    @Override
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (null != mService) {
            boolean isScan = mService.isScanning();
            refreshMenuItem(!isScan);
        }
        return super.onPrepareOptionsMenu(menu);
    }

    static final class ViewHolder {
        TextView mFavoruteTag;
        ImageView mStationTypeView;
        TextView mStationFreqView;
        TextView mStationNameView;
        TextView mStationRdsView;
        RelativeLayout mImgLayout;
        FmVisualizerView mPlayIndicator;
        CheckBox mCheckBox;
    }

    private Cursor getData() {
        Cursor cursor = mContext.getContentResolver().query(Station.CONTENT_URI,
                FmStation.COLUMNS, null, null, FmStation.Station.FREQUENCY);
        return cursor;
    }

    class MyFavoriteAdapter extends BaseAdapter {
        private Cursor mCursor;
        private LayoutInflater mInflater;
        private boolean hiddenFlag = true;

        public MyFavoriteAdapter(Context context) {
            mInflater = LayoutInflater.from(context);
        }

        public void swipResult(Cursor cursor) {
            if (null != mCursor) {
                mCursor.close();
            }
            mCursor = cursor;
            notifyDataSetChanged();
        }

        @Override
        public int getCount() {
            if (null != mCursor) {
                return mCursor.getCount();
            }
            return 0;
        }

        @Override
        public Object getItem(int position) {
            return null;
        }

        @Override
        public long getItemId(int position) {
            return 0;
        }

        @Override
        public View getView(int position, View convertView, ViewGroup parent) {
            ViewHolder viewHolder = null;
            if (null == convertView) {
                viewHolder = new ViewHolder();
                convertView = mInflater.inflate(R.layout.station_item, null);
                viewHolder.mFavoruteTag = (TextView) convertView
                        .findViewById(R.id.tv_Favorite_Tag);
                viewHolder.mStationTypeView = (ImageView) convertView
                        .findViewById(R.id.lv_station_type);
                viewHolder.mStationFreqView = (TextView) convertView
                        .findViewById(R.id.lv_station_freq);
                viewHolder.mStationNameView = (TextView) convertView
                        .findViewById(R.id.lv_station_name);
                viewHolder.mStationRdsView = (TextView) convertView
                        .findViewById(R.id.lv_station_rds);
                viewHolder.mImgLayout = (RelativeLayout) convertView
                        .findViewById(R.id.list_item_right);
                viewHolder.mPlayIndicator = (FmVisualizerView) convertView
                        .findViewById(R.id.fm_play_indicator);
                viewHolder.mCheckBox = (CheckBox) convertView
                        .findViewById(R.id.ck_select);
                convertView.setTag(viewHolder);
            } else {
                viewHolder = (ViewHolder) convertView.getTag();
            }

            /*
             * SPRD : bug568587, new feature FM new UI @{ if (mCursor != null &&
             * mCursor.moveToFirst()) { mCursor.moveToPosition(position); final int stationFreq =
             * mCursor.getInt(mCursor .getColumnIndex(FmStation.Station.FREQUENCY)); String name =
             * mCursor.getString(mCursor .getColumnIndex(FmStation.Station.STATION_NAME)); String
             * rds = mCursor.getString(mCursor .getColumnIndex(FmStation.Station.RADIO_TEXT)); final
             * int isFavorite = mCursor.getInt(mCursor
             * .getColumnIndex(FmStation.Station.IS_FAVORITE));
             */
            if (mActionMode != null && !hiddenFlag) {
                viewHolder.mCheckBox.setVisibility(View.VISIBLE);
                viewHolder.mCheckBox.setChecked(mCheckboxStatus.get(position));
            } else {
                viewHolder.mCheckBox.setVisibility(View.GONE);
            }
            if (mFavoriteList.size() != 0 && position == 0) {
                viewHolder.mFavoruteTag.setVisibility(View.VISIBLE);
                viewHolder.mFavoruteTag.setText(getString(R.string.favorite_station_tag));
            } else if (position == mFavoriteList.size()) {
                viewHolder.mFavoruteTag.setVisibility(View.VISIBLE);
                viewHolder.mFavoruteTag.setText(getString(R.string.other_station_tag));
            } else {
                viewHolder.mFavoruteTag.setVisibility(View.GONE);
            }
            final StationItem item;
            if (position < mFavoriteList.size()) {
                item = mFavoriteList.get(position);
            } else {
                item = mOtherList.get(position - mFavoriteList.size());
            }
            final int stationFreq = item.stationFreq;
            String name = item.name;
            String rds = item.rds;
            final int isFavorite = item.isFavorite;

            if (stationFreq == mCurrentStation) {
                viewHolder.mPlayIndicator.setVisibility(View.VISIBLE);
                if (mService != null && mService.getPowerStatus() == FmService.POWER_UP && !mService.isMuted()) {
                    viewHolder.mPlayIndicator.startAnimation();
                } else {
                    viewHolder.mPlayIndicator.stopAnimation();
                }
            } else {
                viewHolder.mPlayIndicator.stopAnimation();
                viewHolder.mPlayIndicator.setVisibility(View.INVISIBLE);
            }
            if (null == name || "".equals(name)) {
                name = item.program_service;
            }
            /* @} */

            if (rds == null || rds.equals("")) {
                viewHolder.mStationRdsView.setVisibility(View.GONE);
            } else {
                viewHolder.mStationRdsView.setVisibility(View.VISIBLE);
            }

            viewHolder.mStationFreqView.setText(FmUtils.formatStation(stationFreq));
            viewHolder.mStationNameView.setText(name);
            viewHolder.mStationRdsView.setText(rds);
            if (0 == isFavorite) {
                viewHolder.mStationTypeView.setImageResource(R.drawable.btn_fm_list_favorite_off);
                viewHolder.mStationTypeView.setColorFilter(Color.BLACK,
                        PorterDuff.Mode.SRC_ATOP);
                viewHolder.mStationTypeView.setAlpha(0.54f);
            } else {
                viewHolder.mStationTypeView.setImageResource(R.drawable.btn_fm_list_favorite_on);
                viewHolder.mStationTypeView.setColorFilter(null);
                viewHolder.mStationTypeView.setAlpha(1.0f);
            }

            viewHolder.mImgLayout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    /* SPRD : bug568587, new feature FM new UI @{ */
                    if (0 == isFavorite) {
                        addFavorite(stationFreq);
                        mOtherList.remove(item);
                        StationItem newItem = item;
                        newItem.isFavorite = 1;
                        insertItem(mFavoriteList, newItem);
                        notifyDataSetChanged();
                    } else {
                        deleteFavorite(stationFreq);
                        mFavoriteList.remove(item);
                        StationItem newItem = item;
                        newItem.isFavorite = 0;
                        insertItem(mOtherList, newItem);
                        notifyDataSetChanged();
                    }
                    /* @} */
                }
            });
            return convertView;
        }

        /* SPRD : bug568587, new feature FM new UI @{ */
        public void setCheckboxHidden(boolean flag) {
            hiddenFlag = flag;
        }
        /* @} */
    }

    /**
     * Add searched station as favorite station
     */
    public void addFavorite(int stationFreq) {
        // TODO it's on UI thread, change to sub thread
        // update the station name and station type in database
        // according the frequency
        FmStation.addToFavorite(mContext, stationFreq);
        mMyAdapter.swipResult(getData());
    }

    /**
     * Delete favorite from favorite station list, make it as searched station
     */
    public void deleteFavorite(int stationFreq) {
        // TODO it's on UI thread, change to sub thread
        // update the station type from favorite to searched.
        FmStation.removeFromFavorite(mContext, stationFreq);
        mMyAdapter.swipResult(getData());
    }

    @Override
    protected void onResume() {
        super.onResume();
        mIsActivityForeground = true;
        if (null != mService) {
            mService.setFmMainActivityForeground(mIsActivityForeground);
            if (FmRecorder.STATE_RECORDING != mService.getRecorderState()) {
                mService.removeNotification();
            }
        }
    }

    @Override
    protected void onPause() {
        mIsActivityForeground = false;
        if (null != mService) {
            mService.setFmMainActivityForeground(mIsActivityForeground);
        }
        super.onPause();
    }

    @Override
    protected void onStop() {
        // FmUtils.updateFrontActivity(mContext, "");
        if (null != mService) {
            // home key pressed, show notification
            mService.setNotificationClsName(FmFavoriteActivity.class.getName());
            mService.updatePlayingNotification();
        }
        super.onStop();
    }

    @Override
    protected void onDestroy() {
        mMyAdapter.swipResult(null);
        FmService.unregisterExitListener(mExitListener);
        if (mService != null) {
            mService.unregisterFmRadioListener(mFmRadioListener);
        }
        unbindService();
        super.onDestroy();
    }

    private FmService mService = null;

    private boolean mIsServiceBinded = false;

    private void bindService() {
        mIsServiceBinded = bindService(new Intent(FmFavoriteActivity.this, FmService.class),
                mServiceConnection, Context.BIND_AUTO_CREATE);
        if (!mIsServiceBinded) {
            Log.e(TAG, "bindService, mIsServiceBinded is false");
            finish();
        }
    }

    private void unbindService() {
        if (mIsServiceBinded) {
            unbindService(mServiceConnection);
        }
    }

    // Service listener
    private FmListener mFmRadioListener = new FmListener() {
        @Override
        public void onCallBack(Bundle bundle) {
            int flag = bundle.getInt(FmListener.CALLBACK_FLAG);
            if (flag == FmListener.MSGID_FM_EXIT) {
                mHandler.removeCallbacksAndMessages(null);
            }

            // remove tag message first, avoid too many same messages in queue.
            Message msg = mHandler.obtainMessage(flag);
            msg.setData(bundle);
            mHandler.removeMessages(flag);
            mHandler.sendMessage(msg);
        }
    };

    /**
     * Main thread handler to update UI
     */
    private Handler mHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            Log.d(TAG,
                    "handleMessage, what = " + msg.what + ",hashcode:"
                            + mHandler.hashCode());
            Bundle bundle;
            switch (msg.what) {
                case FmListener.MSGID_SCAN_FINISHED:
                    bundle = msg.getData();
                    // cancel scan happen
                    boolean isScan = bundle.getBoolean(FmListener.KEY_IS_SCAN);
                    int searchedNum = bundle.getInt(FmListener.KEY_STATION_NUM);
                    refreshMenuItem(true);
                    /* SPRD : bug568587, new feature FM new UI @{ */
                    Cursor cursor = getData();
                    query(cursor);
                    mMyAdapter.swipResult(cursor);
                    /* @} */
                    mService.updatePlayingNotification();
                    if (searchedNum == 0) {
                        Toast.makeText(mContext, getString(R.string.toast_cannot_search),
                                Toast.LENGTH_SHORT).show();
                        // searched station is zero, if db has station, should not use empty.
                        if (mMyAdapter.getCount() == 0) {
                            View emptyView = (View) findViewById(R.id.empty_tips);
                            emptyView.setVisibility(View.VISIBLE);
                            View searchTips = (View) findViewById(R.id.search_tips);
                            searchTips.setVisibility(View.GONE);
                        }
                        return;
                    }
                    // Show toast to tell user how many stations have been searched
                    String text = getString(R.string.toast_station_searched) + " "
                            + String.valueOf(searchedNum);
                    Toast.makeText(mContext, text, Toast.LENGTH_SHORT).show();
                    break;
                case FmListener.MSGID_SWITCH_ANTENNA:
                    bundle = msg.getData();
                    boolean isHeadset = bundle.getBoolean(FmListener.KEY_IS_SWITCH_ANTENNA);
                    // if receive headset plugout, need set headset mode on ui
                    if (!isHeadset) {
                        finish();
                    }
                    break;
                case FmListener.MSGID_POWERDOWN_FINISHED:
                    finish();
                    break;
                case FmListener.MSGID_TUNE_FINISHED:
                    if (mService != null) mCurrentStation = mService.getFrequency();
                    if (mMyAdapter != null) mMyAdapter.notifyDataSetChanged();
                    break;
                case FmListener.MSGID_SET_MUTE_FINISHED:
                    if (mMyAdapter != null) mMyAdapter.notifyDataSetChanged();
                    break;
                default:
                    break;
            }
        }
    };

    private void refreshMenuItem(boolean enabled) {
        // action menu
        /**
         * SPRD: bug474750, scan from current freq. Original Android code: if (mMenuRefresh != null)
         * {
         * 
         * @{
         */
        if (mMenuRefresh != null && mMenuItemScanFromCurrent != null) {
            mMenuRefresh.setEnabled(enabled);
            mMenuItemScanFromCurrent.setEnabled(enabled);
            resetMenuTitleColor(enabled);
        }
        /**
         * @}
         */
    }

    // When call bind service, it will call service connect. register call back
    // listener and initial device
    private final ServiceConnection mServiceConnection = new ServiceConnection() {

        /**
         * called by system when bind service
         * 
         * @param className component name
         * @param service service binder
         */
        @Override
        public void onServiceConnected(ComponentName className, IBinder service) {
            mService = ((FmService.ServiceBinder) service).getService();
            if (null == mService) {
                Log.e(TAG, "onServiceConnected, mService is null");
                finish();
                return;
            }
            mService.registerFmRadioListener(mFmRadioListener);
            mService.setFmMainActivityForeground(mIsActivityForeground);
            if (FmRecorder.STATE_RECORDING != mService.getRecorderState()) {
                mService.removeNotification();
            }
            // FmUtils.isFirstEnterStationList() must be called at the first time.
            // After it is called, it will save status to SharedPreferences.
            if (FmUtils.isFirstEnterStationList(mContext) || (0 == mMyAdapter.getCount())) {
                refreshMenuItem(false);
                mLvFavorites.setEmptyView(mSearchTips);
                mSearchProgress.setIndeterminate(true);
                mMyAdapter.swipResult(null);
                // SPRD: bug535714, FM search station gain when change orientation.
                if (!mService.isScanning()) {
                    mService.startScanAsync();
                }
            } else {
                boolean isScan = mService.isScanning();
                if (isScan) {
                    mMyAdapter.swipResult(null);
                    mLvFavorites.setEmptyView(mSearchTips);
                    mSearchProgress.setIndeterminate(true);
                } else {
                    // TODO it's on UI thread, change to sub thread
                    mMyAdapter.swipResult(getData());
                }
                refreshMenuItem(!isScan);
            }
        }

        /**
         * When unbind service will call this method
         * 
         * @param className The component name
         */
        @Override
        public void onServiceDisconnected(ComponentName className) {
        }
    };

    /**
     * check gps is open or not
     * 
     * @return true is open
     */
    private boolean isGpsOpen() {
        return mLocationManager.isProviderEnabled(android.location.LocationManager.GPS_PROVIDER);
    }

    @Override
    public void onRequestPermissionsResult(int requestCode,
            String permissions[], int[] grantResults) {
        Log.d(TAG, "onRequestPermissionsResult");
        switch (requestCode) {
            case FmUtils.FM_PERMISSIONS_REQUEST_CODE: {
                boolean resultsAllGranted = true;
                if (grantResults.length > 0) {
                    for (int result : grantResults) {
                        if (PackageManager.PERMISSION_GRANTED != result) {
                            resultsAllGranted = false;
                        }
                    }
                } else {
                    resultsAllGranted = false;
                }
                if (resultsAllGranted) {
                    if (mOptionSelectedId == R.id.fm_station_list_refresh) {
                        onOptionsItemSelectedScan(false);
                    } else {
                        onOptionsItemSelectedScan(true);
                    }
                } else {
                    Toast.makeText(this, getString(R.string.error_permissions), Toast.LENGTH_LONG)
                            .show();
                }
            }
        }
    }

    private int mOptionSelectedId;

    private void onOptionsItemSelectedScan(boolean fromCurrent) {
        if (null != mService) {
            refreshMenuItem(false);

            mMyAdapter.swipResult(null);
            /**
             * SPRD: bug511231 Both emptyView and mSearchTips displayed when no station searched.
             * 
             * @{
             */
            View emptyView = (View) findViewById(R.id.empty_tips);
            emptyView.setVisibility(View.GONE);
            /**
             * @}
             */
            mLvFavorites.setEmptyView(mSearchTips);
            mSearchProgress.setIndeterminate(true);

            // If current location and last location exceed defined distance, delete the RDS
            // database
            if (isGpsOpen()) {
                mCurLocation = mLocationManager
                        .getLastKnownLocation(LocationManager.GPS_PROVIDER);
                if (mCurLocation != null) {
                    double[] lastLocations = FmUtils.getLastSearchedLocation(mContext);
                    float distance[] = new float[2];
                    Location.distanceBetween(lastLocations[0], lastLocations[1],
                            mCurLocation.getLatitude(), mCurLocation.getLongitude(),
                            distance);
                    float searchedDistance = distance[0];
                    boolean exceed =
                            searchedDistance > FmUtils.LOCATION_DISTANCE_EXCEED;
                    mService.setDistanceExceed(exceed);
                    FmUtils.setLastSearchedLocation(mContext, mCurLocation.getLatitude(),
                            mCurLocation.getLongitude());
                }
            }

            if (fromCurrent) {
                mService.startScanAsyncFromCurrent(mService.getFrequency());
            } else {
                mService.startScanAsync();
            }
        }
    }

    /* SPRD: bug568587 FM new UI @{ */
    private class StationItem {
        private int stationFreq;
        private String name;
        private String rds;
        private String program_service;
        private int isFavorite;
    }

    private void query(Cursor cursor) {
        if (cursor == null) {
            return;
        }
        mFavoriteList.clear();
        mOtherList.clear();
        mCheckboxStatus.clear();
        for (int i = 0; i < cursor.getCount(); i++) {
            StationItem item = new StationItem();
            if (cursor.moveToFirst()) {
                cursor.moveToPosition(i);
                item.stationFreq = cursor.getInt(cursor
                        .getColumnIndex(FmStation.Station.FREQUENCY));
                item.name = cursor.getString(cursor
                        .getColumnIndex(FmStation.Station.STATION_NAME));
                item.rds = cursor.getString(cursor
                        .getColumnIndex(FmStation.Station.RADIO_TEXT));
                item.isFavorite = cursor.getInt(cursor
                        .getColumnIndex(FmStation.Station.IS_FAVORITE));
                item.program_service = cursor.getString(cursor
                        .getColumnIndex(FmStation.Station.PROGRAM_SERVICE));
                if (item.isFavorite == 0) {
                    mOtherList.add(item);
                } else {
                    mFavoriteList.add(item);
                }
                mCheckboxStatus.add(false);
            }
        }
    }

    private void insertItem(List<StationItem> list, StationItem item) {
        if (list.size() == 0) {
            list.add(item);
            return;
        }
        int startItem = list.get(0).stationFreq;
        int endItem = list.get(list.size() - 1).stationFreq;
        int lastFreq = startItem;
        if (item.stationFreq < startItem) {
            list.add(0, item);
            return;
        } else if (item.stationFreq > endItem) {
            list.add(item);
            return;
        }
        for (int i = 0; i < list.size(); i++) {
            if (item.stationFreq > lastFreq && item.stationFreq < list.get(i).stationFreq) {
                list.add(i, item);
                return;
            }
            lastFreq = list.get(i).stationFreq;
        }
    }

    private class StationListOnItemLongClickListener implements OnItemLongClickListener {
        @Override
        public boolean onItemLongClick(AdapterView<?> parent, View view,
                int position, long id) {
            if (mActionMode != null) {
                CheckBox checkBox = (CheckBox) view.findViewById(R.id.ck_select);
                if (null == checkBox) {
                    return true;
                }
                if (true == checkBox.isChecked()) {
                    checkBox.setChecked(false);
                } else {
                    checkBox.setChecked(true);
                }
                mCheckboxStatus.set(position, checkBox.isChecked());
                mActionMode.setTitle(String.valueOf(getCheckBoxSelectSize()));
                if (getCheckBoxSelectSize() == mCheckboxStatus.size()) {
                    mCheckBoxSelectAll.setChecked(true);
                }else{
                    mCheckBoxSelectAll.setChecked(false);
                }
                return true;
            } else {
                if (position < mFavoriteList.size()) {
                    FmFavoriteActivity.this.mEditPosition = position;
                    showPopupWindow(view);
                    return true;
                } else {
                    Toast.makeText(mContext, R.string.batch_operation, Toast.LENGTH_SHORT).show();
                    setBatchOperLayoutVisibility(View.VISIBLE);
                    if (mActionMode == null && mMyAdapter != null) {
                        startActionMode(new itemLongClickCallback(position));
                    }
                    return true;
                }
            }
        }
    }

    private PopupWindow popupWindow = null;
    private int mEditPosition = -1;

    private void showPopupWindow(View parent) {
        View contentView = LayoutInflater.from(mContext).inflate(
                R.layout.station_edit_popupwindow, null);
        TextView tv_edit = (TextView) contentView.findViewById(R.id.tv_edit);
        tv_edit.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (popupWindow != null) {
                    popupWindow.dismiss();
                    showEditDialog();
                }
            }
        });
        TextView tv_batch_operation = (TextView) contentView.findViewById(R.id.tv_batch_operation);
        tv_batch_operation.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (popupWindow != null) {
                    popupWindow.dismiss();
                    Toast.makeText(mContext, R.string.batch_operation, Toast.LENGTH_SHORT).show();
                    setBatchOperLayoutVisibility(View.VISIBLE);
                    if (mActionMode == null && mMyAdapter != null) {
                        startActionMode(new itemLongClickCallback(mEditPosition));
                    }
                }
            }
        });
        popupWindow = new PopupWindow(contentView,
                LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, true);
        popupWindow.setTouchable(true);
        popupWindow.setTouchInterceptor(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                return false;
            }
        });
        popupWindow.setBackgroundDrawable(getResources().getDrawable(
                R.drawable.popupwindow_bg));
        int[] location = new int[2];
        parent.getLocationOnScreen(location);
        int offsetX = (mLvFavorites.getWidth() - popupWindow.getWidth()) / 3;
        popupWindow.showAtLocation(mLvFavorites, Gravity.TOP | Gravity.LEFT, location[0] + offsetX,
                location[1]);
    }

    private void showEditDialog() {
        int position = FmFavoriteActivity.this.mEditPosition;
        LayoutInflater inflater = LayoutInflater.from(this);
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        View view = inflater.inflate(R.layout.station_edit_dialog, null);
        final EditText et_freq = (EditText) view.findViewById(R.id.edit_freq);
        et_freq.setText(FmUtils.formatStation(mFavoriteList.get(position).stationFreq));
        final EditText et_name = (EditText) view.findViewById(R.id.edit_name);
        // If the station name is not exist, show program service(PS) instead
        String stationName = mFavoriteList.get(position).name;
        if (TextUtils.isEmpty(stationName)) {
            stationName = mFavoriteList.get(position).program_service;
        }
        et_name.setText(stationName);
        builder.setPositiveButton(R.string.btn_save_recording,
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        String strFreq = et_freq.getText().toString();
                        String strName = et_name.getText().toString();
                        if (strFreq.length() == 0) {
                            Toast.makeText(getApplicationContext(),
                                    getString(R.string.freq_input_info), Toast.LENGTH_SHORT).show();
                        } else {
                            if (strFreq.charAt(0) == '.') {
                                strFreq = '0' + strFreq;
                            }
                            Float fFreq = Float.parseFloat(strFreq);
                            int iFreq = (int) (fFreq * FmUtils.CONVERT_RATE);
                            int oldFrep = mFavoriteList.get(mEditPosition).stationFreq;
                            Log.i(TAG, "iFrep:" + iFreq  +",oldFreq:" + oldFrep);
                            int updateResult = 0;
                            if (FmUtils.isValidStation(iFreq)) {
                                if (oldFrep == iFreq) {
                                    updateResult = FmStation.updateStationToDb(mContext, oldFrep,
                                            strName);
                                } else {
                                    ContentValues values = null;
                                    values = new ContentValues(1);
                                    values.put(Station.STATION_NAME, strName);
                                    values.put(Station.FREQUENCY, iFreq);
                                    values.put(Station.IS_FAVORITE,true);
                                    if (!FmStation.isStationExist(mContext, iFreq)) {
                                        updateResult = FmStation.updateStationToDb(mContext,oldFrep,values);
                                    } else {
                                        // remove dup freq from db
                                        FmStation.deleteStationInDb(mContext, oldFrep);
                                        updateResult = FmStation.updateStationToDb(mContext,iFreq,values);
                                    }
                                }
                                if (updateResult != 0) {
                                    updateCheckboxStatus();
                                    Cursor cursor = getData();
                                    //update the listview data,and make sure the list is sorted:
                                    query(cursor);
                                    mMyAdapter.swipResult(cursor);
                                    mMyAdapter.notifyDataSetChanged();
                                    Toast.makeText(getApplicationContext(),
                                            R.string.toast_station_freq_changed,
                                            Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(getApplicationContext(),
                                        getString(R.string.range_error), Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                });

        builder.setNegativeButton(R.string.btn_discard_recording,
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.cancel();
                    }
                });
        Dialog mDialog = builder.setView(view).create();
        mDialog.setCanceledOnTouchOutside(false);
        mDialog.show();
    }

    private void setBatchOperLayoutVisibility(int visibility) {
        LinearLayout lay = (LinearLayout) findViewById(R.id.layout_batch_operation);
        lay.setVisibility(visibility);
        if (visibility == View.VISIBLE) {
            mCheckBoxSelectAll.setChecked(false);
            for (int i = 0; i < mMyAdapter.getCount(); i++) {
                mCheckboxStatus.set(i, false);
            }
            mMyAdapter.notifyDataSetChanged();
        }
        if (visibility == View.VISIBLE) {

            lay.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    mCheckBoxSelectAll.setChecked(!mCheckBoxSelectAll.isChecked());
                    for (int i = 0; i < mMyAdapter.getCount(); i++) {
                        mCheckboxStatus.set(i, mCheckBoxSelectAll.isChecked());
                    }
                    mMyAdapter.notifyDataSetChanged();
                    if (mActionMode != null) {
                        if (mCheckBoxSelectAll.isChecked()) {
                            mActionMode.setTitle(String.valueOf(mCheckboxStatus
                                    .size()));
                            updateActionModeItem(true);
                        } else {
                            mActionMode.setTitle(String.valueOf(0));
                            updateActionModeItem(false);
                        }
                    }
                }
            });
        }
    }

    private void updateActionModeItem(boolean isEnable) {
        if (isEnable) {
            mMenuItemFavoriteOn.setEnabled(true);
            mMenuItemFavoriteOn.setIcon(getResources().getDrawable(R.drawable.ic_menu_favorite_on));
            mMenuItemFavoriteOff.setEnabled(true);
            mMenuItemFavoriteOff.setIcon(getResources()
                    .getDrawable(R.drawable.ic_menu_favorite_off));
            mMenuItemDelete.setEnabled(true);
            mMenuItemDelete.setIcon(getResources().getDrawable(R.drawable.ic_menu_delete_on));
        } else {
            mMenuItemFavoriteOn.setEnabled(false);
            mMenuItemFavoriteOn.setIcon(getResources().getDrawable(
                    R.drawable.ic_menu_favorite_on_disable));
            mMenuItemFavoriteOff.setEnabled(false);
            mMenuItemFavoriteOff.setIcon(getResources().getDrawable(
                    R.drawable.ic_menu_favorite_off_disable));
            mMenuItemDelete.setEnabled(false);
            mMenuItemDelete.setIcon(getResources().getDrawable(R.drawable.ic_menu_delete_disable));
        }
    }

    private int getCheckBoxSelectSize() {
        int size = 0;
        for (int i = 0; i < mCheckboxStatus.size(); i++) {
            if (mCheckboxStatus.get(i)) {
                size++;
            }
        }
        return size;
    }

    private class itemLongClickCallback implements ActionMode.Callback {

        private int position = -1;

        public itemLongClickCallback(int position) {
            this.position = position;
        }

        @Override
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            mActionMode = mode;
            MenuInflater inflater = mode.getMenuInflater();
            inflater.inflate(R.menu.batch_operation_menu, menu);
            mMenuItemFavoriteOn = menu.findItem(R.id.item_add_to_favourite);
            mMenuItemFavoriteOff = menu.findItem(R.id.item_remove_favourite);
            mMenuItemDelete = menu.findItem(R.id.item_delete);
            mCheckboxStatus.set(position, true);
            mActionMode.setTitle(String.valueOf(getCheckBoxSelectSize()));
            mMyAdapter.setCheckboxHidden(false);
            mMyAdapter.notifyDataSetChanged();
            if (getCheckBoxSelectSize() == mCheckboxStatus.size()) {
                mCheckBoxSelectAll.setChecked(true);
            }
            return true;
        }

        @Override
        public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
            FmUtils.updateStatusBarColor(FmFavoriteActivity.this, true);
            return true;
        }

        @Override
        public void onDestroyActionMode(ActionMode mode) {
            mActionMode = null;
            if (mMyAdapter != null) {
                mMyAdapter.setCheckboxHidden(true);
                mMyAdapter.notifyDataSetChanged();
                setBatchOperLayoutVisibility(View.GONE);
            }
            FmUtils.updateStatusBarColor(FmFavoriteActivity.this, false);
        }

        @Override
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                case R.id.item_add_to_favourite:
                    addSelectItemToFavourite();
                    break;
                case R.id.item_remove_favourite:
                    removeSelectItemToFavourite();
                    break;
                case R.id.item_delete:
                    deleteSelectItemToFavourite();
                    break;
            }
            // buttonClickableChanges();
            return true;
        }
    }

    private void addSelectItemToFavourite() {
        StationItem item = null;
        int favouriteSize = mFavoriteList.size();
        List<StationItem> selectItems = new ArrayList<StationItem>();
        for (int i = 0; i < mCheckboxStatus.size(); i++) {
            if (i >= mFavoriteList.size() && mCheckboxStatus.get(i)) {
                item = mOtherList.get(i - favouriteSize);
                StationItem newItem = item;
                newItem.isFavorite = 1;
                selectItems.add(item);
            }
        }
        for (int i = 0; i < selectItems.size(); i++) {
            item = selectItems.get(i);
            int stationFreq = item.stationFreq;
            addFavorite(stationFreq);
            mOtherList.remove(item);
            insertItem(mFavoriteList, item);
        }
        mMyAdapter.notifyDataSetChanged();
        mActionMode.finish();
    }

    private void removeSelectItemToFavourite() {
        StationItem item = null;
        List<StationItem> selectItems = new ArrayList<StationItem>();
        for (int i = 0; i < mCheckboxStatus.size(); i++) {
            if (i < mFavoriteList.size() && mCheckboxStatus.get(i)) {
                item = mFavoriteList.get(i);
                StationItem newItem = item;
                newItem.isFavorite = 0;
                selectItems.add(item);
            }
        }
        for (int i = 0; i < selectItems.size(); i++) {
            item = selectItems.get(i);
            int stationFreq = item.stationFreq;
            deleteFavorite(stationFreq);
            mFavoriteList.remove(item);
            insertItem(mOtherList, item);
        }
        mMyAdapter.notifyDataSetChanged();
        mActionMode.finish();
    }

    private void deleteSelectItemToFavourite() {
        int favouriteSize = mFavoriteList.size();
        StationItem item = null;
        int stationFreq = -1;
        List<StationItem> selectItems = new ArrayList<StationItem>();
        for (int i = 0; i < mCheckboxStatus.size(); i++) {
            if (mCheckboxStatus.get(i)) {
                if (i < mFavoriteList.size()) {
                    item = mFavoriteList.get(i);
                    selectItems.add(item);
                } else {
                    item = mOtherList.get(i - favouriteSize);
                    selectItems.add(item);
                }
            }
        }
        for (int i = 0; i < selectItems.size(); i++) {
            item = selectItems.get(i);
            if (mFavoriteList.contains(item)) {
                mFavoriteList.remove(item);
            } else if (mOtherList.contains(item)) {
                mOtherList.remove(item);
            }
            FmStation.deleteStationInDb(mContext, item.stationFreq);
        }
        updateCheckboxStatus();
        Cursor cursor = getData();
        mMyAdapter.swipResult(cursor);
        mMyAdapter.notifyDataSetChanged();
        mActionMode.finish();
        if (mFavoriteList.size() == 0) {
            View emptyView = (View) findViewById(R.id.empty_tips);
            emptyView.setVisibility(View.GONE);
            View searchTips = (View) findViewById(R.id.search_tips);
            searchTips.setVisibility(View.GONE);
        }
    }

    private void updateCheckboxStatus() {
        mCheckboxStatus.clear();
        for (int i = 0; i < (mFavoriteList.size() + mOtherList.size()); i++) {
            mCheckboxStatus.add(false);
        }
    }

    /* SPRD: bug568587 FM new UI @} */
}
