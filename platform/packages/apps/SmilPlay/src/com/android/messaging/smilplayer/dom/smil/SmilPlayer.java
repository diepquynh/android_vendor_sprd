...
package com.android.messaging.smilplayer.dom.smil;
...
public class SmilPlayer implements Runnable {
    ...
    //add for bug 458713 begin
    private static final int EVENT_RELEASE = 1;
    //add for bug 458713 end
    ...
    public synchronized void play() {
        if (!isPlayingState()) {
            ...
            mCurrentElement = 0;
            mCurrentSlide = 0;
            //modify for bug 458713 begin
            if (mPlayerHandler == null) {
                mPlayerThread = new HandlerThread("SmilPlayer");
              //modify for bug  463461 begin
               // mState = SmilPlayerState.PLAYING;
              //modify for bug  463461 end
                mPlayerThread.start();
                mPlayerLooper = mPlayerThread.getLooper();
                mPlayerHandler = new PlayerHandler(mPlayerLooper);
            }
          //modify for bug  463461 begin
            mState = SmilPlayerState.PLAYING;
          //modify for bug  463461 end
            mPlayerHandler.post(this);
            //modify for bug 458713 end
        } else {
    ...
    public void release() {
        ...
        mAllEntries = null;
        mRoot = null;*/
        if(mPlayerHandler != null){ // fix for bug 467725 begin
           mPlayerHandler.sendEmptyMessage(EVENT_RELEASE);
           mPlayerHandler = null;
        }// fix for bug 467725 end
        //modify for bug 458713 end
    }
    /** @} */
