...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    enum EndState {
        SYSTEM_APP, NOT_EMPTY_FOLDER, USER_CANCEL, NORMAL
    };
    /* @} */

    EndState mState = EndState.NORMAL;

    protected boolean isWorkspaceOrFolderOwnerWidget(DragObject d){
        throw new RuntimeException("can't call isWorkspaceOrFolderOwnerWidget() here");
    }
    ...
    private boolean isAllAppsWidget(DragSource source, Object info) {
        if (source instanceof AppsCustomizePagedView) {
            if (info instanceof PendingAddItemInfo) {
                ...
                /* SPRD: Feature 261947,OWNER_WIDGET is one of AllAppsWidget. @{ */
    ...
    public static boolean willAcceptDrop(Object info) {
        if (info instanceof ItemInfo) {
            ...
            /* SPRD: Feature 261947,we should accept the drop of OWNER_WIDGET. @{ */
            if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET ||
                    item.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT ||
                    item.itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_OWNER) {
                return true;
            }
            /* @} */
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        mActive = isVisible;
        /* SPRD: Fix bug260895,if is drag on Preview now,do not show this view. @{ */
        if (source instanceof Previews) {
            mActive = isVisible = false;
        }
        /* @} */
        resetHoverColor();
    ...
    private void deferCompleteDropIfUninstalling(DragObject d) {
        ...
        // SPRD: Feature 251656, Remove the application drawer view
        if (isUninstallFromWorkspace(d) || isWorkspaceFolder(d)) {
    ...
    private void completeDrop(DragObject d) {
        ...
        if (isAllAppsApplication(d.dragSource, item)) {
            ...
            if (shortcut.intent != null && shortcut.intent.getComponent() != null) {
                ...
                if(packageName != null){
                    int flags = AppInfo.initFlags(
                        ShortcutInfo.getPackageInfo(getContext(), packageName));
                    /* SPRD: Feature 251656, Remove the application drawer view @{
                    mWaitingForUninstall =
                    mLauncher.startApplicationUninstallActivity(componentName,
                    flags);*/
                    mWaitingForUninstall =
                            mLauncher.startApplicationUninstallActivity(shortcut, flags);
                    /* @} */
                }else{
                    Log.e(TAG,"DeleteDropTarget the completeDrop get packageName is null");
                }
                /* @} */
                    ...
                    final Runnable checkIfUninstallWasSuccess = new Runnable() {
                        ...
                        public void run() {
                            ...
                            if(packageName != null){
                                ...
                                uninstallSuccessful = activities.size() == 0;
                            }
                            /* @} */
            ...
            if (folderInfo.contents.size() == 0) {
                ...
                mWaitingForUninstall = true;
                ((Workspace) d.dragSource).onUninstallActivityReturned(true);
            } else {
                /* SPRD: Feature 253522, Remove the application drawer view @{
                Toast.makeText(mLauncher, R.string.folder_not_empty,
                Toast.LENGTH_SHORT).show();*/
                mState = EndState.NOT_EMPTY_FOLDER;
                /* @} */
            }
            /* @} */
        ...
        /* SPRD: Feature 261947,if this is OWNER_WIDGET,delete it. @{ */
        } else if (isWorkspaceOrFolderOwnerWidget(d)) {
            LauncherModel.deleteItemFromDatabase(mLauncher, item);
        }
        /* @} */
