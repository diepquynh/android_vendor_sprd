			...
			struct ipm_open_out *out, cmr_handle *ipm_class_handle)
{
	...
	if (!class_type_tab[index].tab_info->ops->open) {
		CMR_LOGE("Invalid ops Param!");
		return CMR_CAMERA_INVALID_PARAM;
	}

	ret = class_type_tab[index].tab_info->ops->open(ipm_handle,in,out,ipm_class_handle);
...
cmr_int ipm_notify_isp_finish(cmr_handle ipm_class_handle)
{
	cmr_int              ret             = CMR_CAMERA_SUCCESS;
	struct ipm_common    *common_handle  = (struct ipm_common *)ipm_class_handle;
	cmr_int              index;
	...
	if (!common_handle->ops->notify_isp_finish) {
		CMR_LOGE("Invalid ops Param!");
		return CMR_CAMERA_INVALID_PARAM;
	}

	ret = common_handle->ops->notify_isp_finish(ipm_class_handle);
...
cmr_int cmr_ipm_get_capability (struct ipm_capability *out)
{
	...
	for (i = 0; i < class_type_max; i++) {
		if (NULL != class_type_tab[i].tab_info)
