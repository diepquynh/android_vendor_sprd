...
public class WidgetPreviewLoader {
    ...
    /* SPRD: Fix bug 261340 @{ */
    private static int mWidgetPreviewMaxSize;
    /* @} */
    ...
    public WidgetPreviewLoader(Context context) {
        ...
        /* SPRD: Fix bug 261340 @{ */
        mWidgetPreviewMaxSize = mContext.
                getResources().getInteger(R.integer.widget_preview_max_size);
        /* @} */
    }
    ...
    public Bitmap getPreview(final Object o) {
        ...
        if (o instanceof SprdPendingAddFolderInfo){
            Bitmap b = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.portal_ring_inner_holo);
            return b;
        }
        /* @} */
    ...
    public void recycleBitmap(Object o, Bitmap bitmapToRecycle) {
        ...
        synchronized (mLoadedPreviews) {
            if (mLoadedPreviews.containsKey(name)) {
                ...
                /* SPRD: bug330605 2014-07-07 suppress throw fatal exception. @{ */
                if (b == bitmapToRecycle) {
                    ...
                    Log.e(TAG, "name: " + name);
                    Log.e(TAG, "bitmap in catch: " + b);
                    Log.e(TAG, "bitmap passed: " + bitmapToRecycle);
                    mLoadedPreviews.remove(name);
                    if (bitmapToRecycle.isMutable()) {
                        synchronized (mUnusedBitmaps) {
                            mUnusedBitmaps.add(new SoftReference<Bitmap>(bitmapToRecycle));
                        }
                    }
    ...
    public static void removePackageFromDb(final CacheDb cacheDb, final String packageName) {
        ...
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                /* SPRD:fix bug 270592,SQLiteDiskIOException: disk I/O error@{ */
                SQLiteDatabase db ;
                try{
                    db = cacheDb.getWritableDatabase();
    ...
    public static void onConfigurationChanged() {
        cleanUpWidgetPreviewDb();
    };

    private static void cleanUpWidgetPreviewDb() {
        LauncherAppState app = LauncherAppState.getInstance();
        final CacheDb cacheDb = app.getWidgetPreviewCacheDb();
        if (cacheDb != null) {
            new AsyncTask<Void, Void, Void>() {
                public Void doInBackground(Void ... args) {
                    /* SPRD: fix bug 311378, SQLiteDiskIOException: disk I/O error. @{ */
                    try {
    ...
    public Bitmap generatePreview(Object info, Bitmap preview) {
        if (preview != null &&
            ...
            /* SPRD: add for 262515 @{ */
            if (SprdUtilities.isLauncherBackground(mContext)) {
                Log.d(TAG, "Improperly sized bitmap passed as argument");
                return null;
            }
            /* @} */
    ...
    public Bitmap generateWidgetPreview(ComponentName provider, int previewImage,
        ...
        /* SPRD: Fix bug 261340 @{ */
        if (drawable == null) drawable = mIconCache.getFullResIcon(packageName, iconId);
        /* @} */
        ...
        /* SPRD: bug306395 2014-04-29 adjust y position. @{ */
    ...
    private Bitmap generateShortcutPreview(
        ...
        c.setBitmap(null);
        /* SPRD: Fix bug 278782   @{ */
        renderDrawableToBitmap(icon, preview, 0, 0, maxWidth, maxHeight);
        /* @} */

        return preview;
    }
    /* SPRD: Fix bug 261340 , 271402 @{ */
    public static void renderDrawableToBitmap(
            Drawable d, Bitmap bitmap, int x, int y, int w, int h) {
        //float scale = mWidgetPreviewMaxSize * 1.0f/w;
