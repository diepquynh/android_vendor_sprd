...
int audBBATDev2AudioDev(int *pOutDev)
{
    LOG_I("%s In OutDev = 0x%x",__FUNCTION__, *pOutDev);
    switch(*pOutDev)
    {
        case AUD_OUTDEV_SPEAKER:
            *pOutDev=AUDIO_DEVICE_OUT_SPEAKER;
            break;
        case AUD_OUTDEV_EARPIECE:
            *pOutDev=AUDIO_DEVICE_OUT_EARPIECE;
            break;
        case AUD_OUTDEV_HEADSET:
            *pOutDev=AUDIO_DEVICE_OUT_WIRED_HEADPHONE;
            break;
        default:
            break;
    }
    LOG_I("%s out OutDev = 0x%x",__FUNCTION__, *pOutDev);
    return 0;
}
...
int audPlayerOpen( int outdev, int sampleRate, int stereo, int sync )
{
    ...
    // BBAT Dev to Audio Dev
    audBBATDev2AudioDev(&outDev);
    ...
    AudioSystem::setStreamVolumeIndex(streamType,15,outDev);
    FUN_EXIT;
    return 0;
}
