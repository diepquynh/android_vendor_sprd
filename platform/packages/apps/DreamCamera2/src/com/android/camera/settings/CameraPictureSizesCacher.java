...
public class CameraPictureSizesCacher {
    ...
    public static List<Size> getSizesForCamera(int cameraId, Context context) {
        ...
        if (thisCamera != null) {
            saveSlowMotionValues(thisCamera, context);
            String key_build = PICTURE_SIZES_BUILD_KEY + cameraId;
            String key_sizes = PICTURE_SIZES_SIZES_KEY + cameraId;
            SharedPreferences defaultPrefs = PreferenceManager.getDefaultSharedPreferences(context);
     ...
     * save the slow motion data into SettingsManager.SCOPE_GLOBAL.
     *
     * @param mCamera the camera opened with cameraId.
     * @param mContext context valid android application context.
     */
    private static void saveSlowMotionValues(Camera mCamera, Context mContext) {
        List<String> listSlowMotion = mCamera.getParameters().getSupportedSlowmotion();
        SettingsManager sm = new SettingsManager(mContext);
        if (listSlowMotion != null && sm != null) {
            String slowMotion = listSlowMotion.get(0);
            for (int i = 1; i < listSlowMotion.size(); i++) {
                slowMotion = slowMotion + "," + listSlowMotion.get(i);
            }
            sm.set(SettingsManager.SCOPE_GLOBAL,
                    Keys.KEY_VIDEO_SLOW_MOTION_ALL, slowMotion);
        }
    }
