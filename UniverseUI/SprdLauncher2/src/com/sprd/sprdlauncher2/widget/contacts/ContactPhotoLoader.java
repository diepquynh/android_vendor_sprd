...
public class ContactPhotoLoader implements Callback {
    ...
    private Bitmap bitmapSrc, scaledBitmap;

    public ContactPhotoLoader(Context context, int defaultResourceId) {
        mDefaultResourceId = defaultResourceId;
        mContext = context;
        SCALEDWIDTH = mContext.getResources().getDimensionPixelSize(
                R.dimen.data_item_scaled_width);
        SCALEDHEIGHT = mContext.getResources().getDimensionPixelSize(
                R.dimen.data_item_scaled_height);
    }
    ...
    private boolean loadCachedPhoto(ImageView view, long photoId) {
        ...
        if (holder == null) {
            ...
            bitmapSrc = holder.bitmapRef.get();
            if (bitmapSrc != null) {
                scaledBitmap();
    ...
    private void scaledBitmap() {
        int oldWidth = bitmapSrc.getWidth();
        int oldHeight = bitmapSrc.getHeight();

        float scaledX = (float) SCALEDWIDTH / oldWidth;
        float scaledY = (float) SCALEDHEIGHT / oldHeight;
        Matrix matrix = new Matrix();
        matrix.postScale(scaledX, scaledY);

        scaledBitmap = Bitmap.createBitmap(bitmapSrc, 0, 0, oldWidth,
                oldHeight, matrix, false);
    }
        ...
        if (bytes != null) {
            try {
                bitmapSrc = BitmapFactory.decodeByteArray(bytes, 0,
                        bytes.length, null);
                holder.bitmapRef = new SoftReference<Bitmap>(bitmapSrc);
        ...
        private void loadPhotosFromDatabase() {
            ...
            try {
                cursor = mResolver.query(Data.CONTENT_URI, COLUMNS,
                        mStringBuilder.toString(), mPhotoIdsAsStrings
