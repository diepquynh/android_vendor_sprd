...
import com.android.internal.telephony.cat.AppInterfaceSprd.CommandTypeSprd;
...
abstract class ResponseDataSprd {
    /**
...
class SelectItemResponseDataSprd extends ResponseDataSprd {
    // members
    private int mId;

    public SelectItemResponseDataSprd(int id) {
        super();
        mId = id;
    }
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        int tag = 0x80 | ComprehensionTlvTagSprd.ITEM_ID.value();
        buf.write(tag); // tag
        buf.write(1); // length
...
class GetInkeyInputResponseDataSprd extends ResponseDataSprd {
    // members
    ...
    public GetInkeyInputResponseDataSprd(String inData, boolean ucs2, boolean packed) {
        super();
        mIsUcs2 = ucs2;
        mIsPacked = packed;
        mInData = inData;
    ...
    public GetInkeyInputResponseDataSprd(boolean yesNoResponse) {
        super();
        mIsUcs2 = false;
        mIsPacked = false;
        mInData = "";
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        // Text string object
        int tag = 0x80 | ComprehensionTlvTagSprd.TEXT_STRING.value();
        buf.write(tag); // tag
...
class LanguageResponseDataSprd extends ResponseDataSprd {
    private String mLang;

    public LanguageResponseDataSprd(String lang) {
        super();
        mLang = lang;
    }
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        // Text string object
        int tag = 0x80 | ComprehensionTlvTagSprd.LANGUAGE.value();
        buf.write(tag); // tag
...
// See TS 31.111 section 6.4.15/ETSI TS 102 223
// TS 31.124 section 27.22.4.15 for test spec
class DTTZResponseDataSprd extends ResponseDataSprd {
    private Calendar mCalendar;

    public DTTZResponseDataSprd(Calendar cal) {
        super();
        mCalendar = cal;
    }
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        // DTTZ object
        int tag = 0x80 | CommandTypeSprd.PROVIDE_LOCAL_INFORMATION.value();
        buf.write(tag); // tag
...
/* SPRD: Add here for BIP function @{ */
class OpenChannelResponseDataSprd extends ResponseDataSprd {
    ...
    public OpenChannelResponseDataSprd(byte type, String param, int size, int id, boolean status) {
        super();
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        if (mLinkStatus) {
            ...
            tag = ComprehensionTlvTagSprd.CHANNEL_STATUS.value();
            buf.write(tag);
            // length
            buf.write(2);
        ...
        tag = ComprehensionTlvTagSprd.BEARER_DESCRIPTION.value();
        buf.write(tag); // tag
        ...
        tag = ComprehensionTlvTagSprd.BUFFER_SIZE.value();
        buf.write(tag);
        // length
        buf.write(2);
        // Buffer Size
...
class ChannelStatusResponseDataSprd extends ResponseDataSprd {
    private int mChannelId = 0;
    private boolean mLinkStatus = false;


    public ChannelStatusResponseDataSprd(int id, boolean status) {
        super();
        this.mChannelId = id;
        this.mLinkStatus = status;
    }
...
class SendDataResponseDataSprd extends ResponseDataSprd {
    private int mChannelLen = 0;

    public SendDataResponseDataSprd(int len) {
        super();
...
class ReceiveDataResponseDataSprd extends ResponseDataSprd {
    private int mDataLen = 0;
    private String mDataStr = null;

    public ReceiveDataResponseDataSprd(int len, String str) {
        super();
        this.mDataLen = len;
        this.mDataStr = str;
    }
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        CatLog.d(this, "[stk] ReceiveDataResponseData mDataLen = " + mDataLen +
                        " mDataStr = " + mDataStr);
        tag = 0x80 | ComprehensionTlvTagSprd.CHANNEL_DATA.value();
        buf.write(tag);
        // length
        byte[] data = null;
