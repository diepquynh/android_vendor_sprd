...
public class CameraActivity extends QuickActivity
    ...
    public void onCameraOpened(CameraAgent.CameraProxy camera) {
        ...
        int cameraId = mSettingsManager.getInteger(getModuleScope(),Keys.KEY_CAMERA_ID);
        if (CameraUtil.isLowBatteryNoFlashEnable() && cameraId == 0) {
        ...
        */
        ...
        new AsyncTask<Void, Void, FilmstripItem>() {
            ...
            protected void onPostExecute(final FilmstripItem data) {
                ...
                // the item was safed.
                /**
                 * SPRD Bug:474859 Add new Feature of scenery .
                 * SPRD Bug:501739 when capture in burst mode a  thumbnail appears @{
                ...
                if (mCurrentModule instanceof PhotoModule && mCurrentModule != null
                        ...
                        mCurrentModule instanceof VideoModule || mCurrentModule instanceof SprdSceneryModule
                        || mCurrentModule instanceof UcamFilterPhotoModule) {
                /* @} */
                        AsyncTask.THREAD_POOL_EXECUTOR.execute(new Runnable() {
                        @Override
                        public void run() {
    ...
    public void onCreateTasks(Bundle state) {
        ...
        /*
         * SPRD: Fix 474843 Add for Filter Feature
         * Fix 474847 Add for Time Stamps @{ */
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (UCamUtill.isUcamFilterEnable()) {
                    UcamFilterPhotoUtils.initFilter(CameraActivity.this);
                }
        ...
        MultiStorage.getInstance().initialize(getAndroidContext());
    ...
    private int getModeIndex() {
        ...
        int filterIndex = getResources().getInteger(R.integer.camera_mode_filter);
        ...
        */
            ...
            /* SPRD: Fix bug 570541, MakeupMoule button do not response when click @{ */
            if (mSettingsManager.getBoolean(SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMER_RESET)) {
                if (modeIndex == filterIndex) {
                    modeIndex = photoIndex;
                }
                mSettingsManager.set(SettingsManager.SCOPE_GLOBAL,  Keys.KEY_CAMER_RESET, false);
            }
            /* @} */
    ...
    public void onPauseTasks() {
        ...
        if (CameraUtil.isLowBatteryNoFlashEnable()) {
    ...
    protected void onStopTasks() {
        ...
        /* SPRD: Fix bug 583585, Turn off camera permissions on the fixed screen mode ,Will be reported to the wrong @{ */
        if (!mHasCriticalPermissions) {
            Log.i(TAG, "onStopTasks: Missing critical permissions.");
            return;
        }
        /* @} */
   ...
   /**
    ...
    public void onModeSelected(int modeIndex) {
        ...
        /* SPRD: Fix 474843 Add for Filter Feature @{ */
        boolean isHasEnteredFilter = mSettingsManager.getBoolean(
    ...
    public void onSettingsSelected() {
        ...
        // SPRD: Fix 474843 Add for Filter Feature
        intent.putExtra(CameraSettingsActivity.CURRENT_MODULE, getCurrentModuleIndex());
        // SPRD: add curent module's camera scopename(front/back)
        intent.putExtra(CameraSettingsActivity.CAMERA_MODULE_SCOPENAME, getModuleManager().getModuleAgent(getCurrentModuleIndex()).getScopeNamespace());
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT) && !mSecureCamera && !isCaptureIntent()
                ...
                if (StorageUtil.getStoragePathState(StorageUtil.KEY_DEFAULT_INTERNAL)) {
    ...
    public void onFilterCameraClicked(View v) {
        Log.i(TAG, "onFilterCameraClicked");

        /* SPRD: Fix bug 563064 @{ */
        boolean isHasEnteredBeauty = mSettingsManager.getBoolean(
                SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_BEAUTY_ENTERED);
        if (isHasEnteredBeauty) return;
        /* @} */
        boolean isHasEnteredFilter = mSettingsManager.getBoolean(
                SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_FILTER_ENTERED);
        mSettingsManager.set(SettingsManager.SCOPE_GLOBAL
                , Keys.KEY_CAMERA_FILTER_ENTERED, !isHasEnteredFilter);

        int filterModuleId = getResources().getInteger(
                R.integer.camera_mode_filter);
        mCameraId = mSettingsManager.getInteger(getModuleScope(),Keys.KEY_CAMERA_ID);
        if (mCurrentModule instanceof PhotoModule) {
            Log.i(TAG, "ChangedModule To UcamFilterPhotoModule.");
            mCameraAppUI.freezeScreenUntilPreviewReady();
            changedModule(filterModuleId);
        } else if (mCurrentModule instanceof UcamFilterPhotoModule
                && isShutterEnabled()) {
            Log.i(TAG, "ChangedModule To PhotoModule.");
            UcamFilterPhotoModule filterModule = (UcamFilterPhotoModule)mCurrentModule;
            filterModule.delayUiPause();
            changedModule(getResources()
                    .getInteger(R.integer.camera_mode_photo));
        } else {
            Log.i(TAG, "ChangedModule To Default Module UcamFilterPhotoModule.");
            mCameraAppUI.freezeScreenUntilPreviewReady();
            changedModule(getResources().getInteger(
                    R.integer.camera_mode_filter));
        }
    }

    public void changedModule(int moduleId) {
        Log.i(TAG, "changedModule mCurrentModeIndex = " + mCurrentModeIndex
                + " moduleId = " + moduleId);

        // TODO Auto-generated method stub
        if (mCurrentModeIndex == moduleId) {
            return;
        }

        mCameraAppUI.hideCaptureIndicator();
        closeModule(mCurrentModule);
        setModuleFromModeIndex(moduleId);
        mCameraAppUI.resetBottomControls(mCurrentModule, moduleId);
        mCameraAppUI.addShutterListener(mCurrentModule);
    ...
    public void onStopRecordVoiceClicked(View v) {
        if(mCurrentModule != null){
            if (mCurrentModule instanceof PhotoModule) {
                ...
                photoModule.onStopRecordVoiceClicked(v);
            } else if (mCurrentModule instanceof UcamFilterPhotoModule) {
                UcamFilterPhotoModule ucamFilterPhotoModule = (UcamFilterPhotoModule) mCurrentModule;
                ucamFilterPhotoModule.onStopRecordVoiceClicked(v);
            } else if (mCurrentModule instanceof SprdSceneryModule) {
                SprdSceneryModule sprdSceneryModule = (SprdSceneryModule) mCurrentModule;
                sprdSceneryModule.onStopRecordVoiceClicked(v);
            }
        }
    }
    ...
    public void onVoicePlayClicked(View v) {
    ...
    public int getVolumeControlStatus(){
        String index = mSettingsManager.getString(SettingsManager.SCOPE_GLOBAL,
