...
cmr_int sns_dev_i2c_read_seq(struct sensor_drv_context *sensor_cxt,
					SENSOR_I2C_SEQ_T_PTR i2c_tab)
{
	int ret = SENSOR_SUCCESS;
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);

	ret = ioctl(sensor_cxt->fd_sensor, SENSOR_IO_I2C_READ_SEQ, i2c_tab);
	if (0 != ret)
	{
		CMR_LOGE("_Sensor_Device_I2CRead failed, slave_addr=0x%x, ptr=0x%p, count=%d\n",
			i2c_tab->slave_addr, i2c_tab->i2c_data, i2c_tab->i2c_count);
...
cmr_int Sensor_ReadI2C_seq(cmr_u16 slave_addr, cmr_u8 *cmd, cmr_u16 wr_cnt, cmr_u16 rd_cnt)
{
	SENSOR_I2C_SEQ_T i2c_tab;
	cmr_int ret = SENSOR_SUCCESS;
	struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)sensor_get_dev_cxt();

	i2c_tab.slave_addr 	= slave_addr;
	i2c_tab.i2c_data	= cmd;
	i2c_tab.i2c_count	= wr_cnt > rd_cnt ? wr_cnt:rd_cnt;
	i2c_tab.wr_cnt = wr_cnt;
	i2c_tab.rd_cnt = rd_cnt;

	CMR_LOGV("Sensor_ReadI2C_seq, slave_addr=0x%x, ptr=0x%p, count=%d\n",
		i2c_tab.slave_addr, i2c_tab.i2c_data, i2c_tab.i2c_count);

	ret = sns_dev_i2c_read_seq(sensor_cxt, &i2c_tab);
...
void Sensor_SetExportInfo(struct sensor_drv_context *sensor_cxt)
{
	...
	for (i = SENSOR_MODE_COMMON_INIT; i < SENSOR_MODE_MAX; i++) {
		...
		if ((NULL != exp_info_ptr)
			...
			exp_info_ptr->raw_info_ptr->ioctrl_ptr->read_i2c_seq = Sensor_ReadI2C_seq;
#endif
...
cmr_int sns_identify(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	CMR_LOGI("sensor identifing %d , sensor_cxt %d", sensor_id ,sensor_cxt);
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	sensor_register_info_ptr = &sensor_cxt->sensor_register_info;
