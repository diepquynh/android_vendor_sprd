...
public final class GlideFilmstripManager {
     ...
     */
    public DrawableRequestBuilder<Uri> loadScreen(Context context, Uri uri, Key key, Size original) {
        Size size = clampSize(original, MAXIMUM_SMOOTH_PIXELS, getMaxImageDisplaySize());
     ...
     * SPRD:fix bug593147 modify gif load method @{
     */
    public GenericRequestBuilder<Uri, ?, ?, GlideDrawable> loadTinyThumb(Context context, Uri uri, Key key) {
        Size size = clampSize(TINY_THUMB_SIZE, MAXIMUM_SMOOTH_PIXELS,  getMaxImageDisplaySize());
        return Glide.with(context)
                .load(uri)
                .fitCenter()
