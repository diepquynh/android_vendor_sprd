...
void closeAcquireFDs(hwc_display_contents_1_t *list,int debug)
{
    ...
    if (list)
    {
        for(unsigned int i = 0; i < list->numHwLayers; i++)
        {
            ...
            {
                ALOGI_IF(debug,"<09> close src layers[%d].acq:%d",i,l->acquireFenceFd);
...
int HWCBufferSyncBuild(hwc_display_contents_1_t *list, int display)
{
    ...
    if (releaseFenceFd >= 0)
    {
        ...
        ALOGI_IF(s_debug,"%s[%d]  close fd:%d.",__func__,__LINE__,releaseFenceFd);
    ...
    {
        ALOGI_IF(s_debug,"%s[%d]  close fd:%d.",__func__,__LINE__,fenceData.release_fence_fd);
...
int BufferSyncBuild(hwc_display_contents_1_t *list, int releaseFenceFd, int retiredFenceFd)
{
    ...
    if (releaseFenceFd >= 0)
    {
        for(unsigned int i = 0; i < list->numHwLayers; i++)
        {
            ...
            if (l->releaseFenceFd < 0)
            {
                ...
                ALOGI_IF(s_debug,"<07> src.list[%d].rel:%d = dup(%d)",i, l->releaseFenceFd, releaseFenceFd);
            }
        }
    }
    ...
    if (retiredFenceFd >= 0)
    {
        if (list->retireFenceFd < 0)
        {
            ...
            ALOGI_IF(s_debug,"<08> src.retire:%d = dup(%d)", list->retireFenceFd, retiredFenceFd);
        }
    }
...
int HWCBufferSyncBuildForVirtualDisplay(hwc_display_contents_1_t *list, int *relFenceFd)
{
    HWC_TRACE_CALL;
...
int HWCBufferSyncReleaseForVirtualDisplay(hwc_display_contents_1_t *list)
{
    ...
    ret = sprd_fence_signal(device_type);
    ALOGI_IF(s_debug,"%s[%d]  close fd:%d.",__func__,__LINE__,releaseFenceFdForVirtualDisplay);
