...
public class AttachmentProgressDialogFragment extends DialogFragment {
    ...
    /*
     * Modify by SPRD for Bug:499631 Downloading attachment, make the lock
     * screen, and then unlock. the display is extracting, actually successful
     * extracted. 2015.07.03 Start
     */
    public boolean isAttachmentExistInDB() {
        if (mAttachment == null || mAttachment.contentUri == null) {
            return false;
        }
        Cursor cursor = null;
        try {
            cursor = getActivity().getContentResolver().query(mAttachment.contentUri, null, null,
                    null, null);
