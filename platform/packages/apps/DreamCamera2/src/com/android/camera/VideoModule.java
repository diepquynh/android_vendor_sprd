...
public class VideoModule extends CameraModule
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                ...
                if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                    ...
                    if (path.equals(currentPath)) {
                        ...
                        ToastUtil.showToast(mActivity, R.string.sdcard_remove, Toast.LENGTH_LONG);
                    } else {
                        ToastUtil.showToast(mActivity, R.string.externalstorage_removed, Toast.LENGTH_LONG);
                    }
                } else if (action.equals(Intent.ACTION_MEDIA_SCANNER_STARTED)) {
                    // SPRD: fix bug Bug 577352 Slow recording process, repeatedly tap the screen,
                    // the phone interface will now show time "slow record does not support the camera
                    ToastUtil.showToast(mActivity, R.string.wait, Toast.LENGTH_LONG);
                }
            }
        }
    }
    ...
    private boolean isSlowMotionOn() {
        ...
        Log.i(TAG, "slow_motion = " + slow_motion + " boolean = " + (slow_motion == 1));
        // SPRD: Fix bug 569496 front camera and slowmotion do mutex
        return isCameraBackFacing() ? !(slow_motion == 1) : false;
    }
    /* @} */
        ...
        */
        ...
        if (mMediaRecorderRecording) {
            if (!mSnapshotInProgress && !isSlowMotionOn() && !isRecordingIn4k()
                ...
                // SPRD: fix bug Bug 577352 Slow recording process, repeatedly tap the screen,
                // the phone interface will now show time "slow record does not support the camera
                ToastUtil.showToast(mActivity, R.string.slow_motion_does_not_support_take_snap_shot, Toast.LENGTH_LONG);
            }
            return;
        }
    ...
    public void onCameraAvailable(CameraProxy cameraProxy) {
        ...
        if (!isSlowMotionOn()) {
            ...
            // SPRD: fix bug Bug 577352 Slow recording process, repeatedly tap the screen,
            // the phone interface will now show time "slow record does not support the camera
            ToastUtil.showToast(mActivity, R.string.slow_motion_does_not_support_zoom_change, Toast.LENGTH_LONG);
        }
            ...
            case KeyEvent.KEYCODE_VOLUME_DOWN:
                ...
                if (volumeStatus == 1 || keyCode == KeyEvent.KEYCODE_CAMERA) {
                    if (event.getRepeatCount() == 0) {
                        /**
                         * SPRD: fix bug 473602 add half-press @{ onShutterButtonClick(); @}
                         */
                        return true;
                    }
    ...
    private void initializeRecorder() {
        ...
        for (VolumeInfo vol : vols) {
            if (vol != null) {
                ...
                if ("vfat".equals(fsType) && storagePath.startsWith(path)) {
                    ToastUtil.showToast(mActivity, R.string.video_maximum_size_limit, Toast.LENGTH_LONG);
                    vFatType = true;
                    break;
                }
            }
        }
        mMediaRecorder.setParam64BitFileOffset(!vFatType);
        /* @} */
        ...
        if (mVideoFileDescriptor == null) {
            ...
            if (duration > 0) {
                ...
                if (mCaptureTimeLapse) {
                    ...
                    int slow_motion = Integer.valueOf(sm.getString(SettingsManager.SCOPE_GLOBAL,
                            Keys.KEY_VIDEO_SLOW_MOTION));
                    if (slow_motion != 1 && isCameraBackFacing()) { // SPRD: Fix bug 573812
    ...
    public void onInfo(MediaRecorder mr, int what, int extra) {
        if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_DURATION_REACHED) {
            ...
            /*
             * SPRD:SPRD: fix bug Bug 577352 Slow recording process, repeatedly tap the screen,
             * the phone interface will now show time "slow record does not support the camera Modify for ai detect" @{
             * original code
             *
            // Show the toast.
             ...
             */
            ToastUtil.showToast(mActivity, R.string.video_reach_size_limit, Toast.LENGTH_LONG);
            /* @} */
        }
    ...
    private void updateParametersEOIS() {
        ...
        if(mCameraSettings == null || mActivity.isDestroyed()){
            return;
        }

        if(isCameraFrontFacing() && CameraUtil.isEOISDvFrontEnabled()){
            SettingsManager settingsManager = mActivity.getSettingsManager();
            Log.i(TAG, "front video eois = " + settingsManager.getBoolean(
        ...
        if(!isCameraFrontFacing() && CameraUtil.isEOISDvBackEnabled()){
            SettingsManager settingsManager = mActivity.getSettingsManager();
            Log.i(TAG, "back video eois = " + settingsManager.getBoolean(
                    SettingsManager.SCOPE_GLOBAL,
    ...
    private void setSlowmotionParameters() {
        ...
        String slow_motion = settingsManager.getString(SettingsManager.SCOPE_GLOBAL,
                Keys.KEY_VIDEO_SLOW_MOTION);
        // SPRD: Fix bug 569496 front camera and slowmotion do mutex
        if (mIsVideoCaptureIntent || isCameraFrontFacing()) {
    ...
    private void updateParametersColorEffect() {
        ...
        Log.d(TAG, "update ColorEffect = " + settingsManager.getString(
                SettingsManager.SCOPE_GLOBAL, Keys.KEY_VIDEO_COLOR_EFFECT));
        CameraCapabilities.ColorEffect colorEffect = stringifier.colorEffectFromString(settingsManager.getString(
                SettingsManager.SCOPE_GLOBAL, Keys.KEY_VIDEO_COLOR_EFFECT));
