...
abstract public class ContactEditorBaseActivity extends ContactsActivity
     ...
     * @}
     */
    private static Context mSingle;
    ...
    public void onCreate(Bundle savedState) {
        ...
        if(isInMultiWindowMode()){
            if(mSingle == null){
                ...
                finish();
                Toast.makeText(getApplication(), R.string.edit_only, Toast.LENGTH_LONG).show();
            }
        }
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case MESSAGE_LOCALE_CHANGED:
                        // SPRD:Bug 557324 finish activity when it's top // activity
                        if (isTopActivity()) {
                            finish();
                        }
                    case MESSAGE_CONFIGURATION_CHANGED:
                        Configuration config = getResources().getConfiguration();
                        if (config.fontScale != mFontScale && isTopActivity()) {
                            finish();
                        }
                    default:
                        break;
    ...
    // add for SPRD:Bug557324 add topActivity judgement
    private boolean isTopActivity() {
        ActivityManager am = (ActivityManager) getSystemService(ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        boolean mIsTopActivity = (cn != null && cn.getClassName() != null) ? cn.getClassName()
                .contains("ContactEditor") : false;
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.d(TAG,"I'm destroy");
        if(mSingle == this){
            mSingle = null;
            Log.d(TAG,"mSingle is null");
        }
    ...
    protected final ContactEditorBaseFragment.Listener  mFragmentListener =
        ...
        public void onSaveFinished(Intent resultIntent) {
            if (mFinishActivityOnSaveCompleted) {
                ...
                /**
                 * SPRD:Bug 607203 splite --> normal mode and save will happen crash
                 * @{
                 */
                try {
                    ImplicitIntentsUtil.startActivityInApp(
                            ContactEditorBaseActivity.this, resultIntent);
                } catch (Exception e) {
    ...
    public void onMultiWindowModeChanged(boolean isInMultiWindowMode){
        if(isInMultiWindowMode){
            Log.d(TAG,"into multi windows");
        }else{
            Log.d(TAG,"out of  multi windows");
            mSingle = null;
        }
    }
