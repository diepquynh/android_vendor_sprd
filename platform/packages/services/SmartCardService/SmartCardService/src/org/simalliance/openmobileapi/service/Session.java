...
public class Session {
    ...
    public Session(Terminal reader, int readerId , Context context) {
        mReader = reader;
        mReaderId = readerId;
        mIsClosed = false;
        mContext = context;
        Log.d(TAG , "mReader.addr : " + mReader.toString() + " , mReaderId : " + mReaderId);
    }
    ...
    private void closeChannels() {
        Log.d(SmartcardService.LOG_TAG , "session.addr : " + this.toString());
        synchronized (mLock) {
            while (mChannels.size() > 0) {
                Log.d(SmartcardService.LOG_TAG , "channel.number : " + mChannels.get(0).getChannelNumber() + ", channel.addr : " + mChannels.get(0).toString());
    ...
    public ISmartcardServiceChannel openBasicChannel(
        ...
        /* SPRD add for following OMAPI spec 2.03 @{
        ...
        */
        if(mReader.getName().startsWith("SIM")) {
            Log.w(SmartcardService.LOG_TAG, "open BasicChannel on UICC, just return null.");
            return null;
        }
        /*@}*/
