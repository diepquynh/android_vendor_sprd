...
******************************************************************************/
int setup_packet(CMD_TYPE msg, char *buffer, int offset, int data_size,
  ...
  switch (msg) {
    ...
    case BSL_CMD_MIDST_DATA:
      if (flag == BSL_UART_PACKET) {
        ...
        *((unsigned short *)&buffer[4]) = 0;  // boot_checksum((const unsigned
                                              // char
...
int send_connect_message(int fd, int flag) {
  ...
  if (flag == 0) {
    translated_size =
        translate_packet((char *)send_buffer, (char *)&raw_buffer[4], size);
    do {
...
int send_end_message(int fd, int flag) {
  ...
  MODEM_LOGD("END_NACK:%x %x %x %x %x %x %x %x\n", raw_buffer[0], raw_buffer[1],
             raw_buffer[2], raw_buffer[3], raw_buffer[4], raw_buffer[5],
             raw_buffer[6], raw_buffer[7]);
  return -1;
}
/******************************************************************************
...
int send_data_message(int fd, char *buffer, int data_size, int flag,
  ...
  if (flag == 0) {
    ...
    translated_size =
        translate_packet((char *)send_buffer, (char *)&uart_buffer[4], size);
    do {
      retval = write(fd, send_buffer, translated_size);
      if (retval > 0) translated_size -= retval;
    } while (translated_size > 0);
...
int uart_send_change_spi_mode_message(int fd, int max_tran_sz) {
  ...
  *(unsigned long *)&raw_buffer[8] = cpu2be32(max_tran_sz);
  size =
      setup_packet(BSL_CMD_SWITCH_MODE, raw_buffer, 8, 0, BSL_UART_PACKET, 0);
  translated_size =
      translate_packet((char *)send_buffer, (char *)&raw_buffer[4], size);
  do {
