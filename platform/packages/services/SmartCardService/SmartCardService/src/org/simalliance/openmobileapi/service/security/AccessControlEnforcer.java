...
public class AccessControlEnforcer {
    ...
    public AccessControlEnforcer( Terminal terminal ) {
        ...
        Log.d(ACCESS_CONTROL_ENFORCER, "Create AccessRuleCache");
    }
    ...
    public synchronized void checkCommand(Channel channel, byte[] command) {
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::checkCommand ca = " + ca.toString());

        String reason = ca.getReason();
        if (reason.length() == 0) {
            reason = "Command not allowed!";
        }
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::checkCommand ca.getAccess() = " + ca.getAccess());
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::checkCommand (ca.isUseApduFilter()) = " + (ca.isUseApduFilter()));
        ...
        if (ca.isUseApduFilter()) {
            ...
            for (ApduFilter ac : accessConditions) {
                if (CommandApdu.compareHeaders(command, ac.getMask(), ac.getApdu())) {
                    Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::checkCommand PASS");
                    return;
                }
            }
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::checkCommand finally ");
    }
    ...
    public ChannelAccess setUpChannelAccess(
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::setUpChannelAccess Enter ");
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::setUpChannelAccess mUseAra = " + mUseAra);
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::setUpChannelAccess mUseArf = " + mUseArf);
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::setUpChannelAccess mFullAccess = " + mFullAccess);
        if( mUseAra || mUseArf ){

            try {
                Log.v(SmartcardService.LOG_TAG, "setUpChannelAccess packageName = " + packageName);
        ...
        Log.d(SmartcardService.LOG_TAG, "setUpChannelAccess channelAccess is " + channelAccess);
        if (channelAccess != null){
            Log.d(SmartcardService.LOG_TAG, "setUpChannelAccess channelAccess info is " + channelAccess.toString());
        }
        ...
        if( channelAccess == null // precautionary check
            ...
            if( mFullAccess ){
                ...
                Log.v(SmartcardService.LOG_TAG, "we reuse the initial channel access, channelAccess = " + channelAccess.toString());
        ...
        Log.v(SmartcardService.LOG_TAG, "setUpChannelAccess Exit" + channelAccess.toString());
    ...
    private synchronized ChannelAccess internal_setUpChannelAccess(byte[] aid, String packageName) {
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::internal_setUpChannelAccess Enter packageName = " + packageName);
        try {
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::internal_setUpChannelAccess Exit channelAccess = " + channelAccess.toString());
    ...
    public ChannelAccess getAccessRule( byte[] aid, Certificate[] appCerts ) throws SecurityException, CertificateEncodingException {
        ...
        Log.v(SmartcardService.LOG_TAG, "AccessControlEnforcer::getAccessRule, mRulesRead = " + mRulesRead);
        // if read all is true get rule from cache.
        if( mRulesRead ){
            // get rules from internal storage
            Log.v(SmartcardService.LOG_TAG, "get rules from internal storage");
        ...
        if( channelAccess == null ){
            Log.v(SmartcardService.LOG_TAG, "no rule found then create the ac with all access denied");
    ...
    private synchronized boolean[] internal_isNFCEventAllowed(byte[] aid, String[] packageNames) {
        ...
        int i=0;
        ChannelAccess channelAccess;
        Log.v(SmartcardService.LOG_TAG, "internal_isNFCEventAllowed Enter");
        for( String packageName : packageNames ) {
            // estimate SHA-1 hash value of the device application's certificate.
                ...
                try {
                    ...
                    Log.v(SmartcardService.LOG_TAG, "internal_isNFCEventAllowed getAccessRule begin for packageName= " + packageName);
                    channelAccess = getAccessRule(aid, appCerts);
                    mNfcEventFlags[i] = (channelAccess.getNFCEventAccess() == ChannelAccess.ACCESS.ALLOWED);
                    Log.v(SmartcardService.LOG_TAG, "internal_isNFCEventAllowed getAccessRule end for packageName = " + packageName);
                } catch (Exception e) {
        ...
        Log.v(SmartcardService.LOG_TAG, "internal_isNFCEventAllowed Exit");
