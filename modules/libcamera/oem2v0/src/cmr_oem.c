...
void camera_grab_handle(cmr_int evt, void* data, void* privdata)
{
	...
	if ((0 != cxt->snp_cxt.channel_bits) && (TAKE_PICTURE_NEEDED == camera_get_snp_req((cmr_handle)cxt))
		...
		CMR_LOGI("new_3A_flag = %d , sensor format = %d" , new_3A_flag , sensor_info.image_format);
		if (1 == new_3A_flag && SENSOR_IMAGE_FORMAT_RAW == sensor_info.image_format) {
			//for new 3A
			...
			if (isp_timestamp && (cxt->timestamp_list[i] > isp_timestamp)) {
				if (0 == frame->hdr_start) {
					...
					*/
					...
					frame->ev_skip_num -= compare_num;
					if (frame->ev_skip_num < 0) {
						frame->ev_skip_num = 0;
					}
				}
			...
			if(ipm_cxt->frm_num == ipm_cxt->hdr_num) {
				...
				cmr_snapshot_hdr_sync_wait(cxt->snp_cxt.snapshot_handle);
...
cmr_int camera_local_start_snapshot(cmr_handle oem_handle, enum takepicture_mode mode, cmr_uint is_snapshot)
{
	...
	if (CAMERA_ISP_SIMULATION_MODE == mode) {
		...
		struct lsc_isp_addr lsc_addr = {0};
		...
		if (cxt->isp_malloc_flag == 0) {
			lsc_addr.lsc_buf_num = ISP_LSC_BUF_NUM;
			lsc_addr.lsc_buf_size = ISP_LSC_BUF_SIZE;
			if (cxt->hal_malloc) {
				cxt->hal_malloc(CAMERA_ISP_LSC, &lsc_addr.lsc_buf_size, &lsc_addr.lsc_buf_num,
						&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, cxt->client_data);
				cxt->isp_malloc_flag = 1;
			} else {
				ret = -CMR_CAMERA_NO_MEM;
		...
		lsc_addr.lsc_phys_addr = cxt->isp_lsc_phys_addr;
		lsc_addr.lsc_vir_addr = cxt->isp_lsc_virt_addr;
		ret = isp_ioctl(isp_cxt->isp_handle, ISP_CTRL_TOOL_SIMULATOR_GET_LSC_ADDR, (void*)&lsc_addr);
