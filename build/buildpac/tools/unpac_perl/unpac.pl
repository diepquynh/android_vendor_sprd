...
sub GetPacHeaderInfo{
	$szVersion 					= substr($PacHeaderBuf,0   ,48);			# 48 bytes
	$dwSize	 						= substr($PacHeaderBuf,48  ,4);				# 4 bytes
	...
	$ProductCfg = $ProductCfg."ProductName=".$szPrdName."\n";
...
sub GetFileInfo
{
	...
	my $nFileSize;			# file size,4 bytes
	...
	my $dwDataOffset;   # the offset from the packet file header to this file data,4 bytes
  ...
  $dwSize				= substr($data,0,4);					# 4 bytes
	...
	$szFileVersion= substr($data,1028,512);			# 512 bytes
	$nFileSize		= substr($data,1540,4);				# 4 bytes
	$nFileFlag		= substr($data,1544,4);      	# 4 bytes                
	$nCheckFlag		= substr($data,1548,4);     	# 4 bytes									
	$dwDataOffset	= substr($data,1552,4);   		# 4 bytes
	...
	$nFileSize 			= UnPack2DWORD($nFileSize);	
	$nFileFlag 			= UnPack2DWORD($nFileFlag);	
	$nCheckFlag 		= UnPack2DWORD($nCheckFlag);	
	$dwDataOffset		= UnPack2DWORD($dwDataOffset);	
	...
	print "FileSize=".$nFileSize."\n";
	print "FileFlag=".$nFileFlag."\n";
	print "CheckFlag=".$nCheckFlag."\n";
	print "DataOffset=".$dwDataOffset."\n";
...
sub ReleaseDLFile{
    ...
    seek PACFILE,$dwDataOffset,SEEK_SET;
		...
		{
			$ProductCfg= $ProductCfg."config=".$szFileName."\n";
		}
