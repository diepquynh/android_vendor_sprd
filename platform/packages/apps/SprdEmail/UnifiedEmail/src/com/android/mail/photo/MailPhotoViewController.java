...
public class MailPhotoViewController extends PhotoViewController {
        ...
        if (attachment != null && attachment.canSave()) {
            ...
            if (attachment.destination == UIProvider.AttachmentDestination.CACHE
                ...
                AttachmentUtilities.copyAttachmentFromInternalToExternal(
                        ...
                        attachment.uri, attachment.contentUri, false, false, null);
                /* @} */
            } else {
                mActionHandler.setAttachment(attachment);
                mActionHandler
    ...
    private void printAttachment() {
        ...
        try {
            ...
            /* SPRD: Add for bug564018, use HandlerThread and get looper to be used for print @{ */
            Log.d(LOG_TAG, "<printSelectedImage> printBitmap start");
            int tid = mHandlerThread.getThreadId();
            Looper looper = mHandlerThread.getLooper();
            Log.d(LOG_TAG, "<printSelectedImage> mHanderThread id:" + tid + ", looper:" + looper);
            printHelper.printBitmap(PrintUtils.buildPrintJobName(context, attachment.getName()),
                    attachment.contentUri, new PrintHelper.OnPrintFinishCallback() {
                        @Override
                        public void onFinish() {
                            Log.d(LOG_TAG, "<printSelectedImage> printBitmap finish");
                        }
                    }, looper);
            /* @} */
