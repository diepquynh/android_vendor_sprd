...
//                        mPanoBg.getHeight() - mCaptureProgressBar.getHeight());
    ...
    private void createContentView() {
        ...
        if (mActivity.getOrientationManager() != null) {
            ((RotateLayout) mRootView.findViewById(R.id.pano_progress_rotate_layout)).setOrientation(
                    mActivity.getOrientationManager().getDeviceOrientation().getDegrees(), true);
        }

        // SPRD: Fix bug 575389 and 581078
        if (mActivity.getOrientationManager() != null) {
            RotateLayout panoProgressRotateLayout = (RotateLayout)mRootView.findViewById(R.id.pano_progress_rotate_layout);
            if (panoProgressRotateLayout != null) {
                panoProgressRotateLayout.setOrientation(
                        mActivity.getOrientationManager().getDeviceOrientation().getDegrees(), true);
            }
            RotateLayout panoModuleReviewLayout = (RotateLayout)mRootView.findViewById(R.id.pano_module_review_layout);
            if (panoModuleReviewLayout != null) {
                panoModuleReviewLayout.setOrientation(
                        mActivity.getOrientationManager().getDeviceOrientation().getDegrees(), true);
            }
        }
    ...
    private void initFilterMakeupButton() {
        ...
        if (UCamUtill.isUcamFilterEnable()) {
            ImageView filter = (ImageView) mActivity.getModuleLayoutRoot().findViewById(R.id.btn_filter_button);
            if (filter != null) {
