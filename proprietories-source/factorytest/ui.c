...
extern void gr_draw_point(int x, int y);
...
void ui_init(void)
{
	...
	text_col = text_row = 0;
	text_tp_rows =   (gr_fb_height()-(gr_fb_height()>>3)) / CHAR_HEIGHT - 2;
	text_rows = gr_fb_height() / CHAR_HEIGHT - 1;

	if (text_rows > MAX_ROWS)
		text_rows = MAX_ROWS;

	if (text_tp_rows > MAX_ROWS)
		text_tp_rows = MAX_ROWS;
	...
	gr_font_size(&cwidth, &cheight,&ewidth,&eheight);
		...
		page_num = (gr_fb_height()-(gr_fb_height()>>3)) / CHAR_HEIGHT - 2;
	#else
	...
	pthread_t t1,t2;
	pthread_create(&t1, NULL, progress_thread, NULL);
		...
		for (ptr = buf; *ptr != '\0'; ++ptr)
		{
			if (*ptr == '\n' || text_col >= text_cols)
			{
				...
				if (text_row == text_top)
					text_top = (text_top + 1) % text_tp_rows;
			}
...
int ui_show_text(int row, int col, const char* text)
{
	...
	int clen = 0;
	int max_col = (gr_fb_width() -CHAR_WIDTH/2) / CHAR_WIDTH;	//remain half char_width
	char temp[256];
	const char* pos = text;
	int cur_row = row;
