...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    /* SPRD: Fix bug316701, for slide performace and SlideSettings.apk. @{ */
    ...
    /* SPRD: Feature 248819,porting this two variables for getScaledMeasuredWidth() from SprdLauncher(Android4.1). @{ */
    ...
    /* @SPRD: 3D transfer @{*/
//    protected Bitmap[] mBitmapsBuffer;
//    protected int mBufferCurPage;
//    protected boolean mLoadCompleted;
     ...
     */
    protected void init() {
        mPerformanceTest = Log.isPerformanceTestable();
        ...
        */
        ...
        mTouchSlop = SystemProperties.getInt(KEY_TOUCH_SLOP, DEFAULT_TOUCH_SLOP_FOR_SLIDE_OPTIMIZATION);
        int tempFlingThresholdVelocity = SystemProperties.getInt(KEY_FLING_THRESHOLD_VELOCITY, FLING_THRESHOLD_VELOCITY);
        int tempMinFlingVelocity = SystemProperties.getInt(KEY_MIN_FLING_VELOCITY, MIN_FLING_VELOCITY);
        int tempMinSnapVelocity = SystemProperties.getInt(KEY_MIN_SNAP_VELOCITY, MIN_SNAP_VELOCITY);
        ...
        mDampingCoefficient = (float) SystemProperties.getInt(KEY_DELAY_TIME, DEFAULT_SLIDE_DAMPING_COEFFICIENT) / 10;
        mPageSnapAnimDuration = SystemProperties.getInt(KEY_SLIP_VELOCITY, PAGE_SNAP_ANIMATION_DURATION);
        /* @} */
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mFreeScroll) {
                /* SPRD: Fix bug283646 , @{ */
                final int currScrollX = (int) (mScroller.getCurrX() * (1 / getScaleX()));
                if ((currScrollX  >= mFreeScrollMaxScrollX || currScrollX  <= 0) && !mScroller.isFinished()) {
                    Log.i(TAG, "currScrollX = " + currScrollX + "; and mFreeScrollMaxScrollX = " + mFreeScrollMaxScrollX);
                    mScroller.abortAnimation();
                }
                /* @} */
            }
    ...
    public void computeScroll() {
        computeScrollHelper();
        if (mPerformanceTest) {
            Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time9 = " + System.currentTimeMillis());
        }
    }
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        ...
        if (mCurrentPage >= 0 && mCurrentPage < getPageCount()) {
            childView = getPageAt(mCurrentPage);
            if (childView != null) {
                childView.addFocusables(views, direction, focusableMode);
            }
        }
        if (direction == View.FOCUS_LEFT) {
            if (mCurrentPage > 0) {
                childView = getPageAt(mCurrentPage - 1);
                if (childView != null) {
                    childView.addFocusables(views, direction, focusableMode);
                }
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                if (mPerformanceTest) {
                    Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time2 = " + System.currentTimeMillis());
                }
                /*
                ...
                if (mActivePointerId != INVALID_POINTER) {
                    /* SPRD: Fix bug316701, avoid firt move ev lost. @{ */
            ...
            case MotionEvent.ACTION_DOWN: {
                if (mPerformanceTest) {
                    Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time1 = " + System.currentTimeMillis());
                }
                ...
                /* SPRD: fix bug269770 the mTouchSlog which is 24 at 7715 is too large for us, 2 is already well. @{ */
                int mTempSlop = Math.min(mTouchSlop, 2);
                final boolean finishedScrolling = (mScroller.isFinished() || xDist < mTempSlop);
                /* @} */
         ...
         * drag mode.
         */
        if (mTouchState != TOUCH_STATE_REST && ((action & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_MOVE)) {
            if (mPerformanceTest) {
                Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time3 = " + System.currentTimeMillis());
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mPerformanceTest) {
                Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time4 = " + System.currentTimeMillis());
            }
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (Math.abs(deltaX) >= 1.0f) {
                    ...
                    if (!mDeferScrollUpdate) {
                        if (mPerformanceTest) {
                            Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time5 = " + System.currentTimeMillis());
                        }
        ...
        case MotionEvent.ACTION_UP:
            if (mPerformanceTest) {
                Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time6 = " + System.currentTimeMillis());
            }
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                /* SPRD: fix bug332013, NullPointerException. @{ */
                // final int pageWidth = getPageAt(mCurrentPage).getMeasuredWidth();
                int pageWidth = 0;
                for (int i = 0; i < getChildCount(); i++) {
                    View view = getChildAt(i);
                    if (view != null) {
                        pageWidth = view.getMeasuredWidth();
                        break;
                    } else {
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                        ...
                        if (mPerformanceTest) {
                            Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time7 = " + System.currentTimeMillis());
                        }
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate) {
        ...
        if (mPerformanceTest) {
            Log.startPerfTracking(this.getClass().getName() + " LauncherSlide.time8 = " + System.currentTimeMillis());
        }
    ...
    /**
     * SPRD: Feature 248819,Porting this function from SprdLauncher(Android4.1). @{
     *
     * @param child
     * @return int
     */
...
//    /**
//     * Transfer to page and load next bitmap.
//     * @param pageDelta Transfer to left/right page, the value must be one of -1/0/1.
//     */
//    public boolean loadBitmapsBuffer(boolean supportScroll) {
//        final int pageCount = getChildCount();
//        if (pageCount < 2) {
//            mBufferCurPage = -1;
...
//        if (mLoadCompleted) {
//            if (mBufferCurPage != curPage) {
//                final int delta = mBufferCurPage - curPage;
//                Bitmap tmp;
//                if (pageCount == 2) {
//                    tmp = mBitmapsBuffer[1 + delta];
//                    mBitmapsBuffer[1 + delta] = mBitmapsBuffer[1];
//                    mBitmapsBuffer[1] = mBitmapsBuffer[1 - delta];
//                    mBitmapsBuffer[1 - delta] = tmp;
//                    mBufferCurPage = curPage;
//                    return true;
...
//                    if (delta > 0) {
//                        if (curPage > 0) {
//                            cellLayout = (CellLayout) getPageAt(curPage - delta);
//                            mBitmapsBuffer[1 - delta] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//                            if (null != tmp) {
//                                tmp.recycle();
//                                tmp = null;
//                            }
//                        } else if (supportScroll) {
//                            if (pageCount == 3) {
//                                mBitmapsBuffer[1 - delta] = tmp;
//                            } else {
//                                cellLayout = (CellLayout) getPageAt(pageCount - 1);
//                                mBitmapsBuffer[1 - delta] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//                                if (null != tmp) {
//                                    tmp.recycle();
//                                    tmp = null;
...
//                                mBitmapsBuffer[1 - delta] = tmp;
//                            } else {
//                                cellLayout = (CellLayout) getPageAt(0);
//                                mBitmapsBuffer[1 - delta] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//                                if (null != tmp) {
//                                    tmp.recycle();
//                                    tmp = null;
...
//            }
//        }
//        Log.w(TAG, "-----------loadBitmapsBuffer " + curPage + "-----------");
//        if (mBitmapsBuffer == null) {
//            mBitmapsBuffer = new Bitmap[3];
//        } else if (mBitmapsBuffer.length < 3 && pageCount > 2) {
//            // Impossible code
//            for (Bitmap bitmap : mBitmapsBuffer) {
//                if (null != bitmap) {
//                    bitmap.recycle();
//                    bitmap = null;
//                }
//            }
//            Log.w(TAG, "loadBitmapsBuffer recycle all bitmaps - " + mBitmapsBuffer.length);
//            mBitmapsBuffer = new Bitmap[3];
//        }
//        CellLayout cellLayout = null;
//        if (curPage > 0) {
//            cellLayout = (CellLayout) getPageAt(curPage - 1);
//            mBitmapsBuffer[0] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//            Log.w(TAG, "LoadBitmap 0: " + (curPage - 1));
//        } else if (supportScroll) {
//            cellLayout = (CellLayout) getPageAt(pageCount - 1);
//            mBitmapsBuffer[0] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//            Log.w(TAG, "LoadBitmap 0: " + (pageCount - 1));
//        } else {
//            if (null != mBitmapsBuffer[0]) {
//                mBitmapsBuffer[0].recycle();
//                mBitmapsBuffer[0] = null;
//            }
//            Log.w(TAG, "recycle Bitmap: 0 " + (curPage - 1));
//        }
//
//        cellLayout = (CellLayout) getPageAt(curPage);
//        mBitmapsBuffer[1] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//        Log.w(TAG, "LoadBitmap 1: " + curPage);
//
//        if (curPage == pageCount - 1) {
//            if (supportScroll) {
//                cellLayout = (CellLayout) getPageAt(0);
//                mBitmapsBuffer[2] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//                Log.w(TAG, "LoadBitmap 2: 0");
//            } else if (null != mBitmapsBuffer[2]) {
//                mBitmapsBuffer[2].recycle();
//                mBitmapsBuffer[2] = null;
//                Log.w(TAG, "recycle Bitmap 2: " + (curPage + 1));
//            }
//        } else {
//            cellLayout = (CellLayout) getPageAt(curPage + 1);
//            mBitmapsBuffer[2] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//            Log.w(TAG, "LoadBitmap 2: " + (curPage + 1));
//        }
//        mBufferCurPage = curPage;
//        mLoadCompleted = true;
//        return true;
...
//     * @param pageDelta Transfer to left/right page, the value must be one of -1/0/1.
//     */
//    public void switchToPage(int pageDelta, boolean supportScroll) {
//        if (0 == pageDelta) {
//            return;
//        }
//        final int curPage = getCurrentPage();
//        snapToPageImmediately(curPage + pageDelta);
//        // Log.w(TAG, "snapToPage: " + (curPage + page));
//        if (null == mBitmapsBuffer) {// Fix bug 266820, in monkey test.
//            return;
//        }
//        final int pageCount = getChildCount();
//        if (pageCount == 2) {
//            mBitmapsBuffer[1 - pageDelta] = mBitmapsBuffer[1];
//            mBitmapsBuffer[1] = mBitmapsBuffer[1 + pageDelta];
//            mBitmapsBuffer[1 + pageDelta] = null;
//        } else if (pageCount == 3) {
//            Bitmap tmp = mBitmapsBuffer[1 - pageDelta];
//            mBitmapsBuffer[1 - pageDelta] = mBitmapsBuffer[1];
//            mBitmapsBuffer[1] = mBitmapsBuffer[1 + pageDelta];
//            mBitmapsBuffer[1 + pageDelta] = tmp;
//        } else if (pageCount > 3) {
//            if (mBitmapsBuffer[1 - pageDelta] != null) {
//                mBitmapsBuffer[1 - pageDelta].recycle();
//                mBitmapsBuffer[1 - pageDelta] = null;
//                Log.w(TAG, "recycle bitmap - " + (1 - pageDelta));
//            }
//            mBitmapsBuffer[1 - pageDelta] = mBitmapsBuffer[1];
//            mBitmapsBuffer[1] = mBitmapsBuffer[1 + pageDelta];
//            int nextPage = curPage + pageDelta + pageDelta;
//            if (supportScroll || (nextPage >= 0 && nextPage < pageCount)) {
//                loadBitmapBuffer(1 + pageDelta, nextPage);
//            } else {
//                mBitmapsBuffer[1 + pageDelta] = null;
//            }
//        }
//        mBufferCurPage = curPage + pageDelta;
//    }
//
//    public Bitmap[] getBitmapsBuffer(boolean supportScroll) {
//        if (null == mBitmapsBuffer  || mBufferCurPage!=getCurrentPage()) {
//            loadBitmapsBuffer(supportScroll);
//        }
//        return mBitmapsBuffer;
//    }
//
//    public void freeBitmapBuffer() {
//        mLoadCompleted = false;
//        if (mBitmapsBuffer != null) {
//            for (Bitmap bmp : mBitmapsBuffer) {
//                if (null != bmp) {
//                    bmp.recycle();
//                    bmp = null;
//                }
//            }
//            Log.w(TAG, "freeBitmapBuffer - " + mBitmapsBuffer.length);
//            mBitmapsBuffer = null;
//        }
//    }
//
//    protected void loadBitmapBuffer(int index, int page) {
//        final int pageCount = getChildCount();
//        if(pageCount < 2){
//            return;
//        }
...
//            page -= pageCount;
//        }
//        Log.w(TAG, "loadBitmapBuffer " + index + ", " + page);
//        CellLayout cellLayout = (CellLayout) getPageAt(page);
//        mBitmapsBuffer[index] = cellLayout.getViewBitmap(0, 0, cellLayout.getWidth(), cellLayout.getHeight());
//        mLoadCompleted = true;
//    }
//    /* @} */
//
