...
enum { MODEM_SOFT_RESET = 0x1, MODEM_HARD_RESET, MODEM_MAX };
...
#define MODEM_ENTRY_MAGIC0    0x80400000
#define MODEM_ENTRY_MAGIC1    0x80400001
...
char test_buffer[HS_PACKET_SIZE + MAX_PREFIX_LEN + 1] = {0};
...
static int modem_images_count =
    12;  // modem_images_count and download_image_info[] must keep matched!
struct image_info download_image_info[] = {
    {
        // fdl
        "/dev/block/platform/sprd-sdhci.3/by-name/fdl",
        "/dev/block/platform/sprd-sdhci.3/by-name/fdl", 0x4000, 0x20000000,
        MODE_ALL,
    },
    {
        // PARM DSP
        "/dev/block/platform/sprd-sdhci.3/by-name/tddsp",
        "/dev/block/platform/sprd-sdhci.3/by-name/tddsp", PARM_DSP_SIZE,
        PARM_DSP_OFFSET, MODE_SVLTE,
    },
    {
        // PARM
        "/dev/block/platform/sprd-sdhci.3/by-name/tdmodem",
        "/dev/block/platform/sprd-sdhci.3/by-name/tdmodem", PARM_CODE_SIZE,
        PARM_CODE_OFFSET, MODE_SVLTE,
    },
    {
        //  PARM fixvn
        "/dev/block/platform/sprd-sdhci.3/by-name/tdfixnv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/tdfixnv1", PARM_FIX_NV_SIZE,
        PARM_FIX_NV_OFFSET, MODE_SVLTE,
    },
    {
        //  PARM runtimevn
        "/dev/block/platform/sprd-sdhci.3/by-name/tdruntimenv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/tdruntimenv1",
        PARM_RUN_NV_SIZE, PARM_RUN_NV_OFFSET, MODE_SVLTE,
    },
    {
        //  cmdline
        "/proc/cmdline", "/proc/cmdline", PARM_CMD_LINE_SIZE,
        PARM_CMD_LINE_OFFSET, MODE_SVLTE,
    },
    /*{ //ca5 DSP
     ...
     },*/
    {
        ...
        "/dev/block/platform/sprd-sdhci.3/by-name/l_modem", CA5_CODE_SIZE,
        CA5_CODE_OFFSET, MODE_SVLTE,
    },
    {
        // ca5 fixvn
        "/dev/block/platform/sprd-sdhci.3/by-name/l_fixnv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/l_fixnv1", CA5_FIX_NV_SIZE,
        CA5_FIX_NV_OFFSET, MODE_SVLTE,
    },
    {
        // ca5 runtimevn
        "/dev/block/platform/sprd-sdhci.3/by-name/l_runtimenv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/l_runtimenv1",
        CA5_RUN_NV_SIZE, CA5_RUN_NV_OFFSET, MODE_SVLTE,
    },
    {
        // lte tg DSP
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_tdsp",
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_tdsp", CA5_TG_DSP_SIZE,
        CA5_TG_DSP_OFFSET, MODE_TCSFB,
    },
    {
        //  ca5
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_modem",
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_modem", CA5_CODE_SIZE,
        CA5_CODE_OFFSET, MODE_TCSFB,
    },
    {
        // ca5 fixvn
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_fixnv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_fixnv1", CA5_FIX_NV_SIZE,
        CA5_FIX_NV_OFFSET, MODE_TCSFB,
    },
    {
        // ca5 runtimevn
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_runtimenv1",
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_runtimenv1",
        CA5_RUN_NV_SIZE, CA5_RUN_NV_OFFSET, MODE_TCSFB,
    },
    {
        //  cmdline
        "/proc/cmdline", "/proc/cmdline", CA5_CMD_LINE_SIZE,
        CA5_CMD_LINE_OFFSET, MODE_ALL,
    },
    {
        // ca5 DSP
        "/dev/block/platform/sprd-sdhci.3/by-name/l_ldsp",
        "/dev/block/platform/sprd-sdhci.3/by-name/l_ldsp", CA5_LTE_DSP_SIZE,
        CA5_LTE_DSP_OFFSET, MODE_SVLTE,
    },
    {
        // ca5 DSP
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_ldsp",
        "/dev/block/platform/sprd-sdhci.3/by-name/tl_ldsp", CA5_LTE_DSP_SIZE,
        CA5_LTE_DSP_OFFSET, MODE_TCSFB,
    },
    {
        // DFS ARM7
        "/dev/block/platform/sprd-sdhci.3/by-name/dfs",
        "/dev/block/platform/sprd-sdhci.3/by-name/dfs", 0xf00, 0x2000C000,
        MODE_ALL,
    },
    {
        NULL, NULL, 0, 0, 0,
...
int get_modem_images_info_bak(void) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if ((info[images_count].address == 0) ||
        (info[images_count].image_size == 0)) {
...
static unsigned short calc_checksum(unsigned char *dat, unsigned long len) {
  ...
  if (0 == (unsigned long)dat % 2) {
    ...
    if (len % 2) checksum += *(unsigned char *)pstart;
  } else {
    pstart = (unsigned short *)dat;
    while (len > 1) {
...
static int _chkEcc(unsigned char *buf, int size) {
  ...
  crcOri = (unsigned short)((((unsigned short)buf[size - 3]) << 8) |
                            ((unsigned short)buf[size - 4]));
...
int get_modem_images_info(void) {
  ...
  int max_item =
      (sizeof(download_image_info) / sizeof(download_image_info[0])) - 1;
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (-1 == sscanf(line, "%s %s %x %x", originPath, bakPath,
    ...
    MODEM_LOGD(
        "\t%32s\t%32s\t0x%8x\t0x%8x\n", download_image_info[cnt].image_path,
        download_image_info[cnt].image_path_bak,
        download_image_info[cnt].image_size, download_image_info[cnt].address);

    if ((0 == download_image_info[cnt].address) ||
        0 == (download_image_info[cnt].image_size)) {
      /*get tty device number from modem_images.info*/
  ...
  for (cnt = 0; cnt < modem_images_count; cnt++) {
    if (!strcmp(download_image_info[cnt].image_path,
                download_image_info[cnt].image_path_bak)) {
      continue;
    }
    if (_chkImg(download_image_info[cnt].image_path,
                download_image_info[cnt].image_size)) {
      continue;
    }
    if (!_chkImg(download_image_info[cnt].image_path_bak,
                 download_image_info[cnt].image_size)) {
      continue;
    }
    strncpy(download_image_info[cnt].image_path,
           download_image_info[cnt].image_path_bak, originPath);
  }
  // 3 return
  return cnt;
}
...
void print_modem_image_info(void) {
  ...
  for (i = 0; i < modem_images_count; i++) {
    MODEM_LOGD("image[%d]: %s  size 0x%x  address 0x%x\n", i,
               info[i].image_path, info[i].image_size, info[i].address);
  }
}
...
static unsigned int delta_miliseconds(struct timespec *begin,
                                      struct timespec *end) {
  long ns;
  unsigned int ms;
  time_t sec;
...
static int append_lte_mode(char dst[], int len) {
  char * p_mode = NULL;
  ...
  snprintf(dst, len * sizeof(char), "%s%s", CMDLINE_LTE_MODE_PREFIX, p_mode);
  return strlen(dst);
}

static int append_test_mode(char dst[], int len) {
  char prop[100] = {0};

  property_get("persist.radio.ssda.testmode", prop, "");
  snprintf(dst, len * sizeof(char), "%s%s", CMDLINE_TEST_MODE_PREFIX, prop);
  return strlen(dst);
}
...
int download_image(int channel_fd, struct image_info *info) {
  ...
  MODEM_LOGD(
      "Start download image %s image_size 0x%x address 0x%x , nvbuf.magic  "
      "0x%x \n",
    ...
    do {
      ...
      if (read_len > 0) {
        ...
        if (!strcmp(info->image_path, "/proc/cmdline")) {  // append lte mode
          buffer[0] = '\0';
          offset = append_test_mode(buffer, MAX_PREFIX_LEN - 8);
          append_lte_mode(buffer + offset, MAX_PREFIX_LEN - 8 - offset);
        }
        break;
      }
    } while (packet_size > 0);
...
int download_images(int channel_fd) {
  ...
  for (i = 0; i < image_count; i++) {
    if (info->mask & cap_mask) {
      ...
      if (ret != DL_SUCCESS) break;
    }
    info++;
  }
  if (cap_mask & 0x1)
    send_exec_message(channel_fd, MODEM_ENTRY_MAGIC0, 1);  // parm
  else
    send_exec_message(channel_fd, MODEM_ENTRY_MAGIC1, 1);  // parm
  return ret;
}
...
static void *modemd_listen_uart_thread(void *par) {
  ...
  while (1) {
    ...
    if (!strncmp(temp, TD_MODEM_ALIVE_STR, strlen(TD_MODEM_ALIVE_STR))) {
      ...
      set_modem_state("1");
    } else if ((!strncmp(temp, LTE_MODEM_ASSERT_STR,
                         strlen(LTE_MODEM_ASSERT_STR))) ||
               (!strncmp(temp, TD_MODEM_ASSERT_STR,
                         strlen(TD_MODEM_ASSERT_STR)))) {
      MODEM_LOGD("modemd_listenaccept_uart_thread===>> %s\n", temp);
      set_modem_state("0");
    } else if (!strncmp(temp, LTE_MODEM_RESET_STR,
                        strlen(LTE_MODEM_RESET_STR))) {
