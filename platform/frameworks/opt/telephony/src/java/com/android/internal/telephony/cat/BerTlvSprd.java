...
class BerTlvSprd {
    ...
    private List<ComprehensionTlvSprd> mCompTlvs = null;
    ...
    private BerTlvSprd(int tag, List<ComprehensionTlvSprd> ctlvs, boolean lengthValid) {
        mTag = tag;
        mCompTlvs = ctlvs;
     ...
     */
    public List<ComprehensionTlvSprd> getComprehensionTlvs() {
        return mCompTlvs;
    }
     ...
     */
    public static BerTlvSprd decode(byte[] data) throws ResultExceptionSprd {
        int curIndex = 0;
        ...
        try {
            ...
            if (tag == BER_PROACTIVE_COMMAND_TAG) {
                ...
                if (ComprehensionTlvTagSprd.COMMAND_DETAILS.value() == (tag & ~0x80)) {
                    tag = BER_UNKNOWN_TAG;
                    curIndex = 0;
                }
            }
            ...
            throw new ResultExceptionSprd(ResultCodeSprd.CMD_DATA_NOT_UNDERSTOOD, e.explanation());
        }
        ...
        List<ComprehensionTlvSprd> ctlvs = ComprehensionTlvSprd.decodeMany(data,
                curIndex);

        if (tag == BER_PROACTIVE_COMMAND_TAG) {
            int totalLength = 0;
            for (ComprehensionTlvSprd item : ctlvs) {
        ...
        return new BerTlvSprd(tag, ctlvs, isLengthValid);
    }
}
