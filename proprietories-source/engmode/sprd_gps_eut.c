...
static int gpssnr_result(int prn_num, char *result, int count) {
  ...
  if (i > 0) {
    if (count == 0) {
      ...
      sprintf(result, "%s%s %s%s %s%d", EUT_GPS_SNR_REQ, max_snr, EUT_GPS_SV_ID,
              max_snr_id, EUT_GPS_SV_NUMS, sv_num);
    } else {
      for (k = 0; k < i; k++) {
...
struct eng_gps_eutops gps_eutops = {
    gps_eut,     gpseut_req,       gps_search, gps_search_req,
    set_gps_prn, gps_prnstate_req, gps_snr_req};
