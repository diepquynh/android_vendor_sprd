...
public class CameraAppUI implements ModeListView.ModeSwitchListener,
    ...
    public static class BottomBarUISpec {
        ...
        /**
         * set true if VGesture should visible
         */
        public boolean enableVGesture;

        /**
         * Set true if the button of VGesture should not be visible.
         */
        public boolean hideVGesture;
        ...
        /**
         * A {@link com.android.camera.ButtonManager.ButtonCallback}
         * that will be executed when the vgesture option is pressed. This
         * callback can be null.
         */
        public ButtonManager.ButtonCallback vgestureCallback;
    ...
    private final int MAX_FRAME_COUNT = android.os.SystemProperties.getInt(
            TARGET_CAMERA_UNFREEZE_FRAME_COUNT, 3);;
...
//        }
    ...
    public void onChangeCamera() {
        ...
        applyModuleSpecs(hardwareSpec, moduleController.getBottomBarSpec(), true/*skipScopeCheck*/);
    ...
    public void onSettingChanged(SettingsManager settingsManager, String key) {
        ...
        if (key.equals(Keys.KEY_CAMERA_HDR)) {
            ...
            applyModuleSpecs(moduleController.getHardwareSpec(), moduleController.getBottomBarSpec(),
                             true /*skipScopeCheck*/);
        /* SPRD: New feature vgesture detect @{ */
        } else if (key.equals(Keys.KEY_CAMERA_GRID_LINES) && UCamUtill.isVgestureEnnable()){
            updateGridLines();
        /* @} */
        }
    }
    ...
    private void applyModuleSpecs(final HardwareSpec hardwareSpec,
        ...
        if (skipScopeCheck
            ...
            if (bottomBarSpec.hideHdr || mIsCaptureIntent) {
                ...
                if (mController.getCameraProvider().getCharacteristics(mController.getCameraProvider()
                        .getCurrentCameraId().getLegacyValue()).isFacingFront()) {
                    /*
                     * SPRD: fix bug 568750 Open the front camera and enter the FilterModule from
                     * the Photo Module, and then return to PhotoModule
                     */
        ...
        */
        ...
        /* SPRD: New feature vgesture detect @{ */
        if (UCamUtill.isVgestureEnnable()) {
            if (bottomBarSpec.enableVGesture) {
                /* SPRD: add for bug 567284 avoid open Vgesture and can open HDR at the same time @{ */
                ArrayList<Integer> mutexButtons = new ArrayList<>();
                mutexButtons.add(ButtonManager.BUTTON_COUNTDOWN);
                mutexButtons.add(ButtonManager.BUTTON_HDR);
                buttonManager.initializeButton(ButtonManager.BUTTON_VGESTURE,
                        bottomBarSpec.vgestureCallback, getDisableButtonsCallback(mutexButtons));
                Log.d(TAG, "applyModuleSpecs: inited vgesture button");
                /* @} */
            } else {
                if (bottomBarSpec.hideVGesture) {
                    buttonManager.hideButton(ButtonManager.BUTTON_VGESTURE);
                } else {
        ...
        if (bottomBarSpec.enableExposureCompensation
                ...
                && bottomBarSpec.maxExposureCompensation == 0)
     ...
     *
     * Returns a {@link com.android.camera.ButtonManager.ButtonCallback} that
     * will disable the buttons identified by the parameter.
     *
     * @param conflictingButtons a ArrayList which contains The buttons id to be disabled.
     */
    private ButtonManager.ButtonCallback getDisableButtonsCallback(final ArrayList<Integer> conflictingButtons) {
        return new ButtonManager.ButtonCallback() {
            @Override
            public void onStateChanged(int state) {
                for (int i = 0; i < conflictingButtons.size(); i++) {
                    mController.getButtonManager().disableButton(conflictingButtons.get(i));
                }
            }
        };
    }
    ...
    public void startCircle() {
        mFilmstripLayout.startCircle();
    }

    public void stopCircle() {
        mFilmstripLayout.stopCircle();
    }

    public void setCircleDuration(long duration) {
        mFilmstripLayout.setCircleDuration(duration);
    }
    /* @} */
