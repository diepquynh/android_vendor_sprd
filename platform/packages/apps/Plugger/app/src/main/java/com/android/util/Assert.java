...
public final class Assert {
	 ...
	 * sShouldCrash = sIsEngBuild; if (!sShouldCrash) { sShouldCrash =
	 * gservices.getBoolean( BugleGservicesKeys.ASSERTS_FATAL,
	 * BugleGservicesKeys.ASSERTS_FATAL_DEFAULT); } }
	 * 
	 * // Static initializer block to find out if we're running an eng or //
	 * release build. static { setIfEngBuild(); }
	 * 
	 * // This is called from FactoryImpl once the Gservices class is
	 * initialized. public static void initializeGservices (final BugleGservices
	 * gservices) { gservices.registerForChanges(new Runnable() {
	 * 
	 * @Override public void run() { refreshGservices(gservices); } });
	 * refreshGservices(gservices); }
	 */
	/**
	 * Halt execution if this is not an eng build.
	 * <p>
	 * Intended for use in code paths that should be run only for tests and
	 * never on a real build.
	 * <p>
	 * Note that this will crash on a user build even though asserts don't
	 * normally crash on a user build.
	 */
	...
	public static void equals(final Object expected, final Object actual) {
		if (expected != actual
				&& (expected == null || actual == null || !expected
						.equals(actual))) {
			fail("Expected " + expected + " but got " + actual, false);
		}
	}
	...
	public static void oneOf(final int actual, final int... expected) {
		...
		fail("Expected value to be one of " + Arrays.toString(expected)
				+ " but was " + actual);
	}

	public static void inRange(final int val, final int rangeMinInclusive,
			final int rangeMaxInclusive) {
	...
	public static void inRange(final long val, final long rangeMinInclusive,
			final long rangeMaxInclusive) {
		if (val < rangeMinInclusive || val > rangeMaxInclusive) {
			fail("Expected value in range [" + rangeMinInclusive + ", "
					+ rangeMaxInclusive + "], but was " + val, false);
		}
	...
	public static void isMainThread() {
		if (Looper.myLooper() != Looper.getMainLooper()
				&& !Thread.currentThread().getName()
						.contains(TEST_THREAD_SUBSTRING)) {
	 ...
	 *            message to print when halting execution
	 */
	...
	private static void fail(final String message, final boolean crashRelease) {
		...
		if (crashRelease || sShouldCrash) {
			...
			// Find the method whose assertion failed. We're using a depth of 2,
			// because all public
			// Assert methods delegate to this one (see javadoc on getCaller()
			// for details).
			StackTraceElement caller = null;// DebugUtils.getCaller(2);
			if (caller != null) {
				// This log message can be de-obfuscated by the Proguard retrace
				// tool, just like a
