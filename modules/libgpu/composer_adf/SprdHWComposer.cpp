...
bool SprdHWComposer:: Init()
{
    ...
    mADF = new SprdADFWrapper();
    if (mADF == NULL)
    {
        ALOGE("new SprdADFWrapper failed");
        return false;
    }

    if (!(mADF->Init()))
    {
        ALOGE("SprdADFWrapper Init failed");
        return false;
    }
    adfData = static_cast<void *>(mADF);
#endif
    ...
    if (!(mPrimaryDisplay->Init(adfData)))
    {
    ...
    if (!(mExternalDisplay->Init(adfData)))
    {
    ...
    for(unsigned int i = 0; i < numDisplays; i++)
    {
        ...
        switch(i)
        {
            case DISPLAY_PRIMARY:
                mDisplayAttributes[DISPLAY_PRIMARY].AcceleratorMode |= ACCELERATOR_DISPC;
...
int SprdHWComposer:: commitDisplays(size_t numDisplays, hwc_display_contents_1_t **displays)
{
     ...
     * */
    ALOGI_IF(mDebugFlag, "<11> close ADF output rel_fd:%d,retire_fd:%d.",
...
int SprdHWComposer:: getDisplayConfigs(int disp, uint32_t* configs, size_t* numConfigs)
{
    ...
    if (mADF->GetConfigs(disp, configs, numConfigs))
    {
    ...
    switch(disp)
    {
        case DISPLAY_PRIMARY:
            ...
            size = (*numConfigs > MAX_NUM_CONFIGS)
                        ? MAX_NUM_CONFIGS : *numConfigs;
...
int SprdHWComposer:: getDisplayAttributes(int disp, uint32_t config, const uint32_t* attributes, int32_t* value)
{
    ...
    if (mADF->GetConfigAttributes(disp, config, attributes, value) < 0)
    {
        ...
        switch(attributes[i])
        {
            case HWC_DISPLAY_VSYNC_PERIOD:
                ...
                if(dpyAttr->vsync_period == 0) {
                    ALOGI("getDisplayAttributes: disp:%d vsync_period:0,set to default 60fps.", disp);
    ...
    switch(disp)
    {
	    ...
	    //mDisplayAttributes[disp].connected = true;
            break;
        default:
            ALOGE("SprdHWComposer:: getDisplayAttributes do not support display type %d", disp);
            break;
    }
...
bool SprdHWComposer:: eventControl(int disp, int event, int enabled)
{
     ...
     * */
#ifdef HWC_SUPPORT_ADF_DISPLAY
    mADF->EventControl(disp, event, enabled);
#else
