...
static uint32_t s_ov2680_sensor_close_flag = 0;
...
#define param_update(x1,x2) sprintf(name,"/data/misc/cameraserver/ov2680_%s.bin",x1);\
...
static uint32_t _ov2680_set_sensor_close_flag()
{
	uint32_t rtn = SENSOR_SUCCESS;

	s_ov2680_sensor_close_flag = 1;
...
LOCAL unsigned long _ov2680_access_val(unsigned long param)
{
	...
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			rtn = _ov2680_set_sensor_close_flag();
			break;
		default:
			break;
	}
