...
class ComprehensionTlvSprd {
     ...
     */
    protected ComprehensionTlvSprd(int tag, boolean cr, int length, byte[] data,
            int valueIndex) {
        mTag = tag;
        mCr = cr;
        mLength = length;
     ...
     */
    public static List<ComprehensionTlvSprd> decodeMany(byte[] data, int startIndex)
            throws ResultExceptionSprd {
        ArrayList<ComprehensionTlvSprd> items = new ArrayList<ComprehensionTlvSprd>();
        int endIndex = data.length;
        while (startIndex < endIndex) {
            ComprehensionTlvSprd ctlv = ComprehensionTlvSprd.decode(data, startIndex);
            if (ctlv != null) {
                items.add(ctlv);
     ...
     * @throws ResultException
     */
    public static ComprehensionTlvSprd decode(byte[] data, int startIndex)
            throws ResultExceptionSprd {
        ...
        try {
            ...
            switch (temp) {
            case 0:
                // SRRD: show STK with Ghana MTN simcard @{
                tag = 0;
                cr = false;
                break;
                // @}
            ...
            return new ComprehensionTlvSprd(tag, cr, length, data, curIndex);
