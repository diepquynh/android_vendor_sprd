...
//#define LOG_NDEBUG 0
#define LOG_TAG "AVIExtractorEx"
#include <utils/Log.h>
#include "include/avc_utils.h"
#include "AVIExtractorEx.h"
...
struct AVIExtractorEx::AVISource : public MediaSource {
    AVISource(const sp<AVIExtractorEx> &extractor, size_t trackIndex);
...
private:
    sp<AVIExtractorEx> mExtractor;
    size_t mTrackIndex;
    const AVIExtractorEx::Track &mTrack;
...
struct AVIExtractorEx::MP3Splitter : public RefBase {
    MP3Splitter();
...
AVIExtractorEx::AVISource::AVISource(
        const sp<AVIExtractorEx> &extractor, size_t trackIndex)
...
status_t AVIExtractorEx::AVISource::start(MetaData *params) {
    if (params) {
        ALOGI("%s, have MetaData: %p", __FUNCTION__, params);
    }
    CHECK(!mBufferGroup);
    ALOGV("params=%p",params);
...
status_t AVIExtractorEx::AVISource::stop() {
    CHECK(mBufferGroup);
...
sp<MetaData> AVIExtractorEx::AVISource::getFormat() {
    return mTrack.mMeta;
}

status_t AVIExtractorEx::AVISource::read(
        MediaBuffer **buffer, const ReadOptions *options) {
    CHECK(mBufferGroup);
...
AVIExtractorEx::MP3Splitter::MP3Splitter()
    : mFindSync(true),
      mBaseTimeUs(-1ll),
      mNumSamplesRead(0) {
}
...
void AVIExtractorEx::MP3Splitter::clear() {
    mFindSync = true;
    mBaseTimeUs = -1ll;
    mNumSamplesRead = 0;
...
void AVIExtractorEx::MP3Splitter::append(MediaBuffer *buffer) {
...
bool AVIExtractorEx::MP3Splitter::resync() {
    if (mBuffer == NULL) {
        return false;
    }
...
status_t AVIExtractorEx::MP3Splitter::read(MediaBuffer **out) {
    *out = NULL;
...
status_t AVIExtractorEx::MP3Splitter::readparam(int type,int16_t & param_value)
{
    if (type) {
...
AVIExtractorEx::AVIExtractorEx(const sp<DataSource> &dataSource)
    : AVIExtractor(dataSource),
...
AVIExtractorEx::~AVIExtractorEx() {
}
size_t AVIExtractorEx::countTracks() {
    return mTracks.size();
}

sp<IMediaSource> AVIExtractorEx::getTrack(size_t index) {
    return index < mTracks.size() ? new AVISource(this, index) : NULL;
}

sp<MetaData> AVIExtractorEx::getTrackMetaData(
...
sp<MetaData> AVIExtractorEx::getMetaData() {
...
uint32_t AVIExtractorEx::flags() const {
...
status_t AVIExtractorEx::parseHeaders() {
    mTracks.clear();
    mMovieOffset = 0;
    mFoundIndex = false;
...
ssize_t AVIExtractorEx::parseChunk(off64_t offset, off64_t size, int depth) {
    if (size >= 0 && size < 8) {
        return ERROR_MALFORMED;
    }
    ...
    if (fourcc == FOURCC('L', 'I', 'S', 'T')
        ...
        ALOGV("%s offset %lld LIST of '%c%c%c%c', size %d",
             prefix,
        ...
        ALOGV("%s offset %lld CHUNK '%c%c%c%c'",
             prefix,
             (unsigned long long)offset,
             (char)(fourcc >> 24),
             (char)((fourcc >> 16) & 0xff),
...
status_t AVIExtractorEx::parseStreamHeader(off64_t offset, size_t size) {
    if (size != 56) {
        return ERROR_MALFORMED;
    }
...
status_t AVIExtractorEx::parseStreamFormat(off64_t offset, size_t size) {
    if (mTracks.isEmpty()) {
        return ERROR_MALFORMED;
    }
...
// static
bool AVIExtractorEx::IsCorrectChunkType(
...
status_t AVIExtractorEx::parseIdx1(off64_t offset, size_t size) {
    /*if ((size % 16) != 0) {
        return ERROR_MALFORMED;
    }*/
...
status_t AVIExtractorEx::parseIndx(off64_t offset, size_t size) {
...
static sp<ABuffer> MakeMPEG4VideoCodecSpecificData(const sp<ABuffer> &config) {
    ...
    size_t len3 = len2 + GetSizeWidth(len2+13) + 1 + 3;
    /* SPRD: modify } */
...
status_t AVIExtractorEx::addMPEG4CodecSpecificData(size_t trackIndex) {
    Track *track = &mTracks.editItemAt(trackIndex);
...
status_t AVIExtractorEx::addH264CodecSpecificData(size_t trackIndex) {
    Track *track = &mTracks.editItemAt(trackIndex);
...
status_t AVIExtractorEx::getSampleInfo(
...
status_t AVIExtractorEx::getSampleTime(
...
status_t AVIExtractorEx::getSampleIndexAtTime(
        size_t trackIndex,
