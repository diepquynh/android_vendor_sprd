...
int  test_printlog_thread()
{
    ...
    if (0 != access("/data/ylog/factorytest_log",F_OK) ){
        ret = mkdir("/data/ylog/factorytest_log",S_IRWXU | S_IRWXG | S_IRWXO);
        if (-1 == ret && (errno != EEXIST)) {
            LOGE("mkdir /data/ylog/factorytest_log failed.");
            return 0;
        }
    }
    ret = chmod("/data/ylog/factorytest_log",S_IRWXU | S_IRWXG | S_IRWXO);
    if (-1 == ret) {
        LOGE("chmod /data/ylog/factorytest_log failed.");
        return 0;
    }

    if (0 == access("/data/ylog/factorytest_log/last_factorytest.log",F_OK)){
        ret = remove("/data/ylog/factorytest_log/last_factorytest.log");
        if (-1 == ret) {
            LOGE("remove failed.");
            return 0;
        }
    ...
    if (0 == access("/data/ylog/factorytest_log/factorytest.log",F_OK)){
        ret =  rename("/data/ylog/factorytest_log/factorytest.log","/data/ylog/factorytest_log/last_factorytest.log");
        if (-1 == ret) {
            LOGE("rename failed.");
            return 0;
        }
    ...
    fd = open("/data/ylog/factorytest_log/factorytest.log",O_RDWR|O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
    if (fd == -1 && (errno != EEXIST)) {
        LOGE("creat /data/ylog/factorytest_log/factorytest.log failed.");
        return 0;
    }
    if (fd >= 0 )
        close(fd);

    ret = chmod("/data/ylog/factorytest_log/factorytest.log",0777);
    if (-1 == ret) {
        LOGE("chmod /data/ylog/factorytest_log/factorytest.log failed.");
        return 0;
    }

    ret = system("logcat -v threadtime -f /data/ylog/factorytest_log/factorytest.log &");
