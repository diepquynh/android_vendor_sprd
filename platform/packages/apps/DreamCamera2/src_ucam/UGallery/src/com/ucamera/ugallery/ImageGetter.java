...
/**
 * Here's the loading strategy. For any given image, load the thumbnail into
 * memory and post a callback to display the resulting bitmap. Then proceed to
 * load the full image bitmap. Three things can happen at this point: 1. the
 * image fails to load because the UI thread decided to move on to a different
 * image. This "cancellation" happens by virtue of the UI thread closing the
 * stream containing the image being decoded. BitmapFactory.decodeStream returns
 * null in this case. 2. the image loaded successfully. At that point we post a
 * callback to the UI thread to actually show the bitmap. 3. when the post runs
 * it checks to see if the image that was loaded is still the one we want. The
 * UI may have moved on to some other image and if so we just drop the newly
 * loaded bitmap on the floor.
 */

interface ImageGetterCallback {
    void imageLoaded(int pos, int offset, RotateBitmap bitmap, boolean isThumb);

    boolean wantsThumbnail(int pos, int offset);

    boolean wantsFullImage(int pos, int offset);

    int fullImageSizeToUse(int pos, int offset);
        ...
        private void executeRequest() {
            ...
            for (int i = 0; i < order.length; i++) {
                ...
                if (imageNumber >= 0 && imageNumber < imageCount) {
                    ...
                    Bitmap b;
                    if(mIsSmallMemo) {
                        b = com.ucamera.ugallery.provider.ThumbnailUtils.createImageThumbnail(image.getDataPath(), Images.Thumbnails.MINI_KIND);
                    } else {
                        int sizeToUse = 1024;
                        int num = 3 * 512 * 256;
                        if(UiUtils.screenWidth() >= 1080) {
                            num = 1024 * 1024;
                        }
                        b = image.fullSizeBitmap(sizeToUse, num, IImage.NO_ROTATE, IImage.USE_NATIVE);
                    }
//                    Bitmap b = image.thumbBitmap(IImage.NO_ROTATE);
            ...
            for (int i = 0; i < order.length; i++) {
                ...
                if (imageNumber >= 0 && imageNumber < imageCount) {
                    ...
                    Bitmap b = image.fullSizeBitmap(sizeToUse, Build.USE_LARGE_CAPACITY_IMAGE ? 8 * 1024 * 1024 : 3 * 1024 * 1024,
                            IImage.NO_ROTATE, IImage.USE_NATIVE);
                    InputStream in = null;
                    if (b == null) {
                        try {
                            /* SPRD: CID 110909 : Resource leak (RESOURCE_LEAK) @{ */
                            in = mContext.getResources().openRawResource(R.drawable.missing_thumbnail_picture);
                            b =  BitmapFactory.decodeStream(in);
                            // b = BitmapFactory.decodeStream(mContext.getResources().openRawResource(R.drawable.missing_thumbnail_picture));
                            /* @} */
                        } catch(OutOfMemoryError oom) {
                            Log.w(TAG, "decode default picture oom.");
                            continue;
                        } catch(NoSuchMethodError error) {
                            /*
                             * FIX BUG: 5967
                             * BUG COMMENT: java.lang.NoSuchMethodError
                             * DATE: 2014-03-06
                             */
                            BitmapDrawable d = (BitmapDrawable)mContext.getResources().getDrawable(R.drawable.missing_thumbnail_picture);
                            b = d.getBitmap();
                        } finally {
                            try{
                                if(in != null){
    ...
    public ImageGetter(ContentResolver cr, Context context ,boolean isSmallMemo) {
        mCr = cr;
        mContext = context;
        mIsSmallMemo = isSmallMemo;
