...
public class CompactKindSectionView extends LinearLayout {
    ...
    public void setIsCreateNewGroup(Cursor cursor,RawContactDeltaList state) {
        for (int i = 0; i < mEditors.getChildCount(); i++) {
            ...
            if (view instanceof GroupMembershipView) {
                ((GroupMembershipView) view).setIsCreateNewGroup(cursor, state);
            }
        }
    }
    /**
    ...
    private void addNameEditorViews(AccountType accountType,
        ...
        /** SPRD: For AndroidN Porting @{ */
        if (!USimAccountType.ACCOUNT_TYPE.equals(accountType.accountType) && !SimAccountType.ACCOUNT_TYPE.equals(accountType.accountType)) {
        ...
        if (isNameKindSection) {
            ...
            for (int i = 0; i < mEditors.getChildCount(); i++) {
                ...
                if (view instanceof GroupMembershipView) {
                    ...
                    if ((!groupView.wasGroupMetaDataBound() && !groupView.accountHasGroups()) || mIsUserProfile) {
                        setVisibility(GONE);
                        return;
                    }
                }
