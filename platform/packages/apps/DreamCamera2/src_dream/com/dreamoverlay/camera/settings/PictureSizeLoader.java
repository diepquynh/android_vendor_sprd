...
public class PictureSizeLoader {
    ...
    public PictureSizeLoader(Context context, boolean cachedOnly) {
        ...
        mCameraDeviceInfo = CameraAgentFactory.getAndroidCameraAgent(context,
    ...
    public PictureSizes computePictureSizes() {
        ...
        Optional<SelectedVideoQualities> videoQualitiesBack = computeQualitiesForCamera(SettingsUtil.CAMERA_FACING_BACK);
        Optional<SelectedVideoQualities> videoQualitiesFront = computeQualitiesForCamera(SettingsUtil.CAMERA_FACING_FRONT);
        return new PictureSizes(backCameraSizes, frontCameraSizes,
                videoQualitiesBack, videoQualitiesFront);
    }
    ...
    private Optional<SelectedVideoQualities> computeQualitiesForCamera(
        ...
        int cameraId = SettingsUtil.getCameraId(mCameraDeviceInfo,
                facingSelector);
        if (cameraId >= 0) {
    ...
    private List<Size> computeSizesForCamera(CameraDeviceSelector facingSelector, CameraProxy proxy) {
        List<Size> sizes;
        int cameraId = SettingsUtil.getCameraId(mCameraDeviceInfo, facingSelector);
        if (cameraId >= 0) {
            if (mCachedOnly) {
                ...
                sizes = CameraPictureSizesCacher.getSizesForCamera(cameraId, mContext, proxy);
            }
