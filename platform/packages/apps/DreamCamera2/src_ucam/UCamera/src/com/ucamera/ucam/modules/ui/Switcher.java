...
public class Switcher extends ImageView implements View.OnTouchListener {
    ...
    private static final int HANDLE_UP_EVENT = 1;
    ...
    private boolean mIsWaitingUpEvent = false;
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
            case HANDLE_UP_EVENT:
                handleUpEvent((MotionEvent) msg.obj);
                break;
            }
        }
    };
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        /**
         * CID 109161 : DLS: Dead local store (FB.DLS_DEAD_LOCAL_STORE)
    ...
    private void sendEventMessage(MotionEvent event){
        mHandler.removeCallbacksAndMessages(HANDLE_UP_EVENT);
        Message msg = mHandler.obtainMessage(HANDLE_UP_EVENT, event);
        mHandler.sendMessageDelayed(msg, 500);
    }

    private void handleUpEvent(MotionEvent event){
        if(mIsWaitingUpEvent){
            trackTouchEvent(event);
            tryToSetSwitch(!mSwitch);
            setPressed(false);
            mIsWaitingUpEvent = false;
        }
    }
