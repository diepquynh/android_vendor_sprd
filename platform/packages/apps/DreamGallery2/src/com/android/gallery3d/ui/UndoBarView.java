...
public class UndoBarView extends GLView {
    ...
    private boolean mDownOnDeleteButton;
    ...
    protected boolean onTouch(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mDownOnDeleteButton = inDeleteButton(event);
                break;
            case MotionEvent.ACTION_UP:
                /* SPRD: bug 564348,click deleted, hide undo bar @{ */
                if (mDownOnButton || mDownOnDeleteButton) {
                    if (mOnClickListener != null && (inUndoButton(event) || inDeleteButton(event))) {
                        mOnClickListener.onClick(this);
                    }
                    mDownOnButton = false;
                    mDownOnDeleteButton = false;
                }
                /* @} */
                break;
    ...
    /* SPRD: bug 564348,click deleted, hide undo bar @{ */
    private boolean inDeleteButton(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        int w = getWidth();
        int h = getHeight();
        return (x >= 0 && x < (mBarMargin + mDeletedTextMargin + mDeletedText.getWidth()) && y >= 0 && y < h);
    }

    public boolean isClickDownUndo() {
        return mDownOnButton;
    }
    /* @} */
