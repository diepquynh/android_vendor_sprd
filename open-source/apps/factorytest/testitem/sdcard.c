...
int test_sdcard_start(void)
{
	...
	do{
            fd_mmc = open(SPRD_SD_MOUNT_DEV, O_RDONLY);
            if(fd_mmc < 0) {
                LOGD("open %s failed, times:%d", SPRD_SD_MOUNT_DEV, wait_cnt);
                if(wait_cnt++ >= MAX_OPEN_TIMES){
                    LOGD("open %s failed, try times exceed the max open times",SPRD_SD_MOUNT_DEV);
                    goto TEST_END;
                }
                sleep(1);
            }
	...
	if( mount(SPRD_SD_MOUNT_DEV, "/storage/sdcard0", "vfat", 0, NULL) < 0 )
            LOGE("%s mount failed",SPRD_SD_MOUNT_DEV);
	if(sdcard_rw()< 0) {
		ui_set_color(CL_RED);
