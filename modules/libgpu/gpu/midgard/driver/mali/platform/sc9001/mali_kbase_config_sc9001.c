...
static inline void mali_freq_init(void)
{
	...
	for (i = 0; i < gpu_dfs_ctx.gpu_clk_num; i++) {
		...
		of_property_read_string_index(np, "clock-names", i+1, &clk_name);
	...
	for(i=0; i<gpu_dfs_ctx.freq_list_len; i++)
	{
		...
		gpu_dfs_ctx.freq_list[i].clk_src = gpu_dfs_ctx.gpu_clk_src[clk-1];
...
{
	//all
	sci_glb_clr(REG_PMU_APB_PD_GPU_TOP_CFG, BIT_PMU_APB_PD_GPU_TOP_AUTO_SHUTDOWN_EN);
...
{
	//reduce time of power on
	kbase_reg_write(kbdev, GPU_CONTROL_REG(PWR_KEY), 0x2968a819, NULL);
	kbase_reg_write(kbdev, GPU_CONTROL_REG(PWR_OVERRIDE1), 0xFFFFFFFF, NULL);

	//core0
	sci_glb_clr(REG_PMU_APB_PD_GPU_C0_CFG, BIT_PMU_APB_PD_GPU_C0_AUTO_SHUTDOWN_EN);
	sci_glb_clr(REG_PMU_APB_PD_GPU_C0_CFG, BIT_PMU_APB_PD_GPU_C0_FORCE_SHUTDOWN);

	//core1
	sci_glb_clr(REG_PMU_APB_PD_GPU_C1_CFG, BIT_PMU_APB_PD_GPU_C1_AUTO_SHUTDOWN_EN);
	sci_glb_clr(REG_PMU_APB_PD_GPU_C1_CFG, BIT_PMU_APB_PD_GPU_C1_FORCE_SHUTDOWN);
...
static inline void mali_power_off(void)
{
	...
	//core0
	sci_glb_clr(REG_PMU_APB_PD_GPU_C0_CFG, BIT_PMU_APB_PD_GPU_C0_AUTO_SHUTDOWN_EN);
	sci_glb_set(REG_PMU_APB_PD_GPU_C0_CFG, BIT_PMU_APB_PD_GPU_C0_FORCE_SHUTDOWN);

	//core1
	sci_glb_clr(REG_PMU_APB_PD_GPU_C1_CFG, BIT_PMU_APB_PD_GPU_C1_AUTO_SHUTDOWN_EN);
	sci_glb_set(REG_PMU_APB_PD_GPU_C1_CFG, BIT_PMU_APB_PD_GPU_C1_FORCE_SHUTDOWN);

	//all
	sci_glb_clr(REG_PMU_APB_PD_GPU_TOP_CFG, BIT_PMU_APB_PD_GPU_TOP_AUTO_SHUTDOWN_EN);
	sci_glb_set(REG_PMU_APB_PD_GPU_TOP_CFG, BIT_PMU_APB_PD_GPU_TOP_FORCE_SHUTDOWN);
}
...
static int mali_platform_init(struct kbase_device *kbdev)
{
	...
	//power on
	sci_glb_write(REG_PMU_APB_PD_GPU_TOP_CFG,BIT_PMU_APB_PD_GPU_TOP_PWR_ON_DLY(1),0xFF0000);
	sci_glb_write(REG_PMU_APB_PD_GPU_TOP_CFG,BIT_PMU_APB_PD_GPU_TOP_PWR_ON_SEQ_DLY(1),0xFF00);
	sci_glb_write(REG_PMU_APB_PD_GPU_TOP_CFG,BIT_PMU_APB_PD_GPU_TOP_ISO_ON_DLY(1),0xFF);
...
struct kbase_platform_funcs_conf platform_sc9001_funcs = {
...
struct kbase_pm_callback_conf pm_sc9001_callbacks = {
	...
	u32 util_gl_share, u32 util_cl_share[2])
{
	...
	KBASE_DEBUG_PRINT(3, "MALI_DVFS gpu_boost_level:%d gpu_boost_sf_level:%d \n", gpu_boost_level,gpu_boost_sf_level);
