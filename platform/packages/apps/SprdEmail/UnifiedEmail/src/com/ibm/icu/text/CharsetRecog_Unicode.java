...
abstract class CharsetRecog_Unicode extends CharsetRecognizer {
    ...
    // UTF-16 confidence calculation. Very simple minded, but better than
    // nothing.
    // Any 8 bit non-control characters bump the confidence up. These have a
    // zero high byte,
    // and are very likely to be UTF-16, although they could also be part of a
    // UTF-32 code.
    // NULs are a contra-indication, they will appear commonly if the actual
    // encoding is UTF-32.
    ...
    static class CharsetRecog_UTF_16_BE extends CharsetRecog_Unicode {
        ...
        CharsetMatch match(CharsetDetector det) {
            ...
            for (int charIndex = 0; charIndex < bytesToCheck - 1; charIndex += 2) {
                int codeUnit = codeUnit16FromBytes(input[charIndex],
                        input[charIndex + 1]);
    ...
    static class CharsetRecog_UTF_16_LE extends CharsetRecog_Unicode {
        ...
        CharsetMatch match(CharsetDetector det) {
            ...
            int bytesToCheck = Math.min(input.length, 30);
            for (int charIndex = 0; charIndex < bytesToCheck - 1; charIndex += 2) {
                int codeUnit = codeUnit16FromBytes(input[charIndex + 1],
                        input[charIndex]);
                if (charIndex == 0 && codeUnit == 0xFEFF) {
                    confidence = 100;
                    break;
