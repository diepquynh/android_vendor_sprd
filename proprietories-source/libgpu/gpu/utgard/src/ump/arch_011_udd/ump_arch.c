...
ump_secure_id ump_arch_secure_id_from_phys_block(unsigned long phys_addr, unsigned long phys_size)
{
	_ump_uk_secure_id_from_phys_blocks_s dd_sfp_call_arg;

	dd_sfp_call_arg.ctx = ump_uk_ctx;
	dd_sfp_call_arg.phys_addr = phys_addr;
	dd_sfp_call_arg.phys_size = phys_size;
	dd_sfp_call_arg.secure_id = UMP_INVALID_SECURE_ID;
	if (_UMP_OSU_ERR_OK != _ump_uku_secure_id_from_phys_block(&dd_sfp_call_arg)) {
		UMP_DEBUG_PRINT(1, ("_ump_uku_secure_id_from_phys_block() get invalid secure id\n"));
		return UMP_INVALID_SECURE_ID;
	}

	return dd_sfp_call_arg.secure_id;
}
