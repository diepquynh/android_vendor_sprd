...
public class BrowserPreferencesPage extends PreferenceActivity {
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (requestCode == Controller.PERMISSIONS_REQUEST_STORAGE_READ_WRITE_SELECT_PATH) {
            Intent intent = (Intent)Controller.mPermissionObj;
            if (grantResults.length > 0
                && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Log.i("BrowserPreferencesPage", "onRequestPermissionsResult, storage permission for path granted");
                if (intent != null) {
                    startActivity(intent);
                }
            } else {
                Log.i("BrowserPreferencesPage", "onRequestPermissionsResult, storage permission for path not granted");
