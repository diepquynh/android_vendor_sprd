...
public abstract class BaseImageList implements IImageList {
    ...
    public BaseImageList(ContentResolver resolver, Uri uri, int sort, String bucketId) {
        ...
        // We need to clear the cache because we may "reopen" the image
    ...
    public boolean removeImage(IImage image) {
        Log.d(TAG, "BaseImageList.removeImage(IImage): image = " + image);
        if(image == null) {
            return false;
        }
        // need to delete the thumbnails as well
    ...
    public IImage getImageForUri(Uri uri) {
        if (uri != null && !uri.getScheme().equals("file") && !isChildImageUri(uri)){
            return null;
        }
        ...
        if(uri != null && uri.getScheme().equals("file")) {
            matchName = uri.getPath();
        }else {
            try {
                /* SPRD: CID 108942 : Dereference after null check (FORWARD_NULL) @{ */
                if(uri == null){
                    return null;
                }
                /* @} */
        ...
        // design a better method to get URI of specified ID
        Cursor cursor = getCursor();
        if (cursor == null)
            return null;
        synchronized (this) {
            ...
            for (int i = 0; cursor.moveToNext(); ++i) {
                if ((uri.getScheme().equals("file") && matchName != null && matchName.equals(getImageDatePath(cursor))) ||
                        (!uri.getScheme().equals("file") && getImageId(cursor) == matchId)) {
    ...
    protected String getImageDatePath(Cursor cursor) {
        return cursor.getString(INDEX_DATA_PATH);
    }
