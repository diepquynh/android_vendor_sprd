...
public class DataManager implements StitchingChangeListener {
    ...
    public Path getDefaultSetOf(boolean flag,Path item) {
        MediaSource source = mSourceMap.get(item.getPrefix());
        return source == null ? null : source.getDefaultSetOf(flag,item);
    }
    /**@}*/
    ...
    public boolean delete(ArrayList<Path> items) {
        ...
        MediaObject mo = items.get(items.size() - 1).getObject();
        if (null == mo) {
            return false;
        }
        Uri baseUri;
        if (mo instanceof LocalImage) {
            ...
            baseUri = android.provider.MediaStore.Video.Media.EXTERNAL_CONTENT_URI;
        } else {
            return false;
        }
        ...
        if (1 == items.size()) {
            ...
            for (; i >= 0; i--) {
                ...
                lmItem = (LocalMediaItem) items.get(i).getObject();
                if (null != lmItem) {
                    if (items.get(i).getObject().getClass() == mo.getClass()) {
                        sb.append(lmItem.id);
        ...
        mApplication.getContentResolver().delete(baseUri, sb.toString(), null);
        if(items.size() > 0) delete(items);
