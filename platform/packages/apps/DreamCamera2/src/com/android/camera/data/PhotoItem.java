...
/* @} */
     ...
     */
    public PhotoItem(Context context, GlideFilmstripManager manager, PhotoItemData data,
            PhotoItemFactory photoItemFactory) {
     /* @} */
    ...
    public PhotoItem(Context context, GlideFilmstripManager manager, PhotoItemData data,
        ...
        super(context, manager, data, attributes);
        mPhotoItemFactory = photoItemFactory;
    }
    ...
    public void renderThumbnail(@Nonnull View view) {
        if (view instanceof ImageView) {
            /*SPRD:fix bug593147 modify gif load method @{*/
            String mimeType = (mData.getMimeType() == null) ? "" : mData.getMimeType();
            if (FilmstripItemData.MIME_TYPE_GIF.equals(mimeType))
                return;
            /* @}*/
    ...
    public void renderFullRes(@Nonnull View view) {
        /*
         * Bug 620428 Gif does not large because Glid limit
         */
        String mimeType = (mData.getMimeType() == null) ? "" : mData.getMimeType();
        if (FilmstripItemData.MIME_TYPE_GIF.equals(mimeType)) {
            return;
    ...
    private GenericRequestBuilder<Uri, ?, ?, GlideDrawable> renderTinySize(Uri uri) {
        ...
        String mimeType = (mData.getMimeType() == null) ? "" : mData.getMimeType();
        if (FilmstripItemData.MIME_TYPE_GIF.equals(mimeType)) {
            return mGlideManager.loadTinyThumb(mContext, uri, generateSignature(mData));
        } else {
            return mGlideManager.loadTinyThumb(uri, generateSignature(mData));
        }
    }
