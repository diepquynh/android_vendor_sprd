...
/*        public void draw(Canvas canvas) {
        ...
        public boolean onScaleBegin(ScaleGestureDetector detector) {
            ...
            //mZoomProcessor.showZoomUI();
            android.util.Log.i("mVisible = ", "" + mVisible);
            if (mZoomListener == null) {
                ToastUtil.showToast(mContext, R.string.current_module_does_not_support_zoom_change,
                        Toast.LENGTH_LONG);
                return false;
            }
            if (mVisible) {
...
/*        public void showZoomUI() {
         ...
         Fix bug 568175 that no zoom ui displays @{
        public void showZoomUIWithFingerAngle(double angle) {
            if (mZoomListener == null) {
                return;
            }
            mVisible = true;
    ...
    public void onProgressChanged(SeekBar SeekBarId, int progress,
        ...
        float currentValueF = (progress * mZoomProcessor.mMaxRatio + (100 - progress) * 1.0f) * 0.01f;
        ...
        mDreamZoomUIListener.updateZoomValueText(currentValueF);
        //mController.onZoomChanged(currentValueF);
        setZoom(currentValueF);
    }
