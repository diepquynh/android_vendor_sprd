                                  ...
                                  uint32_t size)
{
    ...
    ring_buf->out += size;
    LOG_D("__ring_buffer_get in:0x%x out:0x%x",ring_buf->in,ring_buf->out);
    return size;
}
                                  ...
                                  uint32_t size)
{
    ...
    ring_buf->in += size;
    LOG_D("__ring_buffer_put in:0x%x out:0x%x",ring_buf->in,ring_buf->out);
    return size;
}
...
struct ring_buffer *ring_buffer_init(uint32_t size,int zero_size)
{
    ...
    if (!is_power_of_2(size)) {
        LOG_E("size must be power of 2 size:%d zero_size:%d\n",size,zero_size);
        return ring_buf;
    }

    if(zero_size>=size){
        LOG_E("zero_size(0x%x) need < size(0x%x)",zero_size,size);
        return ring_buf;
    }

    LOG_I("ring_buffer_init size:0x%x zero_size:0x%x",size,zero_size);

    buffer = (void *)malloc(size);
    if (!buffer) {
        LOG_E("Failed to malloc memory");
        goto err;
    }
    ...
    if (!ring_buf) {
        LOG_E("Failed to malloc ring_buffer,errno:%u,%s",
    ...
    if (pthread_mutex_init(&(ring_buf->lock), NULL) != 0) {
        LOG_E("Failed pthread_mutex_init,errno:%u,%s",
              errno, strerror(errno));
        goto err;
    }
    return ring_buf;
