...
bool SensorTMD2771::hasPendingEvents() const
{
    return  mHasPendingEvent;
}

int SensorTMD2771::setDelay(int32_t handle, int64_t ns)
{
	return 0;
}

int SensorTMD2771::setEnable(int32_t handle, int en)
{
    int what = -1;
    switch (handle) {
	...
	ALOGD("SensorTMD2771::setEnable=0= en=%d; newState=%d; what=%d; mEnabled=%d\n",\
    ...
    //if((uint32_t(newState) << what) != (mEnabled & (1 << what)))
    {
	 ...
	 int flags = newState;
		...
		if(newState)
		{
			...
			ALOGD("taos ---Light::TAOS_IOCTL_ALS_ON=== err=%d\n", err);
		}
		else
		{			
			err = ioctl(dev_fd, TAOS_IOCTL_ALS_OFF, &flags);
			ALOGD("taos ---Light::TAOS_IOCTL_ALS_OFF=== err=%d\n", err);
		}
		break;
		...
		ALOGD("taos ---Proximity::TAOS_IOCTL_PROX_ON=== err=%d\n", err);
		}
		else
		{
		err = ioctl(dev_fd, TAOS_IOCTL_PROX_OFF, &flags);
		ALOGD("taos ---Proximity::TAOS_IOCTL_PROX_OFF=== err=%d\n", err);
		}
		break;
	...
	ALOGD("SensorTMD2771::setEnable=1= en=%d; newState=%d; what=%d; mEnabled=%d\n",\
		en, newState, what, mEnabled);
...
int SensorTMD2771::getEnable(int32_t handle)
{
	int enable=0;
    int what = -1;
    switch (handle) {
	...
	ALOGD("SensorTMD2771::getEnable===mEnabled=0x%x; enable=%d\n", mEnabled, enable);

	return enable;
}

int SensorTMD2771::readEvents(sensors_event_t* data, int count)
{

 ALOGD("SensorTMD2771::readEvents()======data_fd=%d,count=%d",data_fd,count);
    ...
    while (count && mInputReader.readEvent(&event)) {
 	...
 	int64_t time = timevalToNano(event->time);
            ...
            ALOGD("Apds9900Sensor: unknown event (type=%d, code=%d)",
                    type, event->code);
            mInputReader.next();
        }
    }
...
void SensorTMD2771::processEvent(int code, int value)
{

     ALOGD("SensorTMD2771::processEvent()=====code=%d::value=%d ",code,value);
    switch (code) {
        case EVENT_TYPE_PROXIMITY:
            mPendingMask |= 1<<Proximity;
            mPendingEvents[Proximity].distance = value;
            ALOGD("SensorTMD2771: mPendingEvents[Proximity].distance = %f",mPendingEvents[Proximity].distance);
            break;        
        case EVENT_TYPE_LIGHT:
            mPendingMask |= 1<<Light;
            mPendingEvents[Light].light = (float)value;
            ALOGD("SensorTMD2771: mPendingEvents[Light].light = %f",mPendingEvents[Light].light);
            break; 
         default:
            ALOGD("SensorTMD2771: default value = %d",value);
            break;
    }
}
