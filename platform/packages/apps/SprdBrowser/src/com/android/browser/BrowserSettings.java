...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    */
    ...
    private final static String CONF_HOMEPAGE_URL = "homepage_url";
    /*@}*/
    ...
    private Runnable mSetup = new Runnable() {
        ...
        public void run() {
            ...
            */
            ...
            /*SPRD: implement addon in thread begin*/
            if(mPrefs.contains(PREF_USER_AGENT_PLUGIN)){
                sUserAgent = mPrefs.getString(PREF_USER_AGENT_PLUGIN, "");
            } else {
                sUserAgent = Custom.getUserAgentPlugin().getUserAgentString();
                if(sUserAgent==null)  sUserAgent = "";
                mPrefs.edit().putString(PREF_USER_AGENT_PLUGIN, sUserAgent).commit();
            }
            if(mPrefs.contains(PREF_HOMEPAGE_PLUGIN)){
                sFactoryResetUrl = mPrefs.getString(PREF_HOMEPAGE_PLUGIN, "");
            } else {
                sFactoryResetUrl = Custom.getHomePagePlugin().getHomePageString();
                if(sFactoryResetUrl==null) sFactoryResetUrl = "";
                mPrefs.edit().putString(PREF_HOMEPAGE_PLUGIN, sFactoryResetUrl).commit();
            }
            if(mPrefs.contains(PREF_STORAGE_PLUGIN)){
                Util.SUPPORT_SELECT_DOWNLOAD_PATH = mPrefs.getBoolean(PREF_STORAGE_PLUGIN, false);
            } else {
                Util.SUPPORT_SELECT_DOWNLOAD_PATH = Common.getStoragePlugIn().useEnvironmentEx();;
                mPrefs.edit().putBoolean(PREF_STORAGE_PLUGIN, Util.SUPPORT_SELECT_DOWNLOAD_PATH).commit();
            }
            /*SPRD: implement addon in thread end*/
            synchronized (BrowserSettings.class) {
                sInitialized = true;
                BrowserSettings.class.notifyAll();
            }
            Log.d(LOGTAG,"mSetup finish");
        }
    };
    ...
    private void setHomePage() {
        ...
        */
        if (TextUtils.isEmpty(sFactoryResetUrl)){
            if (Util.SUPPORT_CARRIER_HOMEPAGE) {
                String carrierConfig = Settings.Global.getString(mContext.getContentResolver(),
                        HOMEPAGE_CARRIER);
                if (TextUtils.isEmpty(carrierConfig)) {
                    AbsCarrierConfigManager absCfgManager= FrameworkFactory.getInstance().createExtraCarrierConfigManager();
                    if (absCfgManager != null) {
                        ...
                        if (absCfgManager.getConfigForPhoneId(defaultDataPhoneId) != null) {
                            String configUrl = absCfgManager.getConfigForPhoneId(defaultDataPhoneId)
                                    .getString(CONF_HOMEPAGE_URL);
    ...
    private void syncSetting(WebSettings settings) {
        ...
        try {
            Method m = WebSettings.class.getMethod("setForceUserScalable",boolean.class);
            m.invoke(settings, forceEnableUserScalable());
        } catch(Exception e) {
            Log.i(LOGTAG, "no setForceUserScalable in WebSettings");
        }
        /* @} */
    ...
    public String getSearchEngineName() {
         ...
         *@{
         */
        String defaultengine = Custom.getSearchEnginePlugin().getSearchEngineString();
        if (defaultengine == null)
            defaultengine = SearchEngine.GOOGLE;
        return mPrefs.getString(PREF_SEARCH_ENGINE, defaultengine);
    }
