...
public class Channel {
    ...
    public boolean isClosed() {
        if (mSession.getReader().getSEService() == null
                || !mSession.getReader().getSEService().isConnected()) {
            /* SPRD disable this condition for CMCC OMAPI test case @{
                After shutdown SEservice, test case will check the channel state.
                And should retrun the correct state (ture), because channel close is called.
            */
            //throw new IllegalStateException("service not connected to system");
            Log.w(getClass().getSimpleName(), "isClosed: service not connected to system");
            /*@}*/
        }
        if (mChannel == null) {
            throw new IllegalStateException("channel must not be null");
