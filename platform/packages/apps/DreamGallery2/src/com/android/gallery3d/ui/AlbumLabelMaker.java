...
public class AlbumLabelMaker {
    ...
    /* SPRD: bug 620248 cant change fontsize in time @{ */
    public void updateConfig() {
        if (mTitlePaint == null || mCountPaint == null || mSpec == null) {
            return;
        }
        int titleFontSize = mContext.getResources().getDimensionPixelSize(R.dimen.albumset_title_font_size);
        int countFontSize = mContext.getResources().getDimensionPixelSize(R.dimen.albumset_count_font_size);
        if (mSpec.titleFontSize != titleFontSize
                || mSpec.countFontSize != countFontSize) {
            mTitlePaint.setTextSize(titleFontSize);
            mCountPaint.setTextSize(countFontSize);
        }
    }
    /* @} */
}
