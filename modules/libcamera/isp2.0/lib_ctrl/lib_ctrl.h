...
struct ae_lib_fun {
	void* (*ae_init)(struct ae_init_in *in_param, void *out_param);
...
struct af_lib_fun {
	...
	int32_t (*af_get_lib_version)(isp_ctrl_context* handle,char* version,int len);
	int32_t (*af_ioctrl_set_dcam_timestamp)(isp_handle isp_handler,void* param_ptr,int capture);
	uint32_t product_id;
	uint32_t version_id;
};
//af end
...
uint32_t isp_lsclib_init(struct sensor_libuse_info* libuse_info, struct lsc_lib_fun* lsc_lib_fun);//gerald merge http://review.source.spreadtrum.com/gerrit/#/c/281621  
