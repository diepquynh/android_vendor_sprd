...
static int gnss_get_image_path(void) {
  ...
  if (0 == strncmp(partition, "true", sizeof("true"))) {
    ...
    if (0 == strncmp(mode, "gps_beidou", sizeof("gps_beidou"))) {
      memcpy(gPath, "/system/etc/gnssbdmodem.bin",
             sizeof("/system/etc/gnssbdmodem.bin"));
    } else {
      memcpy(gPath, "/system/etc/gnssmodem.bin",
             sizeof("/system/etc/gnssmodem.bin"));
    }
  }
  ...
  ret = socket_init(&pmanager);
	...
	do{
		...
		if(download_state == DOWNLOAD_START && (!gnss_get_image_path())){
                        DOWNLOAD_LOGD("gnss_download version:android7+201611\n");
			acquire_wake_lock(PARTIAL_WAKE_LOCK, "GNSS_DOWNLOAD_LOCK");
			DOWNLOAD_LOGD("gnss_download_lock LOCK\n");
			download_gnss_entry();
			release_wake_lock("GNSS_DOWNLOAD_LOCK");
			DOWNLOAD_LOGD("gnss_download_lock release\n");
