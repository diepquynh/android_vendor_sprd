...
public class PreviewsDefaultItem extends PreviewsBaseItem {
    ...
    private boolean isEmptyPreview(){
        ...
        ShortcutAndWidgetContainer cell;
        try {
            cell = (ShortcutAndWidgetContainer)((CellLayout) mWorkspace.getChildAt(mId)).getChildAt(0);
        } catch (NullPointerException e) {
            e.printStackTrace();
            Log.d("PreviewsDefaultItem", "cell of mId is null");
            return true;
        }
...
//          return true;
    ...
    private void initCloseImageView() {
      ...
      /* SPRD: Fix bug 264250,only show CloseView while screen count big than Workspace.MIN_PAGE. @{ */
      if (mWorkspace.getChildCount() > Workspace.MIN_PAGE && isEmptyPreview()) {
      /* @} */
    ...
    protected void showCloseView(boolean visible) {
        /* SPRD: Fix bug 264250,only show CloseView while screen count big than Workspace.MIN_PAGE. @{ */
        if (visible && isEmptyPreview() && mWorkspace.getChildCount() > Workspace.MIN_PAGE) {
        /* @} */
            mCloseImageView.setVisibility(View.VISIBLE);
        } else {
            mCloseImageView.setVisibility(View.GONE);
    ...
    private void initThumbnail(Workspace workspace) {
        ...
        if (width > 0 && height > 0) {
            ...
            /* SPRD:fix bug 294223,NullPointerException,the cell maybe is null @{*/
            if(cell != null){
                cell.dispatchDraw(c);
            }else{
                Log.w("PreviewsDefaultItem","cellLayout is null,the mId is "+mId);
            }
            /* @} */
        } else {
            mThumbnail = null;
            Log.w("PreviewsDefaultItem","initThumbnail mThumbnail = null");
        }
    }
    ...
    protected void recycleThumbnailBitmap() {
        if (mThumbnail != null && !mThumbnail.isRecycled()) {
            ...
            /* SPRD: bug317042 2014-05-28 reset ImageView's inner bitmap to null. @{ */
