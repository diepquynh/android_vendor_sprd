...
public class WidgetPreviewLoader {
    ...
    static class CacheDb extends SQLiteOpenHelper {
        ...
        /* SPRD fix bug296402 SQLiteCantOpenDatabaseException @{ */
        public boolean isExists() {
            File file = new File(mContext.getCacheDir(), DB_NAME);
            if (file.exists()) {
                return true;
            }
            return false;
    ...
    public static void removePackageFromDb(final CacheDb cacheDb, final String packageName) {
        ...
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                /* SPRD fix bug296402 SQLiteCantOpenDatabaseException,
                 * When the internal storage is less than 10%, the system will recover the resources in Cache,
                 * so before using CacheDb to determine whether there is. @{ */
                if(cacheDb.isExists()){
                    ...
                    Log.w(TAG, "removePackageFromDb: cacheDb no exists.");
                }
                /* @} */
                synchronized(sInvalidPackages) {
    ...
    public static void removeItemFromDb(final CacheDb cacheDb, final String objectName) {
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                /* SPRD fix bug304070 SQLiteDiskIOException,
                 * When the internal storage is less than 10%, the system will recover the resources in Cache,
                 * so before using CacheDb to determine whether there is. @{ */
                if(cacheDb.isExists()){
                    SQLiteDatabase db = cacheDb.getWritableDatabase();
                    ...
                    Log.w(TAG, "removeItemFromDb: cacheDb no exists.");
                }
                /* @} */
                return null;
            }
    ...
    private static void cleanUpWidgetPreviewDb() {
        ...
        if (cacheDb != null) {
            new AsyncTask<Void, Void, Void>() {
                public Void doInBackground(Void ... args) {
                    /* SPRD: fix bug 308798, SQLiteDiskIOException: disk I/O error. @{ */
                    try {
    ...
    private Bitmap readFromDb(String name, Bitmap b) {
        ...
        /* SPRD: fix bug335866, The connection pool has been closed. @{ */
        try {
    ...
    public Bitmap generateWidgetPreview(ComponentName provider, int previewImage,
        ...
        /* SPRD: fix bug 310003, not fully displayed. @{ */
