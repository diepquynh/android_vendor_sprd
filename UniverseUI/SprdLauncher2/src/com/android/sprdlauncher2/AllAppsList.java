...
class AllAppsList {
     ...
     * return will remove all about this package
     */
    // SPRD: fix bug258651
    public boolean updatePackage(Context context, String packageName) {
        final List<ResolveInfo> matches = findActivitiesForPackage(context, packageName);
        Log.d("Launcher.Model", "matches.size:"+matches.size());
     // SPRD: fix bug258651
        boolean willRemoveAll = false;
        if (matches.size() > 0) {
            ...
            for (int i = data.size() - 1; i >= 0; i--) {
                ...
                if (packageName.equals(component.getPackageName())) {
                    if (!findActivity(matches, component)) {
                        /* SPRD: fix bug 296425, WLAN icon is gone. @{ */
                        String[] appInfos = context.getResources().getStringArray(
                                R.array.not_update_appinfos);
                        boolean isUpdate = true;
                        for (int j = 0; j < appInfos.length; j++) {
                            if (appInfos[j].equals(component.getClassName())) {
                                isUpdate = false;
                                break;
                            }
                        }
                        if (isUpdate) {
                            Log.d("Launcher.Model", "remove from data:"+applicationInfo.toString());
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (applicationInfo == null) {
                    add(new AppInfo(context.getPackageManager(), info, mIconCache, null));
                    Log.d("Launcher.Model", "add to data:"+info.toString());
                } else {
                    mIconCache.remove(applicationInfo.componentName);
                    mIconCache.getTitleAndIcon(applicationInfo, info, null);
                    modified.add(applicationInfo);
                    Log.d("Launcher.Model", "modify from data:"+info.toString());
                }
            }
        } else {
         // SPRD: fix bug258651
