...
public class Folder extends LinearLayout implements DragSource, View.OnClickListener,
    ...
    private Resources res ; // SPRD: fix bug 273553

    /* SPRD: Fix bug 292896 @{ */
    private boolean mFolderNameChanged = false;
    /* @} */
    ...
    public Folder(Context context, AttributeSet attrs) {
        ...
        res = getResources();// SPRD: fix bug 273553
        /* SPRD: UUI folders @{
    ...
    protected void onFinishInflate() {
        ...
        mEditContent = (LinearLayout)findViewById(R.id.edit_content);
        /* @} */
    }
    ...
    public void doneEditingFolderName(boolean commit) {
        ...
        /* SPRD: fix bug 273553,the text view can not be changed when change language @{*/
    ...
    public void animateOpen() {
        ...
        ObjectAnimator ob = ObjectAnimator.ofPropertyValuesHolder(mOpenBg, alpha, scaleX, scaleY);
        ...
        as.setDuration(mExpandDuration);
    ...
    protected boolean createAndAddShortcut(ShortcutInfo item) {
        ...
        */
        if(item.unreadNum == 0){
            ...
            Bitmap bitmapWithNum = UnreadLoader.getBitmapWithNum(mContext, item, item.getIcon(mIconCache));
            textView.setCompoundDrawables(null,
    ...
    public void onDropCompleted(final View target, final DragObject d,
        ...
        if (successfulDrop) {
            ...
            /* SPRD: Fix bug287460, some time should not return the cancelled item into this Folder. @{ */
            //if the dragging item is invalid before dropComplete(like STK) ,do not return it into the folder.
            final PackageManager manager = mLauncher.getPackageManager();
            ComponentName cn = null;
            if (d.dragInfo instanceof AppInfo) {
                // Came from all apps -- make a copy
                cn = ((AppInfo) d.dragInfo).intent.getComponent();
            } else {
                cn = ((ShortcutInfo) d.dragInfo).intent.getComponent();
            }
            /* SPRD: Fix bug309437, because the shortcut added from AllApps always do not have ComponentName. @{ */
            if (cn == null || mLauncher.getModel().isValidPackageComponent(manager, cn)) {
        ...
        /* SPRD: Fix bug287479,if folder is opened and an of the item dragging,remove the view delay. @{ */
    ...
    private void centerAboutIcon() {
        ...
        /* SPRD: fix bug 338773, The getChildAt(i) maybe return null. @{
        ShortcutAndWidgetContainer boundingLayout = currentLayout.getShortcutsAndWidgets();
        Rect bounds = new Rect();
        parent.getDescendantRectRelativeToSelf(boundingLayout, bounds);
         */
        ShortcutAndWidgetContainer boundingLayout = null;
        if(currentLayout != null){
            boundingLayout = currentLayout.getShortcutsAndWidgets();
        }else{
            Log.w(TAG,"Folder.centerAboutIcon() currentLayout is null,currentPage = "+currentPage);
        }
        Rect bounds = new Rect();
        parent.getDescendantRectRelativeToSelf(boundingLayout, bounds);
        /* SPRD: fix bug 338773, The getChildAt(i) maybe return null. @} */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        /* SPRD: bug298700 2014-04-17 empty folder does not need measure. @{ */
        if (getItemCount() != 0){
            mScrollView.measure(contentAreaWidthSpec, contentAreaHeightSpec);
        }
        /* SPRD: bug298700 2014-04-17 empty folder does not need measure. @} */

        /* SPRD: bug fix:bug268996 @{ */
        // mFolderName.measure(contentAreaWidthSpec,
        // MeasureSpec.makeMeasureSpec(mFolderNameHeight, MeasureSpec.EXACTLY));
        mFolderName.measure(
                MeasureSpec.makeMeasureSpec(
                        (int) getResources().getDimension(R.dimen.folder_edit_name_width),
                        MeasureSpec.EXACTLY),
                MeasureSpec.makeMeasureSpec(mFolderNameHeight, MeasureSpec.EXACTLY));
        /* @} */
        /* SPRD: add for UUI folders @{*/
        int btnWidthSpec = MeasureSpec.makeMeasureSpec((int)getResources().getDimension(R.dimen.folder_edit_button_width),
                MeasureSpec.EXACTLY);
        mEditBtn.measure(btnWidthSpec,
                MeasureSpec.makeMeasureSpec(mFolderNameHeight, MeasureSpec.EXACTLY));
        int btnHeightSpec = MeasureSpec.makeMeasureSpec((int)getResources().getDimension(R.dimen.folder_edit_content_width),
                MeasureSpec.EXACTLY);
        mEditContent.measure(btnHeightSpec,
                MeasureSpec.makeMeasureSpec(mFolderNameHeight, MeasureSpec.EXACTLY));
        width = getPaddingLeft() + getPaddingRight() + Math.max(mContent.getDesiredWidth() ,mEditContent.getMeasuredWidth());
        /* @} */
    ...
    private void onCloseComplete() {
        ...
        /* SPRD: Fix bug 292896 @{ */
        if (mFolderNameChanged) {
            doneEditingFolderName(true);
            mFolderNameChanged = false;
        }
        /* @} */
    }

    private void replaceFolderWithFinalItem() {
        /* SPRD: Fix bug282801, if the folder in Dragging process,cancel it. @{ */
        if (mDragController.isFolderIconDragging(mInfo, mLauncher)) {
        ...
        /* SPRD: Fix bug287460,if there are no item in this folder, we should delete it also. @{ */
        if (finalChild != null) {
            mFolderIcon.performDestroyAnimation(finalChild, onCompleteRunnable);
        } else if (getItemCount() < 1) {
            mFolderIcon.post(onCompleteRunnable);
        }
        /* @} */
        mDestroyed = true;
    }
    ...
    public void hideItem(ShortcutInfo info) {
        ...
        if(v != null){
            ...
            Log.e(TAG,"hideItem the v is null");
        }
    }
    public void showItem(ShortcutInfo info) {
        ...
        if(v != null){
            ...
            Log.e(TAG,"showItem the v is null");
        }
    }
    ...
    public void onRemove(ShortcutInfo item) {
        ...
        /* SPRD: Fix bug287479,if folder is opened and an of the item dragging,remove the view delay. @{ */
        if (mDragInProgress && mInfo.opened) {
        ...
        if(v != null){
            ...
            Log.e(TAG,"onRemove the v is null");
        }
        ...
        /* SPRD: Fix bug 284656, @{ */
        if (getItemCount() <= 1) {
            /* SPRD: Fix bug 286139, if folder is opened, just close it. @{ */
            //replaceFolderWithFinalItem();
            if (mInfo.opened) {
                if (isEditingName()) {
                    dismissEditingName();
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        if (v == mFolderName) {
            if(hasFocus){
                startEditingFolderName();
                mFolderName.setBackgroundColor(Color.DKGRAY);
