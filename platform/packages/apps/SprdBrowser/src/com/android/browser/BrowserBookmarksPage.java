...
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener,
    ...
    public boolean handleContextItem(int itemId, int groupPosition,
        ...
        case R.id.shortcut_context_menu_id:
            ...
            if (c != null) {
                ...
                String title = c.getString(BookmarksLoader.COLUMN_INDEX_TITLE);
                Toast.makeText(activity, activity.getString(R.string.shortcut_installed, title),
                    Toast.LENGTH_SHORT).show();
            } else {
                Log.d(LOGTAG,"shortcut_context_menu_id Cursor== null");
            }
            break;
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        int group = R.id.BOOKMARK_CONTEXT_MENU;
        if (isFolder) {
            group = R.id.FOLDER_CONTEXT_MENU;
        }
        ...
        for (int index = 0; index < menu.size(); index++) {
            ...
            if (child != null) {
                if (child.getItemId() == (R.id.new_window_context_menu_id)
                        && child.getGroupId() == group) {
                    if(mDisableNewWindow){
                        child.setVisible(false);
                    } else {
                        /* Fix Bug:541001 The title displayed is incorrect. 2016.03.11 start */
                        if (isFolder) {
                            child.setTitle(BrowserSettings.getInstance()
                                    .openInBackground() ? R.string.open_all_in_new_window_background
                                    : R.string.open_all_in_new_window);
                        } else {
                            child.setTitle(BrowserSettings.getInstance()
                                    .openInBackground() ? R.string.contextmenu_openlink_newwindow_background
                                    : R.string.contextmenu_openlink_newwindow);
                        }
                        /* Fix Bug:541001 The title displayed is incorrect. 2016.03.11 end */
                    }
                    break;
                }
            }
    ...
    private void displayRemoveBookmarkDialog(BrowserBookmarksAdapter adapter,
        ...
        */
        ...
        sAlertdialog = BookmarkUtils.displayRemoveBookmarkDialog(id, title, context, null,folder);
        /*@}*/
    }
