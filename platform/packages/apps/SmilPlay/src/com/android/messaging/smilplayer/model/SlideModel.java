...
import com.android.messaging.smilplayer.dom.smil.SmilParElementImpl;
...
public class SlideModel extends Model implements List<MediaModel>, EventListener {
    ...
    /* SPRD: Add for notification message. @{ */
    private MediaModel mVcard;
    private MediaModel mVcalendar;
    /* @} */
    private MediaModel mHtml;
    ...
    private boolean mCanAddVcalendar = true;
    /* @} */
    private boolean mCanAddHtml = true;
    ...
    private void internalAdd(MediaModel media) throws IllegalStateException {
        ...
        if (media.isText()) {
            ...
            if (mCanAddImage) {
                ...
                mImage = media;
                mCanAddVideo = false;
                mCanAddVcard = false; // SPRD:  vcard/vcalendar
                mCanAddVcalendar = false;// SPRD:  vcard/vcalendar
                mCanAddHtml = false;
            } else {
                Log.w(TAG, "[SlideModel] content type " + media.getContentType() +
            ...
            if (mCanAddVcard) {// SPRD:  vcard/vcalendar
                internalAddOrReplace(mVcard, media);
                mVcard = media;
                mCanAddImage = false;
                mCanAddAudio = false;
                mCanAddVideo = false;
                mCanAddVcalendar = false;
                mCanAddHtml = false;
            } else {
                Log.w(TAG, "[SlideModel] content type " + media.getContentType() +
                    " - can't add vcard in this state");
            }
        } else if (media.isVcalendar()) {// SPRD:  vcard/vcalendar
            if (mCanAddVcalendar) {
                internalAddOrReplace(mVcalendar, media);
                mVcalendar = media;// SPRD:  vcard/vcalendar
                mCanAddImage = false;
                mCanAddAudio = false;
                mCanAddVideo = false;
                mCanAddVcard = false;// SPRD:  vcard/vcalendar
    ...
    private void internalAddOrReplace(MediaModel old, MediaModel media) {
        ...
        if (old == null) {
            ...
            if (mOldMediaIndex == -1) {
                Log.d(TAG, "There may be an exception for the old media not exist in history.");
                mMedia.add(media);
                increaseSlideSize(addSize);
                increaseMessageSize(addSize);
            } else {
    ...
    private boolean internalRemove(Object object) {
        if (mMedia.remove(object)) {
            if (object instanceof TextModel) {
                ...
                mImage = null;
                mCanAddVideo = true;
                mCanAddVcard = true;// SPRD:  vcard/vcalendar
    ...
    public void clear() {
        if (mMedia.size() > 0) {
            ...
            mAudio = null;
            mVideo = null;
            mVcard = null;// SPRD:  vcard/vcalendar
    ...
    public void handleEvent(Event evt) {
        if (evt.getType().equals(SmilParElementImpl.SMIL_SLIDE_START_EVENT)) {
            ...
            mVisible = true;
        } else if (evt.getType().equals(SmilParElementImpl.SMIL_SLIDE_END_EVENT) || mFill != ElementTime.FILL_FREEZE) {
            if (LOCAL_LOGV) {
    ...
    /**
     * SPRD: Add method for notification message. @{
     * @return boolean
     */
    ...
    public VcalendarModel getVcalendar() {
        return (VcalendarModel) mVcalendar;
    }
