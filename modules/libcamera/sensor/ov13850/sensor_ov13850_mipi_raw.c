...
#include "newparam/sensor_ov13850_raw_param_main.c"
#else
#endif
...
LOCAL SENSOR_TRIM_T s_ov13850_Resolution_Trim_Tab[] = {
	...
	{0, 0, 2112, 1568, 100, 640, 3328, {0, 0,  2112, 1568}},  //vts
...
#define param_update(x1,x2,x3) \
				switch(x3) { \
				case ISP_NR_COMMON_MODE: \
					sprintf(name,"/data/ov13850_%s.bin",x1);\
				break; \
				case ISP_NR_CAP_MODE: \
					sprintf(name,"/data/noise_cap/ov13850_%s.bin",x1);\
				break; \
				case ISP_NR_VIDEO_MODE: \
					sprintf(name,"/data/noise_video/ov13850_%s.bin",x1);\
				break; \
				case ISP_NR_PANO_MODE: \
					sprintf(name,"/data/noise_panorama/ov13850_%s.bin",x1);\
				break; \
				default: \
					sprintf(name,"/data/ov13850_%s.bin",x1);\
				break; \
...
LOCAL uint32_t Sensor_ov13850_InitRawTuneInfo(void)
{
	...
	for (i=0; i<mode_common_ptr->block_num; i++) {
		...
		case	ISP_BLK_PRE_WAVELET_V1: {
				...
				static struct sensor_pwd_level pwd_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/pwd_param.h"
				};
				static struct sensor_pwd_level pwd_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/pwd_param.h"
				};
				static struct sensor_pwd_level pwd_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/pwd_param.h"
				};

				param_update("pwd_param",pwd_param,ISP_NR_COMMON_MODE);
				param_update("pwd_param",pwd_cap_param,ISP_NR_CAP_MODE);
				param_update("pwd_param",pwd_video_param,ISP_NR_VIDEO_MODE);
				param_update("pwd_param",pwd_pano_param,ISP_NR_PANO_MODE);

				static struct nr_param_ptr nr_pwd_ptr;
				nr_pwd_ptr.common_param_ptr = pwd_param;
				nr_pwd_ptr.capture_param_ptr = pwd_cap_param;
				nr_pwd_ptr.video_param_ptr = pwd_video_param;
				nr_pwd_ptr.pano_param_ptr = pwd_pano_param;

				block->param_ptr = (isp_uint*)&nr_pwd_ptr;
				block->reserved2[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_BPC_V1: {
				...
				static struct sensor_bpc_level bpc_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/bpc_param.h"
				};
				static struct sensor_bpc_level bpc_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/bpc_param.h"
				};
				static struct sensor_bpc_level bpc_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/bpc_param.h"
				};

				param_update("bpc_param",bpc_param,ISP_NR_COMMON_MODE);
				param_update("bpc_param",bpc_cap_param,ISP_NR_CAP_MODE);
				param_update("bpc_param",bpc_video_param,ISP_NR_VIDEO_MODE);
				param_update("bpc_param",bpc_pano_param,ISP_NR_PANO_MODE);

				static struct nr_param_ptr nr_bpc_ptr;
				nr_bpc_ptr.common_param_ptr = bpc_param;
				nr_bpc_ptr.capture_param_ptr = bpc_cap_param;
				nr_bpc_ptr.video_param_ptr = bpc_video_param;
				nr_bpc_ptr.pano_param_ptr = bpc_pano_param;

				block->param_ptr = (isp_uint*)&nr_bpc_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_BL_NR_V1: {
				...
				static struct sensor_bdn_level bdn_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/bdn_param.h"
				};
				static struct sensor_bdn_level bdn_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/bdn_param.h"
				};
				static struct sensor_bdn_level bdn_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/bdn_param.h"
				};

				param_update("bdn_param",bdn_param,ISP_NR_COMMON_MODE);
				param_update("bdn_param",bdn_cap_param,ISP_NR_CAP_MODE);
				param_update("bdn_param",bdn_video_param,ISP_NR_VIDEO_MODE);
				param_update("bdn_param",bdn_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_bdn_ptr;
				nr_bdn_ptr.common_param_ptr = bdn_param;
				nr_bdn_ptr.capture_param_ptr = bdn_cap_param;
				nr_bdn_ptr.video_param_ptr = bdn_video_param;
				nr_bdn_ptr.pano_param_ptr = bdn_pano_param;

				block->param_ptr = (isp_uint*)&nr_bdn_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_GRGB_V1: {
				...
				static struct sensor_grgb_v1_level grgb_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/grgb_param.h"
				};
				static struct sensor_grgb_v1_level grgb_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/grgb_param.h"
				};
				static struct sensor_grgb_v1_level grgb_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/grgb_param.h"
				};

				param_update("grgb_param",grgb_param,ISP_NR_COMMON_MODE);
				param_update("grgb_param",grgb_cap_param,ISP_NR_CAP_MODE);
				param_update("grgb_param",grgb_video_param,ISP_NR_VIDEO_MODE);
				param_update("grgb_param",grgb_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_grgb_ptr;
				nr_grgb_ptr.common_param_ptr = grgb_param;
				nr_grgb_ptr.capture_param_ptr = grgb_cap_param;
				nr_grgb_ptr.video_param_ptr = grgb_video_param;
				nr_grgb_ptr.pano_param_ptr = grgb_pano_param;

				block->param_ptr = (isp_uint*)&nr_grgb_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_NLM: {
				...
				param_update("nlm_param",nlm_param,ISP_NR_COMMON_MODE);

				static struct sensor_vst_level vst_param[32] = {
					#include "noise/vst_param.h"
				};

				param_update("vst_param",vst_param, ISP_NR_COMMON_MODE);

				static struct sensor_ivst_level ivst_param[32] = {
					#include "noise/ivst_param.h"
				};

				param_update("ivst_param",ivst_param,ISP_NR_COMMON_MODE);

				static struct sensor_flat_offset_level flat_offset_param[32] = {
					#include "noise/flat_offset_param.h"
				};

				param_update("flat_offset_param",flat_offset_param,ISP_NR_COMMON_MODE);

				static struct sensor_nlm_level nlm_cap_param[32] = {
					#include "noise_cap/nlm_param.h"
				};

				param_update("nlm_param",nlm_cap_param,ISP_NR_CAP_MODE);

				static struct sensor_vst_level vst_cap_param[32] = {
					#include "noise_cap/vst_param.h"
				};

				param_update("vst_param",vst_cap_param,ISP_NR_CAP_MODE);

				static struct sensor_ivst_level ivst_cap_param[32] = {
					#include "noise_cap/ivst_param.h"
				};

				param_update("ivst_param",ivst_cap_param,ISP_NR_CAP_MODE);

				static struct sensor_flat_offset_level flat_offset_cap_param[32] = {
					#include "noise_cap/flat_offset_param.h"
				};

				param_update("flat_offset_param",flat_offset_cap_param,ISP_NR_CAP_MODE);

				static struct sensor_nlm_level nlm_video_param[32] = {
					#include "noise_video/nlm_param.h"
				};

				param_update("nlm_param",nlm_video_param, ISP_NR_VIDEO_MODE);

				static struct sensor_vst_level vst_video_param[32] = {
					#include "noise_video/vst_param.h"
				};

				param_update("vst_param",vst_video_param,ISP_NR_VIDEO_MODE);

				static struct sensor_ivst_level ivst_video_param[32] = {
					#include "noise_video/ivst_param.h"
				};

				param_update("ivst_param",ivst_video_param,ISP_NR_VIDEO_MODE);

				static struct sensor_flat_offset_level flat_offset_video_param[32] = {
					#include "noise_video/flat_offset_param.h"
				};
				param_update("flat_offset_param",flat_offset_video_param,ISP_NR_VIDEO_MODE);

				static struct sensor_nlm_level nlm_pano_param[32] = {
					#include "noise_panorama/nlm_param.h"
				};

				param_update("nlm_param",nlm_pano_param,ISP_NR_PANO_MODE);

				static struct sensor_vst_level vst_pano_param[32] = {
					#include "noise_panorama/vst_param.h"
				};

				param_update("vst_param",vst_pano_param,ISP_NR_PANO_MODE);

				static struct sensor_ivst_level ivst_pano_param[32] = {
					#include "noise_panorama/ivst_param.h"
				};

				param_update("ivst_param",ivst_pano_param,ISP_NR_PANO_MODE);

				static struct sensor_flat_offset_level flat_offset_pano_param[32] = {
					#include "noise_panorama/flat_offset_param.h"
				};

				param_update("flat_offset_param",flat_offset_pano_param,ISP_NR_PANO_MODE);

				static struct nr_param_ptr nr_nlm_ptr;
				static struct nr_param_ptr nr_vst_ptr;
				static struct nr_param_ptr nr_ivst_ptr;
				static struct nr_param_ptr nr_flat_offset_ptr;

				nr_nlm_ptr.common_param_ptr = nlm_param;
				nr_nlm_ptr.capture_param_ptr = nlm_cap_param;
				nr_nlm_ptr.video_param_ptr = nlm_video_param;
				nr_nlm_ptr.pano_param_ptr = nlm_pano_param;

				nr_vst_ptr.common_param_ptr = vst_param;
				nr_vst_ptr.capture_param_ptr = vst_cap_param;
				nr_vst_ptr.video_param_ptr = vst_video_param;
				nr_vst_ptr.pano_param_ptr = vst_pano_param;

				nr_ivst_ptr.common_param_ptr = ivst_param;
				nr_ivst_ptr.capture_param_ptr = ivst_cap_param;
				nr_ivst_ptr.video_param_ptr = ivst_video_param;
				nr_ivst_ptr.pano_param_ptr = ivst_pano_param;

				nr_flat_offset_ptr.common_param_ptr = flat_offset_param;
				nr_flat_offset_ptr.capture_param_ptr = flat_offset_cap_param;
				nr_flat_offset_ptr.video_param_ptr = flat_offset_video_param;
				nr_flat_offset_ptr.pano_param_ptr = flat_offset_pano_param;


				block->param_nlm_ptr = (isp_uint*)&nr_nlm_ptr;
				block->param_vst_ptr = (isp_uint*)&nr_vst_ptr;
				block->param_ivst_ptr = (isp_uint*)&nr_ivst_ptr;
				block->param_flat_offset_ptr = (isp_uint*)&nr_flat_offset_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_CFA_V1: {
				...
				static struct sensor_cfae_level cfae_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/cfae_param.h"
				};
				static struct sensor_cfae_level cfae_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/cfae_param.h"
				};
				static struct sensor_cfae_level cfae_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/cfae_param.h"
				};

				param_update("cfae_param",cfae_param,ISP_NR_COMMON_MODE);
				param_update("cfae_param",cfae_cap_param,ISP_NR_CAP_MODE);
				param_update("cfae_param",cfae_video_param,ISP_NR_VIDEO_MODE);
				param_update("cfae_param",cfae_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_cfae_ptr;
				nr_cfae_ptr.common_param_ptr = cfae_param;
				nr_cfae_ptr.capture_param_ptr = cfae_cap_param;
				nr_cfae_ptr.video_param_ptr = cfae_video_param;
				nr_cfae_ptr.pano_param_ptr = cfae_pano_param;

				block->param_ptr = (isp_uint*)&nr_cfae_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_RGB_PRECDN: {
				...
				static struct sensor_rgb_precdn_level precdn_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/rgb_precdn_param.h"
				};
				static struct sensor_rgb_precdn_level precdn_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/rgb_precdn_param.h"
				};
				static struct sensor_rgb_precdn_level precdn_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/rgb_precdn_param.h"
				};

				param_update("rgb_precdn_param",precdn_param,ISP_NR_COMMON_MODE);
				param_update("rgb_precdn_param",precdn_cap_param,ISP_NR_CAP_MODE);
				param_update("rgb_precdn_param",precdn_video_param,ISP_NR_VIDEO_MODE);
				param_update("rgb_precdn_param",precdn_pano_param,ISP_NR_PANO_MODE);

				static struct nr_param_ptr nr_precdn_ptr;
				nr_precdn_ptr.common_param_ptr = precdn_param;
				nr_precdn_ptr.capture_param_ptr = precdn_cap_param;
				nr_precdn_ptr.video_param_ptr = precdn_video_param;
				nr_precdn_ptr.pano_param_ptr = precdn_pano_param;

				block->param_ptr = (isp_uint*)&nr_precdn_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_YUV_PRECDN: {
				...
				static struct sensor_yuv_precdn_level yuv_precdn_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/yuv_precdn_param.h"
				};

				static struct sensor_yuv_precdn_level yuv_precdn_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/yuv_precdn_param.h"
				};

				static struct sensor_yuv_precdn_level yuv_precdn_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/yuv_precdn_param.h"
				};

				param_update("yuv_precdn_param",yuv_precdn_param,ISP_NR_COMMON_MODE);
				param_update("yuv_precdn_param",yuv_precdn_cap_param,ISP_NR_CAP_MODE);
				param_update("yuv_precdn_param",yuv_precdn_video_param,ISP_NR_VIDEO_MODE);
				param_update("yuv_precdn_param",yuv_precdn_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_yuv_precdn_ptr;
				nr_yuv_precdn_ptr.common_param_ptr = yuv_precdn_param;
				nr_yuv_precdn_ptr.capture_param_ptr = yuv_precdn_cap_param;
				nr_yuv_precdn_ptr.video_param_ptr = yuv_precdn_video_param;
				nr_yuv_precdn_ptr.pano_param_ptr = yuv_precdn_pano_param;

				block->param_ptr = (isp_uint*)&nr_yuv_precdn_ptr;
				block->reserved3[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_PREF_V1: {
				...
				static struct sensor_prfy_level prfy_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/prfy_param.h"
				};
				static struct sensor_prfy_level prfy_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/prfy_param.h"
				};
				static struct sensor_prfy_level prfy_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/prfy_param.h"
				};

				param_update("prfy_param",prfy_param,ISP_NR_COMMON_MODE);
				param_update("prfy_param",prfy_cap_param,ISP_NR_CAP_MODE);
				param_update("prfy_param",prfy_video_param,ISP_NR_VIDEO_MODE);
				param_update("prfy_param",prfy_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_prfy_ptr;
				nr_prfy_ptr.common_param_ptr = prfy_param;
				nr_prfy_ptr.capture_param_ptr = prfy_cap_param;
				nr_prfy_ptr.video_param_ptr = prfy_video_param;
				nr_prfy_ptr.pano_param_ptr = prfy_pano_param;

				block->param_ptr = (isp_uint*)&nr_prfy_ptr;
				block->reserved3[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_UV_CDN: {
				...
				static struct sensor_uv_cdn_level uv_cdn_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/yuv_cdn_param.h"
				};
				static struct sensor_uv_cdn_level uv_cdn_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/yuv_cdn_param.h"
				};
				static struct sensor_uv_cdn_level uv_cdn_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/yuv_cdn_param.h"
				};

				param_update("yuv_cdn_param",uv_cdn_param,ISP_NR_COMMON_MODE);
				param_update("yuv_cdn_param",uv_cdn_cap_param,ISP_NR_CAP_MODE);
				param_update("yuv_cdn_param",uv_cdn_video_param,ISP_NR_VIDEO_MODE);
				param_update("yuv_cdn_param",uv_cdn_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_uv_cdn_ptr;
				nr_uv_cdn_ptr.common_param_ptr = uv_cdn_param;
				nr_uv_cdn_ptr.capture_param_ptr = uv_cdn_cap_param;
				nr_uv_cdn_ptr.video_param_ptr = uv_cdn_video_param;
				nr_uv_cdn_ptr.pano_param_ptr = uv_cdn_pano_param;

				block->param_ptr = (isp_uint*)&nr_uv_cdn_ptr;
				block->reserved2[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_EDGE_V1: {
				...
				static struct sensor_ee_level edge_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/edge_param.h"
				};
				static struct sensor_ee_level edge_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/edge_param.h"
				};
				static struct sensor_ee_level edge_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/edge_param.h"
				};

				param_update("edge_param",edge_param,ISP_NR_COMMON_MODE);
				param_update("edge_param",edge_cap_param,ISP_NR_CAP_MODE);
				param_update("edge_param",edge_video_param,ISP_NR_VIDEO_MODE);
				param_update("edge_param",edge_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_edge_ptr;
				nr_edge_ptr.common_param_ptr = edge_param;
				nr_edge_ptr.capture_param_ptr = edge_cap_param;
				nr_edge_ptr.video_param_ptr = edge_video_param;
				nr_edge_ptr.pano_param_ptr = edge_pano_param;

				block->param_ptr = (isp_uint*)&nr_edge_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_UV_POSTCDN: {
				...
				static struct sensor_uv_postcdn_level uv_postcdn_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/yuv_postcdn_param.h"
				};
				static struct sensor_uv_postcdn_level uv_postcdn_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/yuv_postcdn_param.h"
				};
				static struct sensor_uv_postcdn_level uv_postcdn_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/yuv_postcdn_param.h"
				};

				param_update("yuv_postcdn_param",uv_postcdn_param,ISP_NR_COMMON_MODE);
				param_update("yuv_postcdn_param",uv_postcdn_cap_param,ISP_NR_CAP_MODE);
				param_update("yuv_postcdn_param",uv_postcdn_video_param,ISP_NR_VIDEO_MODE);
				param_update("yuv_postcdn_param",uv_postcdn_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_uv_postcdn_ptr;
				nr_uv_postcdn_ptr.common_param_ptr = uv_postcdn_param;
				nr_uv_postcdn_ptr.capture_param_ptr = uv_postcdn_cap_param;
				nr_uv_postcdn_ptr.video_param_ptr = uv_postcdn_video_param;
				nr_uv_postcdn_ptr.pano_param_ptr = uv_postcdn_pano_param;

				block->param_ptr = (isp_uint*)&nr_uv_postcdn_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_IIRCNR_IIR: {
				...
				static struct sensor_iircnr_level iir_cnr_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/iircnr_param.h"
				};
				static struct sensor_iircnr_level iir_cnr_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/iircnr_param.h"
				};
				static struct sensor_iircnr_level iir_cnr_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/iircnr_param.h"
				};

				param_update("iircnr_param",iir_cnr_param,ISP_NR_COMMON_MODE);
				param_update("iircnr_param",iir_cnr_cap_param,ISP_NR_CAP_MODE);
				param_update("iircnr_param",iir_cnr_video_param,ISP_NR_VIDEO_MODE);
				param_update("iircnr_param",iir_cnr_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_iir_cnr_ptr;
				nr_iir_cnr_ptr.common_param_ptr = iir_cnr_param;
				nr_iir_cnr_ptr.capture_param_ptr = iir_cnr_cap_param;
				nr_iir_cnr_ptr.video_param_ptr = iir_cnr_video_param;
				nr_iir_cnr_ptr.pano_param_ptr = iir_cnr_pano_param;

				block->param_ptr = (isp_uint*)&nr_iir_cnr_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case	ISP_BLK_IIRCNR_YRANDOM: {
				...
				static struct sensor_iircnr_yrandom_level iir_yrandom_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/iir_yrandom_param.h"
				};
				static struct sensor_iircnr_yrandom_level iir_yrandom_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/iir_yrandom_param.h"
				};
				static struct sensor_iircnr_yrandom_level iir_yrandom_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/iir_yrandom_param.h"
				};

				param_update("iir_yrandom_param",iir_yrandom_param,ISP_NR_COMMON_MODE);
				param_update("iir_yrandom_param",iir_yrandom_cap_param,ISP_NR_CAP_MODE);
				param_update("iir_yrandom_param",iir_yrandom_video_param,ISP_NR_VIDEO_MODE);
				param_update("iir_yrandom_param",iir_yrandom_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_iir_yrandom_ptr;
				nr_iir_yrandom_ptr.common_param_ptr = iir_yrandom_param;
				nr_iir_yrandom_ptr.capture_param_ptr = iir_yrandom_cap_param;
				nr_iir_yrandom_ptr.video_param_ptr = iir_yrandom_video_param;
				nr_iir_yrandom_ptr.pano_param_ptr = iir_yrandom_pano_param;

				block->param_ptr = (isp_uint*)&nr_iir_yrandom_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		...
		case  ISP_BLK_UVDIV_V1: {
				...
				static struct sensor_cce_uvdiv_level cce_uvdiv_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/cce_uv_param.h"
				};
				static struct sensor_cce_uvdiv_level cce_uvdiv_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/cce_uv_param.h"
				};
				static struct sensor_cce_uvdiv_level cce_uvdiv_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/cce_uv_param.h"
				};

				param_update("cce_uv_param",cce_uvdiv_param,ISP_NR_COMMON_MODE);
				param_update("cce_uv_param",cce_uvdiv_cap_param,ISP_NR_CAP_MODE);
				param_update("cce_uv_param",cce_uvdiv_video_param,ISP_NR_VIDEO_MODE);
				param_update("cce_uv_param",cce_uvdiv_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_cce_uvdiv_ptr;
				nr_cce_uvdiv_ptr.common_param_ptr = cce_uvdiv_param;
				nr_cce_uvdiv_ptr.capture_param_ptr = cce_uvdiv_cap_param;
				nr_cce_uvdiv_ptr.video_param_ptr = cce_uvdiv_video_param;
				nr_cce_uvdiv_ptr.pano_param_ptr = cce_uvdiv_pano_param;

				block->param_ptr = (isp_uint*)&nr_cce_uvdiv_ptr;
				block->reserved1[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
		case ISP_BLK_YIQ_AFM:{
				...
				static struct sensor_y_afm_level y_afm_cap_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_cap/y_afm_param.h"
				};
				static struct sensor_y_afm_level y_afm_video_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_video/y_afm_param.h"
				};
				static struct sensor_y_afm_level y_afm_pano_param[SENSOR_SMART_LEVEL_NUM] = {
					#include "noise_panorama/y_afm_param.h"
				};

				param_update("y_afm_param",y_afm_param,ISP_NR_COMMON_MODE);
				param_update("y_afm_param",y_afm_cap_param,ISP_NR_CAP_MODE);
				param_update("y_afm_param",y_afm_video_param,ISP_NR_VIDEO_MODE);
				param_update("y_afm_param",y_afm_pano_param,ISP_NR_PANO_MODE);
				static struct nr_param_ptr nr_y_afm_ptr;
				nr_y_afm_ptr.common_param_ptr = y_afm_param;
				nr_y_afm_ptr.capture_param_ptr = y_afm_cap_param;
				nr_y_afm_ptr.video_param_ptr = y_afm_video_param;
				nr_y_afm_ptr.pano_param_ptr = y_afm_pano_param;

				block->param_ptr = (isp_uint*)&nr_y_afm_ptr;
				block->reserved[0] = SENSOR_MULTI_MODE_FLAG;
			}
			break;
...
#define ov13850_VER_ADDR     0x302A
	...
	pid_value_h = Sensor_ReadReg(ov13850_PID_ADDR_H);
	pid_value_l = Sensor_ReadReg(ov13850_PID_ADDR_L);
	if (ov13850_PID_VALUE_H == pid_value_h && ov13850_PID_VALUE_L == pid_value_l) {
		ver_value = Sensor_ReadReg(ov13850_VER_ADDR);
		SENSOR_PRINT("SENSOR_ov13850: Identify: PID = %x%x, VER = %x", pid_value_h, pid_value_l, ver_value);
		if (ov13850_VER_VALUE == ver_value) {
			SENSOR_PRINT("SENSOR_ov13850: this is ov13850 %x sensor !", ver_value);
			...
			SENSOR_PRINT_HIGH("SENSOR_ov13850: Identify this is OV%x%x %x sensor !", pid_value_h, pid_value_l, ver_value);
		}
	} else {
		SENSOR_PRINT_ERR("SENSOR_ov13850: identify fail,pid_value_h=%x, pid_value_l=%x", pid_value_h, pid_value_l);
	}
...
static unsigned long _ov13850_SetEV(unsigned long param)
{
	...
	case SENSOR_HDR_EV_LEVE_0:
		_calculate_hdr_exposure(s_ov13850_gain,s_capture_VTS,s_capture_shutter/2);
		break;
