...
public class StickyBottomCaptureLayout extends FrameLayout {
    ...
    private int mSceneryIndex = getResources().getInteger(R.integer.camera_mode_scenery);
    // SPRD: Fix 474843 Add for Filter Feature
    private int mFilterIndex = getResources().getInteger(R.integer.camera_mode_filter);
    ...
    private PointF getRoundedThumbnailPosition(
        ...
        final float sceneryLayoutHeight = getResources().getDimensionPixelSize(R.dimen.mode_option_overlay_scenery_height);//SPRD: fix bug474859
        // SPRD: Fix 474843 Add for Filter Feature
        final float filterLayoutHeight = getResources().getDimensionPixelSize(R.dimen.mode_option_overlay_filter_height);
        ...
        if (orientation == Configuration.ORIENTATION_PORTRAIT) {
             ...
             * SPRD: fix bug474859 add for ucam scenery 
             * SPRD: Fix 474843 Add for Filter Feature @{
             */
            if (mCameraAppUI != null) {
                int index = mCameraAppUI.getCurrentModeIndex();
                if (mCameraAppUI.getModuleIndex() == PhotoUI.UCAM_BEAUTY_MODULE_INDEX) {
                    y -= beautyLayoutHeight;
                } else if (index == mSceneryIndex) {
                    y -= sceneryLayoutHeight;
                } else if (index == mFilterIndex) {
                    y -= filterLayoutHeight;
                }
            }
            /* @} */
        }
