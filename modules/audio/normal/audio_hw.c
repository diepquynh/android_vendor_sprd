...
#define SHORT_PERIOD_MULTIPLIER 12 /* 43 ms */
...
#define PLAYBACK_SHORT_PERIOD_COUNT   2      /*4*/
   ...
   pthread_mutex_unlock(&adev->lock);
    ...
    if(cur_out && (!(cur_out & (~AUDIO_DEVICE_OUT_ALL_SCO)))) {
        if((adev->active_output)
            ...
            ALOGE("yaye  need_bypass_output!%s, adev->active_output:%p, bt_sco_state:%x",
                __func__, adev->active_output, adev->bt_sco_state);
            bypass_outdevice = 1;
        }
    }
...
static int start_bt_sco_output_stream(struct tiny_stream_out *out)
{
    ...
    */
    ...
    out->pcm_bt_sco = pcm_open(card, port, PCM_OUT | PCM_MONOTONIC, &pcm_config_btsco);
#else
...
#endif
#ifndef AUDIO_OLD_MODEM
    if((out->devices & AUDIO_DEVICE_OUT_ALL_SCO) && (!out->is_voip )&&(!adev->call_start)) {
#else
