...
public class ButtonManager implements SettingsManager.OnSettingChangedListener {
    ...
    public static final int BUTTON_VGESTURE = 13;
    ...
    private MultiToggleImageButton camera_vgesture_button;
    ...
    public void onSettingChanged(SettingsManager settingsManager, String key) {
        ...
        */
        ...
        } else if (key.equals(Keys.KEY_CAMERA_VGESTURE) && UCamUtill.isVgestureEnnable()) {
            index = mSettingsManager.getIndexOfCurrentValue(SettingsManager.SCOPE_GLOBAL,
                    Keys.KEY_CAMERA_VGESTURE);
            button = getButtonOrError(BUTTON_VGESTURE);
        /* @} */
        }
    ...
    protected MultiToggleImageButton getButtonOrError(int buttonId) {
        switch (buttonId) {
            ...
            case BUTTON_COUNTDOWN:
             ...
             /* SPRD: New feature vgesture detect @{ */
            case BUTTON_VGESTURE:
                if (camera_vgesture_button == null) {
                    throw new IllegalStateException("Camera button could not be found.");
                }
                return camera_vgesture_button;
             /* @} */
            default:
    ...
    public void initializeButton(int buttonId, ButtonCallback cb, ButtonCallback preCb) {
        ...
        switch (buttonId) {
            ...
            case BUTTON_VGESTURE:
                if (UCamUtill.isVgestureEnnable()) {
                    initializeCameraButtonVG(button, cb, preCb,R.array.camera_vgesture_icons);
                }
                break;
            /* @} */
            default:
    ...
    public void resetCameraButton() {
        MultiToggleImageButton button = getButtonOrError(BUTTON_CAMERA);
        int index = mSettingsManager.getIndexOfCurrentValue(mAppController.getModuleScope(),
                Keys.KEY_CAMERA_ID);
        button.setState(index >= 0 ? index : 0, false);
        enableCameraButton();
