...
function board_for_kernel()
{
	...
	TEMP=$KCONFIG_NAME_REF
	echo $TEMP- *
	TEMP1=$BOARD_NAME_N"_dt_defconfig"
	if [  -f $TEMP ];then
		cp $TEMP $TEMP1
	else
	  #有些board的defconfig文件取名为BOARD_NAME_N_defconfig,例如:sp5735ea_defconfig
		TEMP=$KCONFIG_BOARD_NAME_R"_defconfig"
		TEMP1=$BOARD_NAME_N"_defconfig"
		if [  -f $TEMP ];then
			cp $TEMP $TEMP1
		else
			echo -e $KCONFIG_BOARD_NAME_R"***_defconfig not exist,configure kernel fail!!!!"
			exit 0
		fi
	fi
	...
	TEMP1=`tr '[a-z]' '[A-Z]' <<<"$KCONFIG_BOARD_NAME_R"`
	echo TEMP1:$TEMP1
	...
	#clone dt_defconfig
	TEMP_=$KCONFIG_BOARD_NAME_R"_dt_defconfig"
	#echo "xxx"$TEMP_
	...
	if [ $have_dt_board = 1 ];then
		...
		substring=$KCONFIG_BOARD_NAME_R
		begin_string=`tr '[a-z]' '[A-Z]' <<<"$KCONFIG_BOARD_NAME_R"`
	...
	#---------产生*board-BOARD_NAME.h文件 begin-------
	cd $workdir
	echo $PWD
	cd kernel/include/soc/sprd
	TEMP1="__board-"$KCONFIG_BOARD_NAME_R".h"
	TEMP2="__board-"$BOARD_NAME_N".h"
	cp $TEMP1 $TEMP2
	#---------产生*board-BOARD_NAME.h文件end-------

	#---------产生*board-BOARD_NAME.c文件 begin-------
	cd $workdir
	cd kernel/arch/arm/mach-sc
	TEMP1="board-"$KCONFIG_BOARD_NAME_R".c"
	...
	TEMP='sp'
	TEMP1=${KCONFIG_BOARD_NAME_R#*$TEMP}
...
function board_for_uboot()
{
	...
	PATH_R="u-boot64/board/spreadtrum/""$UCONFIG_BOARD_NAME_R"
	PATH_N="u-boot64/board/spreadtrum/""$BOARD_NAME_N"
	...
	do
		...
		PATH_M="u-boot64/include/configs/"
...
#change DTS
	sed -i 's/DTS_DEFCONFIG[[:space:]?]:=[[:space:]?].*/DTS_DEFCONFIG := sprd-'$PLATFORM'_'$BOARD_NAME_N'/g' $PROJECT_NEW".mk"
#change Kernel
...
echo $KCONFIG_NAME_REF

DTS_NAME_REF=`grep "DTS_DEFCONFIG[[:space:]?]:=[[:space:]?]" $PATH_R/$PROJECT_REF".mk"`
DTS_NAME_REF=${DTS_NAME_REF#"DTS_DEFCONFIG := "}
echo $DTS_NAME_REF

#因命名规则不统一建立选择表兼容旧的命名格式,设置默认
CCONFIG_BOARD_NAME_R=$CCONFIG_NAME_REF
echo $CCONFIG_BOARD_NAME_R

UCONFIG_BOARD_NAME_R=$UCONFIG_NAME_REF
echo $UCONFIG_BOARD_NAME_R
