...
#ifdef __KERNEL__
         pr_info("Calibration value is %2.2llu ohm @ %d degrees\n", re25, Tcal);
#else
	printf("Calibration value is %2.2f ohm @ %d degrees\n", re25, Tcal);
#endif
...
#ifdef __KERNEL__
    printf("Calibration value is %2.2llu ohm @ %d degrees\n", re25, Tcal);
#else
    printf("Calibration value is %2.2f ohm @ %d degrees\n", re25, Tcal);
#endif
    /* calculate the tCoefA */
...
Tfa98xx_Error_t tfa98xxCalibration(Tfa98xx_handle_t *handlesIn, int idx, int once )
{
   ...
   else
   {
	    ...
	    */
		   ...
		   if ( fabs(re25) > 0.1){
#ifdef __KERNEL__
         pr_info(" cleaning up old Re (=%.02llu)\n", re25);
#else
	...
	if(tfa98xxCalCheckMTPEX(handlesIn[idx]) == 0)
   {
...
#ifdef __KERNEL__
      pr_err("Error: DSP already calibrated (R=%.02llu Ohms).\n", re25);
#else
      ...
      if (!tfa98xxCalDspSupporttCoef(handlesIn[idx]))
      {
		   ...
		   err = tfa98xxCalComputeSpeakertCoefA(handlesIn[idx], tfaParams.speakerBuffer, tCoef);
		   assert(err == Tfa98xx_Error_Ok);
...
#ifdef __KERNEL__
      pr_info("Calibration value is \t\t\t\t\t%2.2llu ohm\n", re25);
#else
