...
public class DERParser {
    ...
    throws PKCS15Exception {
        Log.d(TAG, "[Parser] DERParser Constructor buffer > " + ByteArrayConverter.byteArrayToHexString(buffer));
        mDERBuffer=buffer;
        mDERIndex=0; mDERSize=0;
        if (mDERBuffer==null) return;
        mDERSize=(short)mDERBuffer.length;
        Log.d(TAG, "[Parser] DERParser Constructor mDERSize " + mDERSize);
        mTLVDataSize=mDERSize;

        // Remove padding
        if (mDERSize==0) return;
        if (mDERBuffer[mDERIndex]==ASN1.TAG_Padding) {
            Log.d(TAG, "[Parser] DERParser Constructor ASN1.TAG_Padding");
            mTLVDataSize=0;
            while(++mDERIndex<mDERSize) {
                Log.d(TAG, "[Parser] DERParser Constructor mDERBuffer " + mDERBuffer[mDERIndex]);
                if (mDERBuffer[mDERIndex]!=ASN1.TAG_Padding)
    ...
    throws PKCS15Exception {
        Log.d(TAG, "[Parser] DERParser isEndofBuffer mDERIndex : " + mDERIndex + " , mDERSize : " + mDERSize);
        if (mDERIndex==mDERSize) return true;
        if (mDERBuffer[mDERIndex]==ASN1.TAG_Padding) {
            // Remove padding
            Log.d(TAG, "[Parser] DERParser isEndofBuffer ASN1.TAG_Padding");
            while(++mDERIndex<mDERSize) {
            if (mDERBuffer[mDERIndex]!=ASN1.TAG_Padding)
                throw new PKCS15Exception("[Parser] Incorrect file format");
            } return true;
    ...
    public byte[] getTLVData() {
        Log.d(TAG, "[Parser] getTLVData before mDERIndex : " + mDERIndex + " , mTLVDataSize : " + mTLVDataSize);
        byte[] data=Arrays.copyOfRange(mDERBuffer,mDERIndex,
                                                        mDERIndex+mTLVDataSize);
        mDERIndex+=mTLVDataSize;
        Log.d(TAG, "[Parser] getTLVData after mDERIndex : " + mDERIndex + " , mTLVDataSize : " + mTLVDataSize);
        Log.d(TAG, "[Parser] getTLVData data > " + ByteArrayConverter.byteArrayToHexString(data));
        return data;
    }
