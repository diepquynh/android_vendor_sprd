...
public class UriImage extends MediaItem {
    ...
    private static final int SUPPORT_DRM_RIGHTS_INFO = 1 << 18;
    ...
    public UriImage(GalleryApp application, Path path, Uri uri, String contentType) {
        ...
        /* SPRD: bug613322 UriImage support DRM image @{ */
        UriImageDrmUtils.getInstance().loadUriDrmInfo(this);
        /* @} */
    }
...
//                    mFileDescriptor.getFileDescriptor(), options, targetSize, mType);
            Bitmap bitmap = UriImageDrmUtils.getInstance().decodeUriDrmImage(jc,
                    mType,mUri.getPath(),options,targetSize,mFileDescriptor.getFileDescriptor());
            /* @} */
    ...
    public int getSupportedOperations() {
        /* SPRD: bug613322 UriImage support DRM image @{ */
        int supported = 0;
        boolean drmFile = UriImageDrmUtils.getInstance().isDrmFile(mUri.getPath(), mContentType);
        if (drmFile) {
            supported = SUPPORT_DRM_RIGHTS_INFO;
        } else {
            // SPRD: Add for bug620619, do not show "set as" menu if decode failed
            if (mIsSupportSetAs) {
