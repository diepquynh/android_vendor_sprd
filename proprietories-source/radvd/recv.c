                 ...
                 struct in6_pktinfo **pkt_info, int *hoplimit)
{
	...
	syslog(LOG_ERR, "recv_rs_ra start");
	if( ! chdr )
	{
		...
		if ((chdr = malloc(chdrlen)) == NULL) {
			...
			syslog(LOG_ERR, "recv_rs_ra: malloc: %s", strerror(errno));
			return -1;
		}
	}
	...
	if( select( sock+1, &rfds, NULL, NULL, NULL ) < 0 )
	{
		if (errno != EINTR)
		{
			...
			syslog(LOG_ERR, "recv_rs_ra select: %s", strerror(errno));
		}
		syslog(LOG_ERR, "recv_rs_ra select: %s", strerror(errno));
		return -1;
	}
	...
	if (len < 0)
	{
		if (errno != EINTR)
		{
			...
			syslog(LOG_ERR, "recv_rs_ra recvmsg: %s", strerror(errno));
		}
		syslog(LOG_ERR, "recv_rs_ra recvmsg: %s", strerror(errno));
		return len;
	}
	...
	*hoplimit = 255;
          ...
          switch(cmsg->cmsg_type)
          {
              ...
              case IPV6_HOPLIMIT:
                ...
                else
                {
                  ...
                  syslog(LOG_ERR, "received a bogus IPV6_HOPLIMIT from the kernel! len=%d, data=%d",
                  	cmsg->cmsg_len, *(int *)CMSG_DATA(cmsg));
                  return (-1);
                }
                break;
              ...
              case IPV6_PKTINFO:
                ...
                else
                {
                  ...
                  syslog(LOG_ERR, "received a bogus IPV6_PKTINFO from the kernel! len=%d, index=%d",
                  	cmsg->cmsg_len, ((struct in6_pktinfo *)CMSG_DATA(cmsg))->ipi6_ifindex);
                  return (-1);
                }
                break;
	...
	syslog(LOG_ERR, "recvmsg len=%d", len);
