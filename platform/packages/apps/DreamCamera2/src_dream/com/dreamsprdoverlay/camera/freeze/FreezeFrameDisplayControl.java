...
public class FreezeFrameDisplayControl implements FreezeFrameDisplayView.ProxyFreezeFrameClick,Rotatable {
 ...
 /*   private FreezeFrameDisplayControl() {
    ...
    private void initialize() {
        layoutRoot = (FrameLayout)mActivity.findViewById(R.id.camera_app_root);

        LayoutInflater inflater = mActivity.getLayoutInflater();

        sView = (FreezeFrameDisplayView)
            inflater.inflate(R.layout.dream_preview_camera_freeze_frame_display, null);
    ...
    */
    ...
    public void proxyAnimation(boolean show) {
        ...
        /*SPRD: fix bug 615408 freeze orientation is not correct @{ */
        ...
        if (mOrientationManager != null) {
            if (mOrientationManager.getDeviceOrientation() != null)
                DreamOrientation.setOrientation(sView,
                        mOrientationManager.getDeviceOrientation().getDegrees(), true);
        }
        /* @} */
        if (show) {
            if (layoutRoot != null && sView != null) {
...
//            initialize();
    ...
    public boolean onBackPressed() {
        if(sView != null && sView.displayed()){
            sView.showAlertDialog();
            return true;
        }
        return false;
    }
