...
import com.dream.camera.ui.DreamVoiceImageView.DreamVoiceImageOnclickListener;
...
import com.sprd.camera.voice.PhotoVoicePlayer;
import com.sprd.gallery3d.aidl.IFloatWindowController;
import com.ucamera.ucam.jni.ImageProcessJni;
import com.ucamera.ucam.modules.ufilter.UcamFilterPhotoUtils;
...
public class CameraActivity extends QuickActivity implements AppController,
    ...
    private CameraManager.AvailabilityCallback mAvailabilityCallback = new CameraManager.AvailabilityCallback() {
        ...
        public void onCameraAvailable(String cameraId) {
            ...
            Log.i(TAG, "onCameraAvailable mBackCameraAvailable = " + mBackCameraAvailable
                    + ", mFrontCameraAvailable = " + mFrontCameraAvailable
                    + ", cameraRequestPending = " + requestPending);
        ...
        public void onCameraUnavailable(String cameraId) {
            ...
            Log.i(TAG, "onCameraUnavailable mBackCameraAvailable = " + mBackCameraAvailable
                    + ", mFrontCameraAvailable = " + mFrontCameraAvailable);
        ...
        public void onServiceConnected(ComponentName arg0, IBinder arg1) {
            ...
            try {
                if (iFloatWindowController.closeFloatWindow()) {
                    Toast.makeText(CameraActivity.this, R.string.close_float_video,
    ...
    private final CameraAppUI.BottomPanel.Listener mMyFilmstripBottomControlListener =
            new CameraAppUI.BottomPanel.Listener() {
                ...
                public void onEdit() {
                    ...
                    /**
                     * SPRD:fix bug620407 not use lastModified() at main thread
    ...
    private final FilmstripContentPanel.Listener mFilmstripListener =
            new FilmstripContentPanel.Listener() {
                ...
                public void onFilmstripShown() {
                    ...
                    // SPRD: Fix bug 533869 Add for voice picture of new ui
                    onStopRecordVoiceClicked();
                    dataAdapterLengthBeforeShow = mDataAdapter.getCount();
                    mLastPhotoId = mDataAdapter.getLastPhotoId();
                    fsItem = mDataAdapter.getItemAt(0);
                }
                ...
                @Override
                public void onScroll(int firstVisiblePosition, int visibleItemCount,
                        int totalItemCount) {
                    mPreloader.onScroll(null /* absListView */, firstVisiblePosition,
    ...
    /**
     * If 'visible' is false, this hides the action bar. Also maintains lights-out at all times.
     *
     * @param visible is false, this hides the action bar and filmstrip bottom controls.
     */
    ...
    private final SessionListener mSessionListener =
            new SessionListener() {
                ...
                public void onSessionQueued(final Uri uri) {
                    ...
                    Optional<SessionItem> newData = SessionItem
    ...
    public String getModuleScope() {
        ...
        String s = SettingsManager.getModuleSettingScope(agent.getScopeNamespace());
        return SettingsManager.getModuleSettingScope(agent.getScopeNamespace());
    }

    public String getRespondModuleScope(int modeIndex) {
        ModuleAgent agent = mModuleManager.getModuleAgent(modeIndex);
        String s = SettingsManager.getModuleSettingScope(agent.getScopeNamespace());
        Log.d(TAG, "AA getRespondModuleScope = " + s);
        return SettingsManager.getModuleSettingScope(agent.getScopeNamespace());
    }
...
//        }
        ...
        mainUiThumbnailRunnable = new Runnable() {
            ...
            public void run() {
                ...
                Log.i(TAG, "updateCaptureIndicatorThumbnail");
            }
        };
               ...
               // SPRD:fix bug 501739 when capture in burst mode a thumbnail appears
                ...
                if (mCurrentModule != null
                        && (!(mCurrentModule instanceof PhotoModule)
                                || mCurrentModule.isSetFreezeFrameDisplay())) {
                        ...
                        public void run() {
                            if (data.getItemViewType() == FilmstripItemType.PHOTO) {
                                try {
                                    ...
                                    InputStream stream = new FileInputStream(data.getData().getFilePath());
                            ...
                            // SPRD: fix bug 604793, Thumbnails are photos, not videos
                            if (mCurrentModule instanceof VideoModule) {
                                FilmstripItem videoCaptureData = mDataAdapter.getItemAt(0);

                                if (videoCaptureData != null
                                        && videoCaptureData.getItemViewType() == FilmstripItemType.PHOTO) {
                                    try {
                                        ExifInterface exif = new ExifInterface();
                                        InputStream stream =
                                                new FileInputStream(videoCaptureData.getData().getFilePath());
                                        exif.readExif(stream);
                                        Bitmap bitmap = exif.getThumbnailBitmap();
                                        if (bitmap != null) {
                                            indicateCapture(bitmap, Exif.getOrientation(exif));
                                        }
                                    } catch (Exception e) {
                                        Log.i(TAG, e.getMessage());
                                    }
    ...
    private void removeItemAt(int index) {
        ...
        /*
         * SPRD:fix bug 496611 when delete photos to the last one, Camera automatically returns to
         * preview interface
         */
...
//        profile.mark("computePictureSizes");
    ...
    public int getModeIndex() {
        int modeIndex = -1;
        int photoIndex = getResources().getInteger(R.integer.camera_mode_auto_photo);
        int videoIndex = getResources().getInteger(R.integer.camera_mode_auto_video);
        ...
        int videoIntentIndex =
                getResources().getInteger(R.integer.camera_mode_video_intent);
        ...
        */
            ...
            /**
             * SPRD:Add for new feature gif @{ original code if ((modeIndex == gcamIndex &&
             * !GcamHelper.hasGcamAsSeparateModule(mFeatureConfig)) || modeIndex < 0) { modeIndex =
             * photoIndex; }
             */

            if ((modeIndex == gcamIndex && !GcamHelper
                    .hasGcamAsSeparateModule(mFeatureConfig)) || modeIndex < 0) {
                modeIndex = photoIndex;
            }
            /**@}*/
        }
        if (mDataModule != null && mModeListView != null && !mModeListView.isSupportMC(modeIndex,mDataModule.getInt(Keys.KEY_CAMERA_ID)))//SPRD:fix bug647661
        {
    ...
    public void onPauseTasks() {
        ...
        getCameraAppUI().hideImageCover();
        getCameraAppUI().hideModeList();
...
/*                if (isSupportGps) {
        ...
        /* SPRD: Add for bug 552560
           BUT SPRD: delete for bug 598511, will make Camera be killed when media process be killed
           @{ * /
    ...
    private void resume() {
        ...
        mCurrentModule.resume();
         */
        if (mIsCameraRequestedOnCreate || (mFrontCameraAvailable && mBackCameraAvailable)) {
    ...
    public void onStartTasks() {
        ...
        if (!isCaptureIntent() && mDataModule.getBoolean(Keys.KEY_INTENT_CAMERA_SWITCH) ||
                isCaptureIntent() && mDataModule.getBoolean(Keys.KEY_CAMERA_SWITCH)) {
            int intentModeIndex;
            if (isCaptureIntent()) {
                intentModeIndex = getResources().getInteger(R.integer.camera_mode_capture_intent);
            } else {
    ...
    public void onDestroyTasks() {
        ...
        if (mPhotoVoicePlayer != null) {
            mPhotoVoicePlayer.releasePlayer();
        }
        /* @} */
        ...
        if (mFeatureConfig != null && mFeatureConfig.isUsingCaptureModule()) {
            if (mSoundPlayer != null) {
                mSoundPlayer.release();
            }
        }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (!mFilmstripVisible && !mCameraAppUI.isModeListOpen() && !mCameraAppUI.isSettingLayoutOpen()) {
            ...
            if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                ...
                return true;
            } else if (keyCode == KeyEvent.KEYCODE_MENU && mCameraAppUI.isModeListOpen()){
                mCameraAppUI.openModeList();
                return true;
            }
        }
    ...
    public void onModeSelected(int modeIndex) {
        Log.d(TAG, "onModeSelected mCurrentModeIndex=" + mCurrentModeIndex + ",change to "
                + modeIndex);
        ...
        /* SPRD: nj dream camera test debug 116 @{ *
        SRRD: add for Bug #596203 back camera mode is wrong when close camera and restart, useless
        boolean isHasEnteredFilter = DataModuleManager.getInstance(this)
                .getDataModuleCamera()
                .getBoolean(Keys.KEY_CAMERA_FILTER_ENTERED);
        if (modeIndex == getResources()
                .getInteger(R.integer.camera_mode_filter)) {
            DataModuleManager.getInstance(this).getDataModuleCamera()
                    .set(Keys.KEY_CAMERA_FILTER_ENTERED, true);
        } else if (modeIndex == getResources().getInteger(
                R.integer.camera_mode_photo)
                && isHasEnteredFilter
                && getCurrentModuleIndex() == getResources().getInteger(
                        R.integer.camera_mode_filter)) {
            DataModuleManager.getInstance(this).getDataModuleCamera()
                    .set(Keys.KEY_CAMERA_FILTER_ENTERED, false);
            isHasEnteredFilter = false;
        }
        if (modeIndex == getResources().getInteger(R.integer.camera_mode_photo)
                && isHasEnteredFilter) {
            modeIndex = getResources().getInteger(R.integer.camera_mode_filter);
        }
        @} */
...
//                    modeIndex);
            if (!isCaptureIntent()) { // Bug #533869 new feature: check UI 27,28: dream camera of intent capture
                DataModuleManager.getInstance(this).getDataModuleCamera()
                        .set(Keys.KEY_CAMERA_MODULE_LAST_USED, modeIndex);
            }
        }
        ...
        Log.d(TAG, "onModeSelected mCurrentModule=" + mCurrentModule + ",modeIndex=" + modeIndex);
    ...
    private void setModuleFromModeIndex(int modeIndex) {
        ...
        Log.e(TAG, "mCurrentModeIndex=" + mCurrentModeIndex + ",mCurrentModule=" + mCurrentModule);
    }

    @Override
    public SettingsManager getSettingsManager() {
        /* SPRD: Fix bug 583607 that null Exception happens @{ */
        if (mSettingsManager == null) {
            mSettingsManager = getServices().getSettingsManager();
        }
        /* @} */
     ...
     */
    /**
     * SPRD:fix bug 474673 add ucam edit @{ public void launchEditor(FilmstripItem data) { Intent
     * intent = new Intent(Intent.ACTION_EDIT) .setDataAndType(data.getData().getUri(),
     * data.getData().getMimeType()) .setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); try {
     * launchActivityByIntent(intent); } catch (ActivityNotFoundException e) { final String
     * msgEditWith = getResources().getString(R.string.edit_with);
    ...
    private void openModule(CameraModule module) {
        ...
        if (!mPaused) {
            android.util.Log.e("Dream", "module:" + module);
            module.resume();
            UsageStatistics.instance().changeScreen(currentUserInterfaceMode(),
                    NavigationChange.InteractionCause.BUTTON);
            updatePreviewVisibility();
    ...
    private void updateBottomControlsByData(final FilmstripItem currentData) {
        ...
        // SPRD: Fix bug 535110, Photo voice record.
        mPhotoVoicePlayer.updateVoicePlayButtonByData(currentData);
...
//    }
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mModuleMediaBroadcastReceiver != null) {
                Log.d(TAG, "on media onReceive mModuleMediaBroadcastReceiver != null");
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mCurrentModule instanceof VideoModule
                    || mCurrentModeIndex == getResources().getInteger(
                            R.integer.camera_mode_gif_video)) {
                flashId = ButtonManagerDream.BUTTON_VIDEO_FLASH_DREAM;
            } else if (mCurrentModeIndex == getResources().getInteger(
                    R.integer.camera_mode_gif_photo)) {
            ...
            if(intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED) && cameraId== 0){
                final int level = intent.getIntExtra(BatteryManager.EXTRA_LEVEL, 0);
                Log.i(TAG, "action_battery_changed level = " + level);
    ...
    private void storageUpdateThread(){
    ...
    public void switchFrontAndBackMode() {
        Log.d(TAG, "switchFrontAndBackMode ");
        /* Bug #533869 new feature: check UI 27,28: dream camera of intent capture @{ */
        if (!isCaptureIntent()) {
            mDataModule.set(Keys.KEY_CAMERA_SWITCH, true);
        } else {
...
//            modeIndex = getResources().getInteger(R.integer.camera_mode_capture_intent);
        ...
        /* SPRD: Fix bug 595400 the freeze screen for gif @{ */
        if (!(mCurrentModule.getModuleTpye() == DreamModule.GIF_MODULE
                || mCurrentModule.getModuleTpye() == DreamModule.FILTER_MODULE
                || mCurrentModule.getModuleTpye() == DreamModule.QR_MODULE)) {
    ...
    private class ModeSelectRunnable implements Runnable {
        ...
        public ModeSelectRunnable(int modeIndex) {
            super();
    ...
    public void syncThumbnail() {
        ...
        if (data != null) {
            ...
            if (!mSecureCamera) {
                defaultBitmap = android.graphics.BitmapFactory.decodeResource(getResources(), R.drawable.ic_gallery_defult_sprd);
            } else {
    ...
    public void onSettingReset() {
        ...
        if (mDataModuleManager.getCurrentDataModule() instanceof DataModulePhoto) {
            modeIndex = getResources().getInteger(R.integer.camera_mode_auto_photo);
        } else {
            modeIndex = getResources().getInteger(R.integer.camera_mode_auto_video);
        }
        final int finalModeIndex = modeIndex;
        if (mCurrentModule.getModuleTpye() == DreamModule.GIF_MODULE) {
            mCurrentModule.freezeScreenforGif(CameraUtil.isFreezeBlurEnable(), true);
        } else {
...
//                    mCurrentModule.updateParameter(Keys.KEY_CAMERA_SHUTTER_SOUND);
    ...
    public String getModuleScope(int modeIndex) {
        ModuleAgent agent = mModuleManager.getModuleAgent(modeIndex);
        return SettingsManager.getModuleSettingScope(agent.getScopeNamespace());
    }
    /* @} */
     ...
     * @return
     */
    public void onGifCancel(View v) {
        if (mCurrentModule != null) {
            mCurrentModule.onGifCancel();
        }
    }
     ...
     * @return
     */
    public void onGifFinish(View v) {
        if (mCurrentModule != null) {
            mCurrentModule.onGifFinish();
        }
    }
    /* @} */
    ...
    private PhotoVoicePlayer mPhotoVoicePlayer;
    ...
    private FilmstripItem fsItem = null;
    ...
    private boolean waitToChangeMode = false;//Fix bug 649158/628424
