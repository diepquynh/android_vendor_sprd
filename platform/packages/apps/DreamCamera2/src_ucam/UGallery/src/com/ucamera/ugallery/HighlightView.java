...
//                mOutlinePaint.setColor(0xFFFF8A00);
            ...
            if (mMode == ModifyMode.Grow || mMode == ModifyMode.Init) {
                if (mCircle) {
    ...
    void moveBy(float dx, float dy) {
        ...
        mCropRect.offset(Math.max(0, mImageRect.left - mCropRect.left), Math.max(0, mImageRect.top
                - mCropRect.top));

        mCropRect.offset(Math.min(0, mImageRect.right - mCropRect.right), Math.min(0,
                mImageRect.bottom - mCropRect.bottom));
    ...
    private Rect computeLayout() {
        ...
        mMatrix.mapRect(r);
        return new Rect(Math.round(r.left), Math.round(r.top), Math.round(r.right), Math
                .round(r.bottom));
    }
    ...
    private ModifyMode mMode = ModifyMode.Init;
