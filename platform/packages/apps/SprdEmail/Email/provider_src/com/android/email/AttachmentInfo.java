...
public class AttachmentInfo {
     ...
     *@{
     */
    public static final boolean ENABLE_SPRD_DENY_PROCESS = true;
    /*@}*/
     ...
     */
    public AttachmentInfo(Context context, long id, long size, String fileName, String mimeType,
            long accountKey, int flags, boolean isDenyMalware) {
        mSize = size;
        mContentType = AttachmentUtilities.inferMimeType(fileName, mimeType);
        mName = fileName;
        mId = id;
        ...
        // 1.Check whether deny
        String extension = AttachmentUtilities.getFilenameExtension(mName);
        if (!TextUtils.isEmpty(extension)) {
            if (isDenyMalware){ // need to filter attachment extension maybe potential malware
                if (Utility.arrayContains(AttachmentUtilities.UNACCEPTABLE_ATTACHMENT_EXTENSIONS,
                        extension)) {
                    denyFlags |= DENY_MALWARE;
                }
            } else { // No need to filter attachment extension
                LogUtils.i(LogUtils.TAG, "[AttachmentInfo]No need to filter attachment extension.");
            }
        }
        if ((flags & Attachment.FLAG_POLICY_DISALLOWS_DOWNLOAD) != 0) {
            denyFlags |= DENY_POLICY;
        ...
        if (mSize > AttachmentUtilities.MAX_ATTACHMENT_DOWNLOAD_SIZE) {
            int networkType = EmailConnectivityManager.getActiveNetworkType(context);
            if (networkType != ConnectivityManager.TYPE_WIFI) {
                LogUtils.i(LogUtils.TAG, "[AttachmentInfo]user request. flags:%x, fileName:%s", flags, fileName);
                if ((flags & Attachment.FLAG_DOWNLOAD_USER_REQUEST) == 0) {
                    denyFlags |= DENY_WIFIONLY;
                }
            }
        }

        // 2.set canView, canSave and canInstall
        if (denyFlags != ALLOW) {
            canView = false;
            canSave = false;
        } else {
            ...
            if (!TextUtils.isEmpty(extension) &&
                    Utility.arrayContains(AttachmentUtilities.INSTALLABLE_ATTACHMENT_EXTENSIONS,
                            extension)) {
                canInstall = Settings.Secure.getInt(context.getContentResolver(),
                    Settings.Secure.INSTALL_NON_MARKET_APPS, 0 /* sideload disabled */) == 1;
            }
        }
