...
cmr_int cmr_sensor_get_cnr_param(cmr_handle sensor_handle, cmr_uint sensor_id,  struct cnr_param **cur_cnr_param)
{
	cmr_int ret = CMR_CAMERA_SUCCESS;
	struct sensor_cnr_param_tag *snr_cnr_param = NULL;
	cmr_uint sensor_param = 0;

	snr_cnr_param = malloc(sizeof(struct sensor_cnr_param_tag));
	if (!snr_cnr_param) {
		CMR_LOGE("fail to malloc");
		return CMR_CAMERA_NO_MEM;
	}
	snr_cnr_param->cmd = SENSOR_EXT_CNR_PARAM;
	snr_cnr_param->cnr_thrs_param = NULL;
	//gain_thrs_param->param = param_ptr->cmd_value;

	ret = cmr_sensor_ioctl(sensor_handle, sensor_id, SENSOR_GET_CNR_PARAM, (cmr_uint)snr_cnr_param);
	*cur_cnr_param = snr_cnr_param->cnr_thrs_param;
	CMR_LOGI("--->	cmr_sensor_get_cnr_param = %p",snr_cnr_param->cnr_thrs_param);

	if(snr_cnr_param->cnr_thrs_param == NULL)
		ret = CMR_CAMERA_INVALID_PARAM;
	...
	switch (in_cmd) {
		...
		case SENSOR_ACCESS_VAL:
			*sns_cmd = SENSOR_IOCTL_ACCESS_VAL;
			break;
#if !(defined(CONFIG_CAMERA_ISP_VERSION_V3) || defined(CONFIG_CAMERA_ISP_VERSION_V4))
		case SENSOR_CLOSE_FLAG:
			*sns_cmd = SENSOR_IOCTL_CLOSE_FLAG;
			break;
#endif
		case SENSOR_YUV_FPS:
			*sns_cmd = SENSOR_IOCTL_FOCUS;
