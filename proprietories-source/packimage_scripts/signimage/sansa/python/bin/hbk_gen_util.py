...
def WriteOutputToFile(HASHwordList, buffData, size):
    fobHash = open(PRIM_KEY_HASH_FILE_NAME, 'w')
    fobZeroNum = open(ZEROS_NUM_FILES_NAME, 'w')
...
def main():
    ...
    WriteOutputToFile(HASHWordsList, str(hex(CalculateZeroBitCountOnData(HASHWordsList,outputWordSize))), outputWordSize)
