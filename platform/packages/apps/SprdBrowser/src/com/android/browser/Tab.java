...
import com.android.browser.util.LogUtil;
...
class Tab implements PictureListener {
    ...
    /* SRPD: Add for Commlog feature @{ */
    private static final int  BROWSER_EXCEPTION = 2;
    ...
    /* SRPD: Add for Commlog feature @{ */
    private void sendErrReport(int errorCode, String description, String failingUrl) {
        try {
            Intent intent = new Intent(ACTION_REPORT_ERRLOG);
            intent.putExtra("FaultId",BROWSER_EXCEPTION);
            intent.putExtra("SubId",SubscriptionManager.getDefaultDataSubscriptionId());
            intent.putExtra("ErrorCode",errorCode);
            intent.putExtra("Description",description);
            intent.putExtra("FailingUrl",failingUrl);
            mContext.sendBroadcast(intent,PERMISSION_RECEIVE_ERRLOG);
        } catch (Exception e) {
            Log.e(LOGTAG, "sprd commlog Tab sendErrReport exception !");
        }
    }
    /* @} */
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            LogUtil.i(LOGTAG, "WebChromeClient.onPageStarted: url is: " + url);
            mInPageLoad = true;
        ...
        public void onPageFinished(WebView view, String url) {
            LogUtil.i(LOGTAG, "WebChromeClient.onPageFinished: url is: " + url);
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
             ...
             */
            if (isSnapshot()) {
                LogUtil.d(LOGTAG,"TAB--------mWebViewClient->shouldOverrideUrlLoading() isSnapshot, url = " + url);
                return false;
            }
            /*@}*/
            LogUtil.d(LOGTAG,"TAB--------mWebViewClient->shouldOverrideUrlLoading()  url = "+url);
        ...
        public void onReceivedError(WebView view, int errorCode,
            ...
            /* SRPD: Add for Commlog feature @{ */
            sendErrReport(errorCode, description, failingUrl);
            /* @} */
    ...
    Tab(WebViewController wvcontroller, WebView w, Bundle state) {
        ...
        float density = DisplayMetrics.DENSITY_DEVICE_STABLE/(float)DisplayMetrics.DENSITY_DEFAULT;
        Log.d(LOGTAG, "screen density = " + density);
        mCaptureWidth = (int) (mContext.getResources().getDimensionPixelSize(
                R.dimen.tab_thumbnail_width) * density + 0.5f);
        mCaptureHeight = (int) (mContext.getResources().getDimensionPixelSize(
                R.dimen.tab_thumbnail_height) * density + 0.5f);
