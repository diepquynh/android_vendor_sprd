...
public class OneCameraCharacteristicsImpl implements OneCameraCharacteristics {
    ...
    @Override
    public Size[] getSupportedThumbnailSizes() {
        android.util.Size[] originalSize = mCameraCharacteristics.get(CameraCharacteristics.JPEG_AVAILABLE_THUMBNAIL_SIZES);
        Size[] sizes = new Size[originalSize.length];
        for (int i = 0; i < originalSize.length; i++) {
            sizes[i] = new Size(originalSize[i]);
        }
        return sizes;
    }
}
