...
void Sensor_SetExportInfo(struct sensor_drv_context *sensor_cxt)
{
	...
	for (i = SENSOR_MODE_COMMON_INIT; i < SENSOR_MODE_MAX; i++) {
		...
		if ((NULL != exp_info_ptr)
			...
			exp_info_ptr->raw_info_ptr->ioctrl_ptr->get_gain_scale = exp_info_ptr->ioctl_func_ptr->read_gain_scale;
			exp_info_ptr->raw_info_ptr->ioctrl_ptr->get_capture_gain = exp_info_ptr->ioctl_func_ptr->read_snapshot_gain;
			...
			exp_info_ptr->raw_info_ptr->ioctrl_ptr->ex_set_exposure = exp_info_ptr->ioctl_func_ptr->ex_write_exposure;
		}
...
const cmr_s8*  sensor_get_cur_name(void)
{
	struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)sensor_get_dev_cxt();
