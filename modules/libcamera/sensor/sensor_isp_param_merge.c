...
int  sensor_isp_param_merge(struct sensor_raw_info* sensor_info_ptr,struct isp_data_info* out_param ,int mode)
{
	...
	uint32_t alg_id = 0xFFFFFFFF;
	struct ae_param_tmp_001 *ae_tmp_001 = PNULL;
	...
	for (i = 0;i < src_mode_ptr->block_num;i++) {
		...
		if (src_header[i].block_id == ISP_BLK_AE_V1) {
			...
			ae_tmp_001 = (struct ae_param_tmp_001 *)ae_ptr;
			alg_id = ae_tmp_001->alg_id;
	...
	if (ae_index >= 0 ) {
		...
		/*for reserved*/
		if(2 == alg_id) {
			memcpy((void*)tmp_ptr,(void*)ae_tmp_ptr,OLD_PARAM_RESERVE_LEN*sizeof(uint32_t));
		} else {
